...
class Task(BaseModel):
    ...
    classification: Literal["Action", "Waiting", "Reference", "Optional", "Noise"] = (
        Field(description="Task classification for triage")
    )
    priority: int = Field(
        description="Priority level: 1=High, 2=Medium, 3=Low", ge=1, le=3
    )
    ...
    type: str = Field(
        default="generic",
        description="Task type: email_reply, slack_message, note, zoom_action, etc.",
    )
    project: str = Field(default="", description="Associated project identifier")
    due: str | None = Field(default=None, description="ISO 8601 timestamp of due date")
    preview: str | None = Field(
        default=None, description="Short preview text (max ~160 chars)"
    )
    ...
    action_required: str | None = Field(
        default=None, description="Specific action needed"
    )
    requires_reply: bool | None = Field(
        default=None, description="Whether task requires a reply"
    )
    time_estimate: float | None = Field(
        default=None, description="Estimated time in hours"
    )
    response_draft: str | None = Field(
        default=None, description="Draft response (for emails)"
    )

    # System fields (optional)
    archived_at: str | None = Field(
        default=None, description="ISO 8601 timestamp when archived"
    )
    completed_at: str | None = Field(
        default=None, description="ISO 8601 timestamp when completed"
    )
    ...
    @classmethod
    def from_email(
        cls, email_data: dict[str, Any], triage_result: dict[str, Any]
    ) -> "Task":
        ...
        def get_field(d: dict[str, Any], *paths, default=None):
        ...
        received_time = get_field(
            email_data, "receivedTime", "date", "Date", default=None
        )
