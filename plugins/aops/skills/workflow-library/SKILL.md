...
retired only in the body, by an opening `# RETIRED` heading or a `## Retired —
superseded by …` section, with no `status` field and no `retired` tag. Read each
document's opening lines, and treat `status: cancelled` as retired too.
...
- A retired template carrying no retirement marker. Until that is fixed, every
  other composing pass keeps picking it up.
...
a template carrying **either** `status: retired` **or** a `retired`/`superseded`
tag, so set both; a document with only one is still composed by half the passes.
