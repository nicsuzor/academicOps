...
2. **Search with varied phrasing**: Run 2-3 phrasing variants per term using MCP search tools.
   - Run each variant through `pkb.search`; it indexes subtasks as their own documents, so no separate task-listing call is needed to avoid missing open work.
...
4. **Emit flat shortlist**: Group by kind (tasks, knowledge, terms), omitting empty kinds:
   ...
   ```
   If a search yields nothing, state the exact queries and scope: `no prior task (pkb.search: '...', 0 hits)`.
