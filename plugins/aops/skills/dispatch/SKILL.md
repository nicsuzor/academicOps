...
**Claim at launch.** Before the worker starts, write the unit's claim onto the task record as a line beginning `Dispatched:` — who it went to, under which session, on which surface, when. Use that word: a reconcile sweep looks for it by name, and a claim phrased freely is one no later pass can find. Any dispatch whose loss would matter beyond its own session gets one; a cheap read-only probe does not, since nothing is lost by running it again. The worker still claims the task itself when it starts — that handshake is what moves the status and proves the brief arrived. Your launch-time record is what makes a worker that died before ever claiming legible as an unanswered dispatch rather than as work nobody picked up.
