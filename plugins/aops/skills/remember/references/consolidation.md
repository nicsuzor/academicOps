...
`consolidated:` marker, and closed tasks with substantive bodies. For each
source, apply the extraction method above — identify durable facts (models,
architecture, decisions, contacts, URLs); record an empty extraction if nothing
is durable; otherwise write to the canonical destination note first, verify
read-back by ID, rewrite the source task body in place under 1,500 characters,
and mark episodic notes `consolidated: <date>` and advance their status.
