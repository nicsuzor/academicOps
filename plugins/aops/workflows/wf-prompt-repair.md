...
- critique
- prompt-engineering
title: wf-prompt-repair
type: template
---

# wf-prompt-repair — repair the instructions that produced a failing output
...
**Authoring standard lives in `craft`** (`plugins/aops/skills/craft/`). This template states only
the process the repair runs under — what gets critiqued, diagnosed, and validated, and in what
order; whether the replacement text is good is craft's question, not this one.
...
2. **Synthesis — depends on all critiques.** Reconcile them: independent convergence is strong
   ...
   diagnosis per failure; (b) replacement instructions, held to the `craft` standard, that prevent
   each failure _class_, not just its instance; (c) a checklist a reviewer can score — one check per
   substantive difference between old and new instructions, on substance, never expression or style.
3. **Validation — depends on synthesis; the evaluator wrote none of it.** Regenerate an output
   under the replacement or test it adversarially, and score the checklist: every diagnosed failure
   excluded, no new failure mode introduced. One failed validation returns to synthesis; a second is
   a finding about the diagnosis.
...
validation; the replacement passes the `craft` standard; the original location holds it.
