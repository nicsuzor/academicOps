...
(`tools:peer-review`): the map extracts what the document _says about itself_, and stops there.

## Note

The four-part frame is Nic's own reading preference, not a validated universal structure. A
research task exists to establish what is genuinely universal across academic work of any genre;
this template will be revised against its findings once that lands.
...
- **Extract, don't infer.** If the document doesn't state it, write `Not stated`. An empty heading
  is the most useful thing the map produces.
- **Distinguish claimed from demonstrated.** Contribution bullets record what the author _claims_
  is new; checking the claim is a separate pass (`tools:peer-review`).
- **Ignore the section structure.** Aims, method and theory are scattered through prose; the
  abstract or the closing paragraph of the introduction usually carries the whole map.
- **One pass, no dialogue.** Produce the map, then stop.
...
Source: a DSA transparency grant proposal, closing paragraph of the introduction.
...
- Mixed methods: comparative qualitative analysis of implementation documents; surveys and
  interviews with regulators and platforms; longitudinal analysis of a transparency database.
- Scope extended to underexamined actors: European-based platforms and regulatory bodies.

**Analytical theory**

- Treats the regime as a space where actors negotiate and institutionalise competing ideas of
  platform governance.
- Treats transparency as a dynamic, negotiated process rather than a static regulatory outcome.

**Contribution**

- _Methodological_: a novel longitudinal method leveraging NLP over the transparency database.
- _Empirical_: coverage of European platforms and regulators beyond the usual major US platforms.
- _Theoretical_: a typology of transparency models, and an account of transparency as a structure
  through which actors assert and consolidate power.
