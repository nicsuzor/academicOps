...
1. **Behavior Over Implementation:** Test what the code DOES, not what it LOOKS LIKE. Test inputs, transformations, invariants, and side effects via public APIs.
2. **No Tautological or Mirror Tests:** Never write tests that merely assert source strings, hardcoded constants, or mirror-image return values.
3. **Triangulation Requirement:** Write at least 2–3 distinct test cases with varied inputs and outputs for any non-trivial logic before considering it covered.
4. **Boundary & Error Cases:** Every test suite must cover edge cases (e.g., empty input, boundary limits) and error handling (e.g., invalid arguments, expected exceptions).
5. **Falsifiability:** A valid test must be capable of failing if the business logic is broken or modified.
