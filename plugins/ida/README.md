...
`pc` has one mode: a detached `polecat run agy` under tmux, given a task id,
returning the moment the container is up. There is no synchronous run and no
return path. The worker writes its result onto the task record and pushes its
branch; ida learns what happened by asking pauli, not by waiting.
...
| Agent | `pc`         | The polecat launcher. Takes a task id, starts a detached container, returns.                                                                          |
