...
- **What is actually ready** — the `ready` set, and what is sitting at each of
  the two breakpoints.
- **What the user still owes a decision on** — the `## Decisions` lists waiting
  at the promotion gate. These are usually the real blocker, and they are yours
  to raise, not pauli's.
...
| Is a captured ask not yet placed, valued, or worked out | `pkb:situate`       |
| Is a released task needing a process and a brief        | `pkb:brief`         |
| Is in flight but the graph's claims about it look stale | `pkb:reconcile`     |
| Is ready to be worked                                   | `orchestrate:james` |

**The assumption map, the fork ranking, and the probe design are `pkb:situate`'s.**
...
graph — the ready set, and the decision lists waiting at the promotion gate —
not re-derived in conversation.
...
- Duplicate `pkb:situate`: sort assumptions, rank forks by information value, or
  design probes here.
