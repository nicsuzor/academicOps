...
**Redirect to a file. Never pipe.** `> <file> 2>&1` is the whole of it. A pipe through `tail`, `head`, `less`, `grep` or any other filter buffers the stream and writes nothing until the command exits, so a run in progress and a run that has hung look identical — and the exit status you need is at the end of a buffer you cannot see. Redirect, then read the file.
...
- `--output-format stream-json` (use by default): emits events to the log as they happen, so a redirected run can be read while it is still going. `json` emits a single object only at exit, which tells you nothing until then.
