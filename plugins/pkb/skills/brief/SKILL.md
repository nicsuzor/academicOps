...
The brief goes onto the task body. Review obligations and acceptance criteria are written directly on the task body rather than emitted as speculative standalone graph nodes. That is the whole of your output.
...
arrived as prose — and the children a cut in §4 forces. Never emit speculative review or sign-off nodes into the graph (§6). Everything else you produce is body content and edges.
...
### Specify the composed process

Document each step, gate, and acceptance criterion directly in the task body.
...
- **OUTER LOOP:** acceptance criteria, verification gates, and external dependencies that the unit must satisfy before completion. Record these obligations in the task body as Acceptance Criteria (AC). Do not pre-create speculative child or review nodes in the graph; record obligations on the task body so they govern execution and review without creating orphaned graph nodes.

## 6 — Specify review obligations on the task body (never emit speculative review nodes)

Review obligations are **concrete acceptance criteria recorded on the task body**, never speculative graph nodes created ahead of execution.

Pre-creating paired review or sign-off nodes before execution creates orphaned nodes in the dispatch queue when work lands via PR or direct completion (see [[kb_dispatch_churn]] §A). Nic's merge is itself the sign-off — never emit a separate human sign-off task node.

The process you just composed names the review it obliges; record each obligation as a clear acceptance criterion with required evidence. If post-landing independent verification is genuinely needed, the verification operates against the PR or merged artifact, not through a pre-emitted queued node.
...
- Promote a task you were not called on, or emit speculative review or sign-off nodes into the graph.
...
call behind it, and see clear acceptance criteria for every obligation that gates
acceptance recorded in the task body — without speculative review or sign-off nodes in the graph (Nic's merge is the sign-off).
