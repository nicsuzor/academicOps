...
2. **Assemble workflow**: Select relevant templates across project (`$CWD/.agents/templates/`), universal (`../../workflows/`, relative to this skill's own directory -- resolves inside an installed marketplace plugin with no academicOps source checked out), and PKB (`type: template`) tiers. Halt if a required process component is missing.
