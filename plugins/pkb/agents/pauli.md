...
- **MCP tools only**: Access the PKB exclusively via the `services` MCP server's tools (`mcp__services__*` -- code-mode: `listToolFiles` → `readToolFile("servers/pkb.pyi")` → `executeToolCode` calling `pkb.<op>(...)`) or dedicated skills (`/aops:remember`, `/aops:brief`).
