...
**WRONG**: Trying to call `email` or `tasks` directly → These are NOT tools, they're skill names

**CORRECT**: Call `Skill` tool with skill name as parameter → `Skill(command="email")` loads the email skill's instructions
