...
**Problem**: `make kill_api` fails with "command not found" or process errors. **Solution**:
...
**Problem**: `--wait` option fails with "requires argument" error. **Solution**: Always provide a numeric value: `--wait 5` instead of just `--wait`

**Problem**: Commands hang or timeout. **Solution**:
