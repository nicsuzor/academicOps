...
- **When writing or modifying tests**, invoke `Skill(skill='python-dev')` first.
  The testing philosophy requires: real captured data (not fabricated), assertions
  on observable outcomes (not internal state), and mocking only at system boundaries.
...
The PR will be reviewed by automated agents (axiom-review, review-and-fix, merge-prep).
