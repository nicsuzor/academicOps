...
domain:
  - planning
  - operations
allowed-tools: Read,mcp__pkb__search,mcp__pkb__task_search,mcp__pkb__get_task,mcp__pkb__append,mcp__pkb__retrieve_memory,mcp__pkb__get_semantic_neighbors,mcp__pkb__get_dependency_tree
version: 1.0.0
permalink: skills-hydrate
---
...
Supersedes `aops-pkb/skills/hydrate/` (the `hydrator` skill). That skill also selected and composed
full process workflows into the task body — this skill does not: workflow/regime composition moved
to `decompose`. Don't resurrect that step here.
