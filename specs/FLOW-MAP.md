...
| execute → Stop reminder              | first clean `Stop` of a session                                                | `exit_reflection_reminder` gate — injects `handover.md`, warn, once per session, per-session JSON state; skips while `background_tasks` pending (absorbed `router.py`'s Stop branch, task `aops_cace51f9`)                                                                                 | ✅ `aops/hooks/gates/registry.py`, `gates/state.py`                                                                      |
...
| execute → **PKB** (via Stop)         | `Stop`                                                                         | `handover.md` injected by the `exit_reflection_reminder` gate — a warn, cannot block exit (today)                                                                                                                                                                                          | ✅ `aops/templates/hooks.template.json`, `gate_dispatch.py`, `gates/exit_reflection.py`                                  |
...
| human approval → merge               | approved SHA                                                                   | one-way door — a distinct, named human authorisation, not agent judgment                                                                                                                                                                                                                   | ✅ [[wf-human-approval]] (PKB template)                                                                                  |
...
8. **`router.py` retirement.** Two hook systems fire on the same events (router reminder injections
   - gate engine). Target: router's SessionStart env plumbing stays as infra; its reminders become
     ...
     **Progress:** the `Stop` reminder is consolidated — router's Stop branch is deleted and
     `exit_reflection_reminder` now injects `handover.md` with the `background_tasks` skip preserved
     (task `aops_cace51f9`, 2026-07-23). `SubagentStop`/`UserPromptSubmit`/`PostToolUse` reminders
     still live in `router.py`.
