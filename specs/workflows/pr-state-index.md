...
4. **Efficiency**: The `body` field must be truncated to approximately 2KB per PR to keep the artefact size manageable. Per-PR projection further drops fields no consumer reads: `author` is reduced to `{login, is_bot}`; each `statusCheckRollup` entry is reduced to `{name, conclusion, status, context, state}` (CheckRun entries carry the first three, StatusContext entries — the pipeline agent statuses like `mechanic-status` — carry `context`/`state`, which `apply_triage` reads to tell a halted pipeline from transient red CI); `recent_merged`/`recent_closed` entries omit `statusCheckRollup`, `mergeable`, `reviewDecision`, `isDraft`, and `labels` entirely (only open PRs need bucketing signals).
