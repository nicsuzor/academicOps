...
4. **Continuous Compliance (enforcer)**: The framework periodically audits the session for scope drift and principle violations. The user expects immediate notification (and session halting in block mode) if the agent diverges from the original intent or violates core axioms.
...
| Continuous Compliance    | Layer 2.5: enforcer periodic audit    |
