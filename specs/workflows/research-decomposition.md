...
This spec composes with the general decomposition workflow in `plugins/pkb/skills/decompose/SKILL.md` and the conceptual review workflow. It does not replace either. It provides the domain-specific primitives, lenses, and sequencing rules that those upstream systems use when the domain is academic research.
...
The existing `plugins/pkb/skills/decompose/SKILL.md` provides general decomposition infrastructure:
...
1. **Decomposer agent produces plan.** The agent uses the research primitives, the sequencing rules from this spec, and the Academic Output Layer from `plugins/pkb/skills/decompose/SKILL.md`. It runs the post-decomposition self-checks (decision prerequisites, execution gating, writing data dependencies, methodology layer). It produces all four output items: assumptions table, task graph, dependency visualisation, and MVC.
...
- Integration with effectual planner and `plugins/pkb/skills/decompose/SKILL.md`
