...
id: workflows-9ded9ca7
modified: 2026-05-17T00:43:31.132763313+00:00
...
Users should expect a system that prioritizes **traceability**, **quality gates**, and **epistemic integrity** over immediate execution.

1. **Work is Enqueued, Not Direct**: For any request that involves modifying files, adding features, or complex investigation, the main agent acts as a **Dispositor**. It will classify the intent, create or update a task, and prompt the user to run `/pull`. Users should expect this "stop and enqueue" behavior as the default for all non-trivial work.
2. **Constraint-Checked Planning**: Every execution plan (TodoWrite) generated by the hydrator is verified against the logical constraints of the selected workflow (e.g., TDD, feature-dev). If a plan violates constraints (like skipping tests or planning without approval), the user will see a "Constraint Violations" report and a corrected plan.
3. **Enforced Quality Gates**: When executing work via `/pull`, the agent strictly follows the **Verification Loop**. Users can expect that "tests pass" is a hard requirement for completion, and that complex tasks will produce independent evidence of correctness before being marked `merge_ready`.
4. **Clear Audit Trail**: Every change in the codebase is linked to a task ID that documents the context, the workflow followed, and the constraints satisfied. This ensures that the "why" behind every change is always discoverable and verified.
...
- Parent task: [[aops-a31d483c]]
  **2026-05-17 00:43 UTC** — [sleep 2026-05-17] Flagged as stale: `in_progress` since 2026-04-27 (≥20 days). Confirm whether still active or should be moved to `paused`/`ready`.
