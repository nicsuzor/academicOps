...
| **v1 fixer** = `agent-merge-prep.yml` + `merge-prep-cron.yml` + `merge-prep.agent.md`                                                                                                                                                                                                                                                                                                                             | **RETIRED**   | `agent-merge-prep.yml` + `merge-prep.agent.md` deleted at Phase 5; `merge-prep-cron.yml` survived as a live zombie (cron `*/30`, dispatching the non-existent `agent-merge-prep.yml` → 100% failure) until aops-886351fd deleted it; vestigial `merge-prep-status` carry-forward removed from `pr-pipeline.yml` `initialize` at Phase 5 |
...
`workflow_run` dispatcher + `.github/agents/merge-prep.agent.md` behaviour) is **retired**.
`agent-merge-prep.yml` and `merge-prep.agent.md` were deleted at Phase 5; `merge-prep-cron.yml`
survived as a live zombie (cron `*/30 * * * *`, dispatching the non-existent `agent-merge-prep.yml`
→ 100% failure, verified 5/5) until it was deleted in aops-886351fd. This section is **retrospective**: it preserves the still-true behavioural
...
- **Phase 5 — Stage-2 dev/mechanic. DONE / LIVE.** `agent-mechanic.yml` +
  ...
  fixer). The vestigial `merge-prep-status` carry-forward in `pr-pipeline.yml`'s `initialize`
  is **deleted**. (`merge-prep-cron.yml` was intended to be deleted here but survived as a
  zombie — dispatching the already-deleted `agent-merge-prep.yml` every 30 min → 100% failure;
  actually removed in aops-886351fd.) The hardcoded `origin/main`
...
- `.github/workflows/{agent-merge-prep,merge-prep-cron}.yml` + `.github/agents/merge-prep.agent.md`
  — the v1 transitional fixer (RETIRED; all three deleted — `agent-merge-prep.yml` + `merge-prep.agent.md` at Phase 5, `merge-prep-cron.yml` in aops-886351fd; behaviour inherited per §8).
