...
| **Unresolved third-party comment/review gate** (§3.10.1, Phase 9, PR #2094): fail-closed check over inline comments + review bodies from anyone other than our own automation; runs on every push and at admission; feeds `decide-mechanic`'s lowest-priority `review-response` dispatch condition                                                                                                                | **LIVE** (ruleset entry: **in-ruleset, apply pending**) | `pr-pipeline.yml` `comment-triage-status` job; `admit-on-review.yml` `admit-comment-triage` job; `scripts/ci/check-unresolved-comments.sh`; `tests/test_check_unresolved_comments.py`                                                                                               |
...
the SHA it ran on must reflect only what it is _handing off unresolved_ — never a problem it
...
6. **An agent's own terminal status for the SHA it ran on must never go red for a finding it
   ...
   _orchestrator_ gates downstream agents on `committed`, never on verdict colour; pt 6 is
   about how an agent computes _its own_ verdict when it both fixed something and posted a
...
### 3.10.1 Unresolved third-party comment/review gate — **LIVE** (Phase 9, PR #2094)

**The gap.** §3.10 above dispatches the mechanic in `review-response` mode only when a
**human, write-class maintainer** submits a `CHANGES_REQUESTED` review. Third-party reviewers
— GitHub Copilot, Gemini — post their review as **`COMMENTED`**, never `CHANGES_REQUESTED`.
Such a review carries no commit-status signal of its own, so nothing else in the merge gate
(`enforcer-status`, `qa-status`, mergeability) ever reads it either. Result (PR #2094,
2026-07-04): two Copilot inline comments landed, the maintainer approved ~74s later,
enforcer/qa/admit were already green, and the PR auto-merged ~50s after approval — before
fixes for either comment existed. F5's "read ALL reviews — framework agents + Gemini +
Copilot + humans" (§8) was a written contract with no live trigger for this case; v1's
`merge-prep` used to satisfy it by re-checking every reviewer's state on **every** green PR,
but that unconditional run was optimized away at Phase 5 as a "wasteful no-op" (§8), which
silently deleted the only hook that read third-party comments before merge.

**The fix — `comment-triage-status`, a new REQUIRED, fail-closed check.** Independent of
`enforcer-status`/`qa-status`/review _state_ (COMMENTED vs CHANGES_REQUESTED), this check asks
one deterministic question over `scripts/ci/check-unresolved-comments.sh`: is there an inline
review-comment thread, or a COMMENTED/CHANGES_REQUESTED review body, from anyone other than
this repo's own automation, that nothing has replied to since? Presence, not content — the
mechanic's `review-response` mode does the qualitative triage once dispatched.

- Posted on **every push** by `pr-pipeline.yml`'s `comment-triage-status` job, and
  re-verified at admission by `admit-on-review.yml`'s `admit-comment-triage` job — so a
  comment/review landing at any point in the PR's life (not just pre-admission) blocks merge
  until triaged.
- Fail-closed: any `gh api` fetch error, or a step that fails to run at all, posts `failure`
  — matching `review-attestation.sh`'s convention (§3.7) for this class of required check.
  Consistent with `strict_required_status_checks_policy` being a GitHub-native mechanism: an
  already-armed `gh pr merge --auto` (§3.2) does not complete until **every** required check,
  including this one, reads `success` — no reordering of `admit-pr.sh`'s existing arm-merge
  call was needed.
- `admit-on-review.yml`'s `decide-mechanic` job also reads this check's output (via
  `admit-comment-triage`'s job output, no duplicate fetch) as a **third** dispatch condition,
  lowest priority after red-reviewer and CONFLICTING: reviewers green + not conflicting + an
  unresolved comment/review remains → dispatch the mechanic in `mode: review-response`
  specifically (not the Stage-2 default, which carries no comment-triage mandate — passing
  the wrong mode here would silently no-op on exactly this case). If a PR matches a
  higher-priority condition too, comment-triage is deferred to the mechanic's next pass; safe
  either way because the REQUIRED check, not the dispatch decision, is what blocks merge.
- `mechanic.agent.md`'s `review-response` mode scope-fetch now also reads `COMMENTED` reviews
  (previously `CHANGES_REQUESTED` only) — closing the same COMMENTED-vs-CHANGES_REQUESTED gap
  inside the mechanic's own fetch, not just the dispatch trigger.
- Release-please PRs auto-satisfy `comment-triage-status` via `release-autogreen`, same as the
  other four agent/human required checks (§3.13) — deterministic bumps carry no third-party
  review comments to triage.

**Enforcement-change cost-benefit analysis** (per `specs/enforcement/enforcement.md` §4.1):

1. **Recurrence** — this is the second incident of this exact shape: PR #2094 merged past
   unaddressed Copilot comments; the v1 `merge-prep` retirement (Phase 5) removed the only
   prior defense without anything replacing it, so every admitted PR since has carried the
   same latent exposure — not a one-off.
2. **Benefit** — restores F5's "read ALL reviews" as a proactively enforced, always-on gate
   instead of an unenforced written contract; closes the specific COMMENTED-review blind spot
   that both the merge gate and the mechanic's own review-response fetch shared.
3. **Cost of NOT fixing** — a maintainer's approval can be silently followed by a merge that
   never reflects valid third-party feedback, undermining the reason third-party review tools
   are enabled at all.
4. **Ongoing cost** — one extra job per push (`comment-triage-status`) and one extra job per
   admission (`admit-comment-triage`): a checkout + 3 paginated `gh api` reads + one status
   POST each, no agent/LLM invocation. Materially cheaper than enforcer/qa; comparable to
   `review-attestation`.
5. **Reversibility** — fully reversible: remove `comment-triage-status` from
   `.github/rulesets/pr-review-and-merge.yml`'s required checks (with `scripts/sync-ruleset.sh`
   or the GitHub UI) and delete the two posting jobs. No state migration; no other mechanism
   depends on this context existing.

**Applying the ruleset.** `.github/rulesets/pr-review-and-merge.yml` records
`comment-triage-status` as required (kept in sync by
`scripts/validate-ruleset-alignment.sh`), but — like other ruleset-file changes — a maintainer
must run `scripts/sync-ruleset.sh` (or add it via the GitHub UI) to apply it to the _live_
branch protection; see the same **in-ruleset (apply pending)** convention `review-attestation`
uses in `specs/ENFORCEMENT-MAP.md` §3. Until applied live, `comment-triage-status` is posted
informationally but does not yet block merge.
...
_before_ `gh pr review` is called, so the posted review's `commit_id` is the new SHA — never
...
marketplace installs from (§ [releasing.md](releasing.md)). `build-extension.yml`'s "Publish
...
- type: required_status_checks
  parameters:
    ...
    required_status_checks:
      ...
      # Fail-closed unresolved third-party comment/review gate (§3.10.1, PR #2094)
      # — in-ruleset, apply pending (not yet applied to the live branch protection)
      - context: "comment-triage-status"
...
- **F6 — Refuse to approve while any `CHANGES_REQUESTED` stands.** The success path counts
  ...
  merging silently.
  > **Gap PR #2094 exposed, closed at Phase 9 (§3.10.1):** F5/F6 as written above describe
  > human `CHANGES_REQUESTED` reviews. Nothing dispatched the mechanic (or blocked merge) for
  > a **third-party** review — Copilot, Gemini — because those post as `COMMENTED`, never
  > `CHANGES_REQUESTED`. The contract existed on paper; nothing triggered it for this case, so
  > a PR could carry valid, unaddressed Copilot feedback and still auto-merge the moment
  > enforcer/qa/admit went green (PR #2094, 2026-07-04). `comment-triage-status` (§3.10.1)
  > is the fix: a REQUIRED, fail-closed check, independent of review _state_, that restores
  > "read ALL reviews" as an always-enforced gate rather than an unenforced aspiration.
