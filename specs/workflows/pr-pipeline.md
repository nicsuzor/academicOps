...
| **Fail-closed liveness + named-reviewer-on-this-SHA attestation** (`review-attestation` job + status; a dead/skipped pipeline reads RED, not as a silent pass; §3.7)                      | **LIVE** (in-repo); ruleset entry **LIVE in file**, live-apply is a deploy step | `pr-pipeline.yml` `review-attestation` job + `scripts/ci/review-attestation.sh` + `tests/test_review_attestation.py`; ruleset file lists `review-attestation` (apply via `scripts/sync-ruleset.sh`) |
| **Inverted review gradient** — a red enforcer VERDICT no longer gates `qa` off (a failing PR gets MORE review, not less; §3.4, #1450)                                                     | **LIVE**                                                                        | `pr-pipeline.yml` `qa` job `if:` tolerates `needs.enforcer.result == 'failure'`                                                                                                                     |
