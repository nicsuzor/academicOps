...
**Third-party/reviewer comment guard (#2094, `scripts/ci/check-unresolved-comments.sh`).** `decide-mechanic` also dispatches the mechanic whenever any inline PR review comment thread — from anyone other than this repo's own automation (`claude[bot]`, `botnicbot`, `github-actions[bot]`) — has zero replies, regardless of reviewer colour or mergeability. This restores the retired v1 `merge-prep`'s "read ALL reviews" mandate (F5 above: "framework agents and Gemini and Copilot and humans") as a dispatch signal. It closes a real gap: Copilot's code review posts as `COMMENTED`, never `CHANGES_REQUESTED`, so it never trips the §3.10 review-response path (wired to `review.state == 'changes_requested'` only) and it carries no commit-status signal at all — so a PR with green `enforcer-status`/`qa-status` and valid-but-unaddressed Copilot comments previously sailed straight to auto-merge with nobody reading them. Worked example: PR #2094 (2026-07-04) — two Copilot comments landed, the maintainer approved 13 seconds later (before reading them), admit/enforcer/qa were already green, and the PR auto-merged 50 seconds after approval with neither comment addressed. The check is content-blind: it only asks whether a thread has ANY reply (from a human or a bot), never whether the reply is correct — the actual triage remains the mechanic's job once dispatched.
...
- `admit-on-review.yml` (§3.2), on a maintainer's PR **review approval**, does: (1) sets
  ...
  - **not green** (absent because the §3.1 gate skipped the reviewers on pre-admission
    ...
    `decide-mechanic` re-reads the settled reviewer state and checks for unaddressed review
    comment threads (`check-unresolved-comments.sh`, #2094), and dispatches the mechanic's
    first pass if red/pending remains, the PR is `CONFLICTING`, **or** any review comment
    thread is unaddressed; only when all three are clear does auto-merge fire with no
    mechanic.
