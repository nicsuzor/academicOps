...
This is the detail catalogue referenced from `specs/enforcement.md` §6. For the design statement — why enforcement is shaped the way it is, how the pipeline and pyramid views relate, and how the evidence loop closes — read `specs/enforcement.md` first. For the failure-mode registry that maps problems to the mechanisms currently catching them, read `specs/enforcement-map.md`.

Entries below use the fixed schema declared in `specs/enforcement.md` §6 and are organised by **pipeline layer** (L0 → L11 → Evidence loop), not by pyramid tier. Mechanisms that span pyramid tiers (e.g. `/dump` runs as middle-tier during normal handover, tip-tier when the handover gate blocks Stop) carry both tiers with a conditional clause. Status fields are derived from source files where observable; unverifiable fields are flagged "verify".
...
- `specs/enforcement.md` — design statement (read first)
