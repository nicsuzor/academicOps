...
| `asyncRewake` (Stop, exit 2) — **RETIRED (#2181)**      | ✗ | ✓ | ✓ | Removed: this Claude-only quiet-split (full body → agent `<system-reminder>`, one-line summary → user) silently discarded exit-0 JSON `decision:block` output from other Stop gates sharing the same hooks.json entry. `hooks.json`'s `asyncRewake`/`rewakeMessage`/`rewakeSummary` keys and `router.async_rewake_body_for` no longer exist. The `ida·reminder` warn-solo case now rides the `additionalContext` (Stop, no-block) row above via `router.ida_warn_solo_decision_for`. Discriminating measurements: `tests/hooks/fixtures/pty_capabilities.json` cells `stop-jsonblock-plain` / `stop-jsonblock-asyncrewake` / `stop-jsonblock-asyncrewake-agentpin` / `stop-jsonblock-plain-agentpin`. |
