...
stamped `due=today` on an Errand X task as an ad hoc "intent marker" and Nic
reverted it hard.
...
Two related hook-text changes were referenced, not specified, here: the
honesty-at-Stop notice's firing cadence, and background-task notifications
moving to "act, don't surface unless important." Both are wiring work on the
enforcement side (see [`specs/enforcement/hook-gate-system.md`](../enforcement/hook-gate-system.md)),
not attention-surface contract content, and are out of scope for this document.

## Gate user-visibility

The enforcement layer (see [`specs/enforcement/hook-gate-system.md`](../enforcement/hook-gate-system.md)) decides how a gate's verdict is delivered, and every gate verdict reaches the agent's context — that is a single enforcement invariant, not a per-gate table. (Shipped verdicts are non-blocking reminders; the old block/warn mode machinery was deleted.) What is a genuine per-gate CHOICE is whether anything _also_ reaches the user terminal. That choice is a surfacing decision, not an enforcement one — how much of the enforcement machinery Nic should see, on the same "storage ≠ surfacing" logic that governs the rest of this contract — so it lives here.
...
Whether a given (client, event) pair can actually _deliver_ a user-visible message at all is a lower-level capability question, answered by [`CLIENT-TRANSLATION.md`](../CLIENT-TRANSLATION.md)'s per-client channel matrix — this section is the CHOICE made against that capability, not the capability itself. Verified against `aops-core/lib/gates/definitions.py` + `aops/hooks/templates/*.md` (source of the wire field each row cites) — a `message_key`/`message_template` on a `GatePolicy` always renders real text; it is never silent by omission.

> **⚠ Updated 2026-07-23 to shipped reality.** The prior version of this table described block/warn `IDA_GATE_MODE` modes, `exit_reflection` FULL/LITE tiers, and a degraded escape-hatch — **all deleted** in the v0.4 hook simplification. Nothing blocks now; every reminder is non-blocking. Mechanism SSoT: [`specs/enforcement/hook-gate-system.md`](../enforcement/hook-gate-system.md). Source: [`aops/hooks/router.py`](../../aops/hooks/router.py), [`aops/hooks/gates/`](../../aops/hooks/gates/).

| Reminder · fire (event)                                  | Blocks? | Delivery                                                                                                                            |
| :------------------------------------------------------- | :------ | :---------------------------------------------------------------------------------------------------------------------------------- |
| `exit_reflection` (first Stop of session)                | no      | `warn` → `additionalContext` (agent-directed; on Claude also renders to the user as a "Stop hook feedback" line — not user-silent). |
| handover reminder (Stop)                                 | no      | `systemMessage` (user-visible) + `additionalContext` (`handover.md`).                                                               |
| honesty reminder (SubagentStop)                          | no      | `systemMessage` + `additionalContext` (`honesty.md`).                                                                               |
| hydrate nudge (UserPromptSubmit)                         | no      | `systemMessage` + `additionalContext` (`hydrate.md`).                                                                               |
| verify reminder (PostToolUse, `tool==Agent`)             | no      | `systemMessage` + `additionalContext` (`verify.md`).                                                                                |
| `require_subagent_model` (PreToolUse, Agent w/o `model`) | no      | `warn` → `additionalContext`.                                                                                                       |
...
| :------------------------- | :----------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `exit_reflection.bound`    | `ephemeral`  | Fires on `update_task` PostToolUse (`in_progress`); Claude's PostToolUse carries a user_message channel, so "▶ Task bound. Full exit-reflection required before exit." (`exit_reflection.bound`) is user-visible.                                                                   |
| `exit_reflection.complete` | `ephemeral`  | "◇ Exit-reflection complete." (`exit_reflection.complete`) — fires on a reflection-auditor run or any other legal exit; same PreToolUse/SubagentStart/SubagentStop caveat as the former `enforcer.verified`/`rbg_review.complete` rows (aops_4c2949d9 — folded into this one ping). |

**Retired (aops_4c2949d9).** The turn-based `rbg` gate's PreToolUse pings (`enforcer.verified` reset, the block/countdown/dispatch rows above) no longer exist — nothing fires mid-session on any surface any more. `qa.complete` and `handover.complete` are folded into `exit_reflection.complete` above.

**Out of scope:** `SUBAGENT_INSTRUCTION` templates (`exit_reflection.context`) reach **neither** Nic nor the main agent — they are written to the temp file a _dispatched_ subagent reads. No user-visibility disposition applies.
