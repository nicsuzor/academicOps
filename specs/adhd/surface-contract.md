...
| :---------------------------------------------------------------- | :------------------------------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `exit_reflection` · FULL tier block (Stop, task-bound + did work) | `ephemeral`                     | `reason` on `decision:"block"`: "✕ Exit-reflection required before exit — self-audit, durable capture, commit/push/PR, honest handover." (`exit_reflection.policy_message`). Agent separately gets the full checklist via `context_injection` (`exit_reflection.policy_context`) — replaces the former separate `rbg-review`/`qa`/`handover` policy_message rows (aops_4c2949d9). |
| `exit_reflection` · FULL tier warn (Stop)                         | `silent` (intended; see caveat) | Non-blocking `additionalContext` delivery — same caveat as `ida` warn below.                                                                                                                                                                                                                                                                                                      |
| `exit_reflection` · LITE tier warn (Stop, everyone else)          | `silent` (intended; see caveat) | Non-blocking `additionalContext` delivery of the honesty-only reminder (`exit_reflection.lite_reminder`) — never `block`. Same caveat as `ida` warn below.                                                                                                                                                                                                                        |
