...
| C6  | A declared allowlist is not runtime ground truth: a spawned `ida` held a top-level tool set wider than `ida.md` declared                                                                                                                                 | —                                                                        |
| C7  | `tools`, `allowedTools` and `disallowedTools` are three distinct surfaces. `tools` is the capability grant; `allowedTools` is only the approval rule set in `Tool(pattern)` form. The field that restricts which subagents may be spawned is `subagents` | —                                                                        |
