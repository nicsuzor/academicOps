...
- **`/supervisor`** — the multi-tick delegate-and-verify loop that runs across
  a set of tasks.
...
1. **Contract** — the unit's premise, worth, and acceptance criteria are
   ...
   `brief` emits the contract as a standing, early-blocking node in the epic's
   DAG, so the rest of the epic's work depends on it clearing. `brief` plans
   only: it emits this task, and the boundary-check/QA-around tasks below,
   into the graph — it never dispatches or runs them itself.
...
- **High-risk / wide blast radius** — per-chunk review subtasks, wired with
  ...
  next chunk starts.
- **Low-risk / narrow blast radius** — workers self-assess and hand back
  separate commits under one shared branch; a single consolidated
  boundary-check + QA-around pass runs once, its receipt landing in the PKB.
  The final PR (the `cohesive-pr-epic` mechanism above) is the PR-surface
  instance of that pass, and the GHA PR pipeline is one optional executor of
  it.
...
steps 3–5 then review the shipped chunk plus its declared-deferred remainder,
and draft → ready-for-the-principal still requires the pauli-specified review.
