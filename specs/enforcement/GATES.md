...
- **`ida` is the exception.** The `ida` gate has no satisfaction predicate, so it is fire-once in **both** modes (`ida:block` = fire-once-**loud**, not persist) — the only gate in this class. Its warn-mode delivery uses a Claude-only **asyncRewake** quiet-split (full body → agent `<system-reminder>`, one-line → user), keyed on ida warn-mode. `ida` remains live in code — see [§ `ida` gate](#ida-gate); disposition is OPEN, pending [[aops_3eabb0ae]], not retired as this spec previously (and mistakenly) stated under H6.
