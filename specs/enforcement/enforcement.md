...
| `ida` gate (Stop honesty check + `AskUserQuestion` nudge) | **Retired as a hook** (H6) — deferred to the head-personality surface (`aops-interactive` — `ida`/`junior`, see [head-role-charter.md](../interactive-experience/head-role-charter.md)) interacting directly with the human, not a router-level gate firing uniformly across surfaces with no human present. In exchange, agents get explicit instruction on how to supply the completion proof `release_task` requires — that instruction content is pending the mem-server schema-floor work (B2/SEAM-2), out of this redraft's scope. |
