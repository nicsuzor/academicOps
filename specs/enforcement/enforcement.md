...
flowchart TD
  ...
  subgraph L1[L1 Context injection]
    HYD["hydrator, skills routing (aops/aops-adhd, H11), pkb-nudge (aops-core, H5/H14), CLAUDE.md/GEMINI.md safety-floor import, status strip"]
  end
...
| `ida` gate (Stop honesty check + `AskUserQuestion` nudge)                                                                                                   | Fire-once Stop honesty/criterion-substitution reminder plus a PreToolUse `AskUserQuestion` nudge. **Face-scoped, RATIFIED** (aops_5ea32596 / note_296e5520 §3 — supersedes the prior "disposition OPEN" note, see §6.2): fires on the head/face surface only (bare interactive CLI — `IDA_GATE_MODE` bare-fallback = `warn`), structurally absent from every dispatched polecat run/crew or sibling-crew-agent session (`gates.ida: off`, inherited from `session_defaults` — see `polecat/defaults/polecat.yaml.example`). No code-level session-type branching: the split is entirely the two `IDA_GATE_MODE` resolution paths — see GATES.md. | `honest-epistemics`                                                  | `gates.ida` (dispatched surfaces, off) / bare-CLI fallback in `hooks/gate_config.py` (head surface, warn) — the residual "WSL crew container as live orchestrator" edge case is still pending the session-type walk ([[aops_3eabb0ae]]) | Middle     | [`GATES.md#ida-gate`](GATES.md#ida-gate)                                                                                               |
...
| Skills-routing hint (`hydration.warn`)                                                                                                                      | UserPromptSubmit routing-table hint, main session only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `do-one-thing`                                                       | Ownership **moves up to aops/aops-adhd** (H11) — aops-core stops injecting this at the router level                                                                                                                                     | Base       | [`ENFORCEMENT-MAP.md`](../../specs/ENFORCEMENT-MAP.md)                                                                                 |
