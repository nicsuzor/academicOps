...
**Four levers, not severity alone.** "Encouragement with detection" means compliance is a risk-reduction game, not a guarantee — no mechanism forces an outcome, each only shifts the odds. Behavioral economics gives that shift four independent levers, and pyramid severity is only the last of them:
...
| :---------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :-------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :--------- | :------------------------------------------------------------------------------------------------------------------------------------- |
| `enforcer`/`rbg` gate                                                                                                                                       | Periodic PreToolUse compliance re-audit; dispatches the `rbg` subagent every N write ops.                                                                                                                                                                                 | All axioms                                                | `gates.enforcer_threshold` — lowered to **~17 default** (was 50), H2 (ruling left the exact number open — "drops to ~13; 17 maybe as default" — 17 is what shipped) | Middle     | [`GATES.md#enforcer-gate`](GATES.md#enforcer-gate)                                                                                     |
...
**Uniform application** (H8, scope-disciplined by H12): gates fire on **all** sessions and events — main, subagent, worker — with no `is_subagent` PreToolUse/PostToolUse skip. This is pursued as a reorganisation of the existing per-session `GateState` design, not a rewrite to full statelessness. See [`GATES.md` § Subagent & worker session scope](GATES.md#subagent--worker-session-scope).
...
> **Correction (2026-07-07, forensics on issues #2153/#2154, walk task [[aops_3eabb0ae]]).** The `ida` gate was previously listed in this table as retired from hook-level enforcement under ruling H6. That was a misrecording: Nic's actual H6 words (session c3d962f5, 2026-07-05 06:12 UTC, on [[aops-fef39347]]) were "ida can _probably_ be deferred to the head surface for interaction with the human; but we will need to tell agents how to provide the completion proof that release_task is going to require" — hedged and directional, never an approved retirement. Nic, live 2026-07-07: "I didn't approve retiring Ida." The gate REMAINS LIVE in code (`aops-core/lib/gates/definitions.py`, `GateConfig(name="ida", ...)`) and is now listed in §6.1 above with disposition OPEN, pending the session-type walk ([[aops_3eabb0ae]]).
...
| `is_subagent` PreToolUse/PostToolUse skip         | **Retired** (H8/H12) — see Uniform application above.                                                                                                                                                                                                                                                                                     |
