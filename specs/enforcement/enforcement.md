...
**Uniform application** (H8, scope-disciplined by H12; PreToolUse exception made permanent by aops_571771b4): gates fire on **all** sessions and events — main, subagent, worker — except PreToolUse, which stays skipped for subagent-classified sessions as a deliberate, permanent exception (blocking a subagent with no Agent-tool access, or via a policy meant for the main session, would deadlock rather than enforce — aops-55bcf1a2). The `is_subagent` PostToolUse skip is retired; the PreToolUse skip is retained. This is pursued as a reorganisation of the existing per-session `GateState` design, not a rewrite to full statelessness. See [`GATES.md` § Subagent & worker session scope](GATES.md#subagent--worker-session-scope).
