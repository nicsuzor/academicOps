...
The pyramid _learns_ by an evidence loop: incidents become anonymised reports become patterns become recommendations become rule changes. The loop is split deliberately by the `recusal` axiom into a witness phase and a judge phase — the agent that lived through the failure files facts only; a separate, detached agent later reads accumulated reports and proposes change.
...
| `check-no-fallbacks`     | L4 | warn   | Silent-fallback patterns across ALL first-party python/shell — `aops-core/`, `polecat/`, `scripts/`, `aops-tools/` (`halt-on-failure` / P#8; #930, scope-widened #1585). Pre-existing sites grandfathered in `check_no_fallbacks_baseline.json` pending sweep `aops-682e75a5`; new ones still fail | `scripts/check_no_fallbacks.py`     |
...
Branch protection requires each agent's `<agent>-status` check directly — there is no transitive triage agent deciding mergeability on the others' behalf. The judgment lives in the LLM review agents themselves: a failing `enforcer-status` (the LLM axiom review of the diff) blocks the merge. Branch protection is just the mechanical AND-gate that combines the required checks; "no triage LLM" is the point, not "the merge is unreviewed." For the canonical architectural details of this design and its phased rollout, see [`pr-pipeline.md`](../workflows/pr-pipeline.md).

**Live vs planned (verified against ruleset `13762049` 2026-06-09, then refreshed at Phase 5).** Two GHA agent gates are deployed and required: `enforcer-status` (enforcer-v1, the LLM axiom review) and `qa-status` (marsha runtime verification — `agent-qa.yml` + `trigger-qa.yml` exist; the manual `/verify` path remains for the dev session). The merge gate also requires `admit-status` — the Environment-gated single human approval, parked at the `pr-fix-loop` Environment (required reviewer `nicsuzor`) — plus the mechanical checks (Lint, Pytest). `required_approving_review_count` is **0**; `merge-prep-status` has been **removed** from the gate (replaced by `admit-status`). `alignment-status` is **advisory by design and intentionally NOT in the ruleset** — the in-pipeline queue surface (`pr-pipeline.yml`'s `alignment-queue` job: posts `alignment-status: pending`, files an `alignment:queued` issue per PR) is LIVE; the host-side cron + polecat-pauli dispatcher that drains the queue and posts the terminal verdict is SPEC-ONLY (pauli runs host-side because the PKB MCP is Tailnet-internal — GHA cannot reach it; see [`pr-pipeline.md`](../workflows/pr-pipeline.md) §6). The maintainer reads pauli's verdict at the human Env gate; alignment never blocks merge. The post-admission **dev/mechanic split** is **LIVE as of Phase 5**: `agent-mechanic.yml` + `.github/agents/mechanic.agent.md` exist; the orchestrator (`pr-pipeline.yml`) appends `mechanic` after `qa` gated on `admit-status=success`; the v1 `merge-prep` agent (`agent-merge-prep.yml` + `merge-prep-cron.yml` + `merge-prep.agent.md`) is **deleted**. `mechanic-status` is informational and is never in the required-checks list — the mechanic does work, not a verdict; enforcer + qa re-verify each mechanic SHA ([`pr-pipeline.md`](../workflows/pr-pipeline.md) §3.5), and the §3.6 bound (`MAX_MECHANIC_RUNS=5`) + exhaustion handler (reset `admit-status` + escalation review + maintainer request) live in the mechanic workflow. The canonical contract is [`pr-pipeline.md`](../workflows/pr-pipeline.md); the table below marks current reality in its Status column.
...
| `alignment-status` | L6 | advise | pauli review of PKB design-intent (NOT axioms)                               | partially live — queue surface LIVE (`pr-pipeline.yml` `alignment-queue` job); host-side cron + polecat-pauli dispatcher SPEC-ONLY. **Advisory by design — intentionally NOT a required check.** | `.github/workflows/pr-pipeline.yml` (queue); host-side cron + dispatcher (out of repo) — no `agent-alignment.yml` |
| `qa-status`        | L6 | block  | marsha runtime/intent verification                                           | **live** (required) — auto on PR events; manual `/qa` or `/verify` route                                                                                                                         | `.github/workflows/agent-qa.yml` + `trigger-qa.yml`; `.github/agents/qa.agent.md`                                 |
| `admit-status`     | L7 | block  | Environment-gated single human approval (Stage-2 gate)                       | **live** (required) — parks at `pr-fix-loop` Environment, reviewer `nicsuzor`                                                                                                                    | `.github/workflows/stage2-admission.yml`; `pr-fix-loop` Environment                                               |
| `mechanic-status`  | L4 | —      | Post-admission dev work + conflict-resolve only (no approval, no auto-merge) | **live** (Phase 5) — informational, never required; `MAX_MECHANIC_RUNS=5` bound                                                                                                                  | `.github/workflows/agent-mechanic.yml`; `.github/agents/mechanic.agent.md`                                        |
| branch protection  | L7 | block  | AND-gates all required `<agent>-status` checks at merge                      | **live** (ruleset `13762049`, `enforcement: active`)                                                                                                                                             | GitHub repo settings; `.github/rulesets/pr-review-and-merge.yml`                                                  |
| `loop_detector`    | L7 | block  | Refuses merge if Stage-2 mechanic loop ceiling reached                       | **live** (in `agent-mechanic.yml`, §3.6)                                                                                                                                                         | `.github/workflows/agent-mechanic.yml` (steps: SHA-skip, Loop-ceiling check, Exhaustion handler)                  |
