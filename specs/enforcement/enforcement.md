...
flowchart TD
  ...
  subgraph L3[L3 Workflow composition]
    WF["/planner decomposition, cohesive-pr-epic, /supervisor — see workflow.md"]
  end
  subgraph L4[L4 Soft gates]
    GATES["task-binding gate (reactivated, H4), unified logger, premise gate (spend-surface refusal)"]
  end
  ...
  subgraph L8[L8 Handover]
    HAND["/dump, framework reflection, exit_reflection gate (FULL tier), commit gate, pre-commit mechanical checks, release_task evidence contract (H7)"]
  end
...
### §4.2b Worked example: `polecat.yaml` config-load hard-fail, universal (note_296e5520 §4)

User-directed architectural change (Nic, ratified 2026-07-11, note_296e5520 §4) — skips the ≥3-recurrence requirement per §4.1's carve-out, but still requires the pyramid-position reasoning below.

- **What changed**: `hooks/gate_config.py`'s `__getattr__` previously printed `WARNING: Gate mode '<name>' not set...` and silently returned a hardcoded default when a `*_GATE_MODE` env var was absent — the ONLY path a bare/direct-CLI session (no polecat launcher) ever took. This is now a two-step resolution — env var, then `polecat.yaml`'s `face` section, resolved directly — with a hard `RuntimeError`/DENY if neither resolves. `polecat.yaml` itself moved from `session_defaults` + `crew_defaults`/`run_defaults` overlay to four explicit, independently-required, non-overlaid sections (`face`/`crew`/`worker`/`subagent`).
- **Cheapest plausible position**: L0 (documentation only) was already tried and had already failed — `SURFACES.md`'s "Hook env stripping" section pre-dates this change and explicitly flagged "gates silently fail on import if `$AOPS_SESSIONS` isn't in hook env" as a KNOWN, documented gap. Writing it down did not stop it from recurring; the gap was structural (a code-level fallback), not a knowledge gap.
- **Why escalate to L1 (structural, config-loader exception)**: the failure mode is silent — a misconfigured or unreachable `polecat.yaml` produced a session that LOOKED normal (gates apparently active, all "warn") but was actually running on a hardcoded default nobody chose, on the exact surfaces (direct/interactive CLI) least likely to be scrutinised. A judgment gate (L4+) cannot fix this — there is nothing to judge; the config either resolves or it doesn't. The only correct fix is removing the silent-continue branch entirely, per `halt-on-failure` (A14) and `single-source-of-truth`.
- **Ongoing cost**: near-zero per-invocation cost (one extra YAML parse on the cold path, memoised per process). The real cost is operator friction: a genuinely misconfigured machine now hard-fails at `SessionStart` instead of limping along ungated — this is the intended trade (loud beats silent), not a side effect to be minimised.
- **Reversibility**: if this proves too strict in practice (e.g. legitimate ad hoc sessions with no `polecat.yaml` at all become common and the hard-fail is pure friction with no corresponding safety win), the documented contingency is to reintroduce a DELIBERATE, DOCUMENTED default for the `face` surface specifically — not a silent code fallback, but an explicit "if `polecat.yaml` is absent AND `$AOPS_SESSIONS` is unset, treat this as a genuinely ad hoc session and use a named, visible posture" branch, reviewed and CBA'd on its own terms.
...
**Standing rule, all gates (H3).** Gate posture — armed/disarmed, on/off, which mode a surface runs in — is expressed **only** via environment variables and `polecat.yaml`. No gate's mode may be selected by on/off, session-type, or other state code in the repo; `exit_reflection` (below) is the sharpest example but the constraint binds every gate in this index, not just that one. Per-gate mode-key detail: [`GATES.md`](GATES.md#config-plumbing).

| Mechanism                                                                                                                                                   | What it is                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Enforces                                                             | Configured via                                                                                                                                                                                                                          | Tier       | Detail                                                                                                                                 |
| :---------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------- | :------------------------------------------------------------------------------------------------------------------------------------- |
| `exit_reflection` gate (aops_4c2949d9)                                                                                                                      | Consolidated Stop gate replacing `rbg-review`+`qa`+`handover`. FULL tier (task-bound, did work this turn): axiom self-audit, durable capture, commit/push/PR, evidenced handover — blocks until a legal exit (auditor ran, honest `release_task`, or `/end-session`/`/dump`/`/continue`). LITE tier (everyone else): non-blocking honesty reminder only. The turn-based `rbg` PreToolUse compliance-counter gate is retired entirely — nothing fires mid-session on any surface any more.                                                                                                                                                        | All axioms, `exercise-authority`, show-don't-tell, `halt-on-failure` | `gates.exit_reflection` / `EXIT_REFLECTION_GATE_MODE` — posture is **env-var/`polecat.yaml` only, no on/off/session-type code** (H3)                                                                                                    | Middle–Tip | [`GATES.md#exit_reflection-gate`](GATES.md#exit_reflection-gate)                                                                       |
...
| `ida` gate (Stop honesty check + `AskUserQuestion` nudge)                                                                                                   | Fire-once Stop honesty/criterion-substitution reminder plus a PreToolUse `AskUserQuestion` nudge. **Face-scoped, RATIFIED** (aops_5ea32596 / note_296e5520 §3 — supersedes the prior "disposition OPEN" note, see §6.2): fires on the head/face surface only (bare interactive CLI — `polecat.yaml`'s `face.gates.ida` = `warn`), structurally absent from every dispatched surface (`crew`/`worker`/`subagent` each set `gates.ida: off` explicitly — see `polecat/defaults/polecat.yaml.example`). No code-level session-type branching: the split is entirely a per-surface config value under DEFAULTS-NONE — see GATES.md. | `honest-epistemics`                                       | `gates.ida` — `off` on `crew`/`worker`/`subagent`, `warn` on `face` (resolved directly by `hooks/gate_config.py` when no launcher has staged the env var) — the residual "WSL crew container as live orchestrator" edge case is still pending the session-type walk ([[aops_3eabb0ae]]) | Middle     | [`GATES.md#ida-gate`](GATES.md#ida-gate)                                                                                               |
...
| Evidence contract (`release_task`)                                                                                                                          | The completion claim must carry independent-verification evidence, or a stated failure reason. **Primary enforcement point** (H7); agents "land the plane" — commit → push → `release_task`, or the work is garbage-collected (H10). Universal field-shape, substance-over-form rule, and grandfather cutover policy are canonical in [evidence-contract.md](evidence-contract.md).                                                                                                                                                                                                                                                              | `honest-epistemics`                                                  | `mem` MCP server schema floor (coordinates with B2/SEAM-2)                                                                                                                                                                              | Middle     | [task-contract.md](task-contract.md), [evidence-contract.md](evidence-contract.md)                                                     |
...
> **Note.** The `ida` gate REMAINS LIVE in code (`aops-core/lib/gates/definitions.py`, `GateConfig(name="ida", ...)`) and is listed in §6.1 above. Its scoping disposition is now RATIFIED (aops_5ea32596 / note_296e5520 §3, face-scoped: fires only on the head/face surface) — superseding the earlier "disposition OPEN, pending the session-type walk" note. One narrow edge case (a live orchestrator session running as a long-lived "WSL crew container" rather than a dispatched `polecat crew` sibling-agent session) is the only piece still deferred to the session-type walk ([[aops_3eabb0ae]]); see §6.1's row and [`GATES.md#ida-gate`](GATES.md#ida-gate) for the current mechanism.
...
| `rbg` gate (turn-based PreToolUse compliance counter) | **DELETED** (aops_4c2949d9) — not disabled/versioned, deleted: `GateConfig`, `gates.rbg_threshold`/`RBG_GATE_MODE`/`RBG_TOOL_CALL_THRESHOLD`, the compliance-report custom action, and every `rbg-{audit,context,countdown,instruction,policy-context,policy-message,verified}.md` template. Nothing fires mid-session (PreToolUse) on any surface any more; the periodic-check concern is not replaced — the module-boundary thesis (see Purpose above) and the `exit_reflection` gate's end-of-session FULL tier are judged sufficient. |
| `rbg-review` gate                                     | **Folded into `exit_reflection`** (aops_4c2949d9) — the end-of-session axiom-audit concern survives as the FULL tier's RBG-lens self-audit step, not as a separate gate.                                                                                                                                                                                                                                                                                                                                                                  |
| `qa` gate                                             | **Folded into `exit_reflection`** (aops_4c2949d9) — the completion-quality concern survives as a FULL-tier checklist step.                                                                                                                                                                                                                                                                                                                                                                                                                |
| `handover` gate                                       | **Folded into `exit_reflection`** (aops_4c2949d9) — the exit-discipline concern survives as the FULL tier's durable-capture / commit-push-PR / evidenced-handover steps.                                                                                                                                                                                                                                                                                                                                                                  |
...
This layer is formalised in [`workflow.md`](workflow.md): the recursive
five-step shape (contract → execution → boundary check → QA-around →
principal sign-off), with review depth at steps 3–4 chosen by the planner at
decomposition time based on risk. It is named here as pipeline L3 so the
pyramid/map/mechanism specs have a consistent reference point; `workflow.md`
is the canonical entry, this section is a pointer.
