...
| `admit-status`     | L7 | block  | Environment-gated single human approval (Stage-2 gate)                       | **live** (required) — parks at `pr-fix-loop` Environment, reviewer `nicsuzor`                                                                                                                    | `.github/workflows/pr-pipeline.yml` (in-pipeline `admit` job); `pr-fix-loop` Environment                          |
