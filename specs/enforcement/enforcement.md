...
- **`specs/enforcement/GATES.md`** — forensic reference for the runtime gate catalogue. Gates are design features with their own dev-defined operating logic (code, not state); the map (above) holds each gate's _configuration_ — its mode (block/warn/off). GATES.md documents where each gate lives in source, how it's configured, how to verify firing, and how to debug it. When to reach for it: a forensic-debug question about a specific gate.

## Kinds of enforcement mechanism

The framework has ~40 distinct enforcement mechanisms, of six kinds:

| Kind                      | What it is                                                                                                      | Examples                                                                                                                 | Catalogued in                                                            |
| ------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| **Instructions**          | Prose an agent reads and applies with judgment; no code checks compliance.                                      | `AXIOMS.md`, `HEURISTICS.md`, `RULES.md`, agent/skill files, `CLAUDE.md`                                                 | `.agents/rules/`, `aops-core/agents/*.md`, `aops-core/skills/*/SKILL.md` |
| **Hooks / gates**         | Deterministic runtime code that intercepts a lifecycle event (PreToolUse, Stop, …) and returns allow/warn/deny. | `sentinel`, `enforcer`, `qa`, `handover`, `ida`, `rbg-review`                                                            | [`GATES.md`](GATES.md)                                                   |
| **LLM classifiers**       | Per-action model judgment, pre-execution, allow/deny only — no "ask," no state.                                 | Claude Code auto-mode classifier                                                                                         | [`auto-mode-classifier.md`](auto-mode-classifier.md)                     |
| **Pre-commit scripts**    | Deterministic local checks that block a commit until fixed.                                                     | ruff, markdownlint, actionlint, `check_no_fallbacks`                                                                     | `.pre-commit-config.yaml`                                                |
| **CI / platform checks**  | Deterministic checks and native platform rules enforced on push, PR, or merge.                                  | `lint.yml`, `pytest.yml`, `typecheck.yml`, branch protection                                                             | `.github/workflows/`                                                     |
| **Post-hoc agent review** | An LLM subagent (or, at the tip, a human) reads a diff or transcript after the fact and returns a verdict.      | `rbg`, `marsha`, `james` (in-session); `agent-enforcer.yml`, `agent-qa.yml`, `pr-reviewer` (GHA); project-owner approval | [`ENFORCEMENT-MAP.md`](../ENFORCEMENT-MAP.md), `.github/agents/*.md`     |

Kind is orthogonal to _when_ a mechanism fires (the pipeline, below) and _how invasive_ it is (the pyramid, below) — the same kind appears at every tier, and the same tier holds every kind.

## Two views of the same mechanisms

Two organising principles are useful for thinking about mechanisms beyond kind.
...
flowchart TD
  ...
  subgraph L4[L4 Soft gates]
    GATES["hydration gate, enforcer gate, QA gate (planned), unified logger, premise gate (spend-surface refusal)"]
  end
...
A malformed axiom corrupts the legislative layer the rest of this pyramid depends on, so the legislative layer's form discipline is part of this design statement, not a separate style guide. These principles govern how [`.agents/rules/AXIOMS.md`](../../.agents/rules/AXIOMS.md) and its paired review checklist [`.agents/rules/AXIOMS-REVIEW.md`](../../.agents/rules/AXIOMS-REVIEW.md) MUST be written. They are spec rules, not style preferences — an axiom that breaks them is malformed and `rbg` should flag it.
...
### Tier placement: Base / Middle / Tip
...
| ---------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Base**   | High-volume, low-invasiveness, non-blocking, runs constantly           | lightweight hydrator (L1), skills routing table (L1), CLAUDE.md / AGENTS.md load (L1), gate status strip (L1), session_env_setup (L1), unified logger (L6), task-file append (L6), session logs (L6), task template conventions (L2), pre-commit scripts (dprint, ruff, markdownlint, actionlint, `check_no_fallbacks`, etc. — see `.pre-commit-config.yaml`)                                                                                                                                                                                                                                                                                                                                               |
...
**Default to instructions.** Agents are intelligent and instructions work in the large majority of cases; the burden of proof is on adding a mechanism, not on keeping behaviour in prose. Every new hook or gate is permanent complexity and a new place for the framework to fail. Prefer the lightest sufficient instruction; prefer making an existing instruction land (relocate, propagate, strengthen) over creating a new mechanism.

The instruction tier is not one setting — it has two independent sub-features, and escalation must exhaust both before ever leaving the tier:

1. **Tone** — the instruction itself gradates from a gentle suggestion, to a stated requirement, to a hard rule in prose.
2. **Placement** — _where and when_ it's shown: which delivery channel carries it (L1 SessionStart read, L2 lifecycle injection, L3 voluntary skill), and how insistently within that channel (urgency, visibility, salience, relocation to the failing surface, propagation to every surface that hits the failure, structured/parsed format) — see [`enforcement-design.md`](../../aops-core/skills/aops/references/enforcement-design.md) ("Within-class Insistence & Placement Spectrum").

"Escalating a failure" at the instruction tier means **first walking both sub-features** — sharpening tone and improving placement — within the current mechanism class; crossing into a heavier mechanism class is the move of last resort, not the default.

**Escalation rules.**

- **Escalate up** only when the evidence loop (§5) shows the instruction tier is being bypassed with reproducible consequences — and only after confirming both sub-features were exhausted: tone was a hard rule, and placement was clear, salient, correctly placed, and propagated to every surface that hit the failure. A quiet, softly-toned, mislocated, or unpropagated instruction is **not** evidence the tier is exhausted; it's evidence one of the two sub-features was never walked.
- **Bias hard against new hard gates.** Each hard gate (L5+/tip) is permanent maintenance and a new failure surface. Add/escalate proposals into the tip need §4.1 CBA evidence plus explicit demonstration that both instruction sub-features — and, within placement, rungs 5 (relocation), 6 (propagation), and 7 (structured) specifically — were tried before the L4 chokepoint especially (see above).
...
Any proposal to **add, escalate, or remove** a mechanism (a new gate, a pyramid-position change, a new axiom, an additional hook firing surface) must answer:

1. **Friction evidence**: ≥3 concrete recurrences with links (transcript, PR, issue, /retro report). Fewer than 3 → premature, unless the user has explicitly directed the change.
2. **Cheapest plausible position**: which row of the pyramid could reasonably address this?
3. **Why escalate above that position?**: what was tried at the cheaper position, and specifically why it failed.
4. **Ongoing cost**: token cost per fire × fire frequency, or latency estimate — **and** the failure-mode cost (e.g. classifier theatre / death-by-denial; funnel coordination-tax + relocated assurance). A mechanism cheap per fire but corrosive when it misfires is not cheap.
5. **Reversibility**: if this doesn't reduce recurrences in the next 5 /retro reviews, how do we retire it?

A bug fix within an existing surface at the same position (correcting wrong logic or prose, no position change) needs only a clear description of the bug and the corrective edit — items 1–3 don't apply. A user-directed architectural change skips item 1's recurrence bar but still needs items 2–3's pyramid-position reasoning.

**Where this is enforced** — which PRs must carry this analysis in the PR body, and what a reviewer warns or blocks on — is contributor process, not design; see `CONTRIBUTING.md` and the `pr-reviewer` agent.
...
Periodically — when the issue queue feels heavy, or on a cadence the user sets — the user runs `/issue-sweep`. The dispatched agent enters with no prior exposure to any individual incident, classifies each open issue (close-stale, comment-only, single-task, fix-epic, defer), and for any issue whose remediation would touch the framework applies the §4 escalation rules and Cost-Benefit Analysis (§4.1) — cheapest sufficient position, evidence-gated escalation — citing the specific register row the fix propagates from or would add. It surfaces the proposed cycle to the user (gated per disposition group) for approval, edit, or deferral, then files the approved fix-epics or single tasks, stamped and logged; fix-epics stay `queued` until dispatched via `/supervisor`.

Consistent with §4.1: the sweep agent will not propose **adding or escalating** a rule from one incident — that needs ≥3 cited recurrences plus the CBA. This bar does NOT apply to **fixes** within an existing surface at the same position (a misrouting skill, a broken hook, an incomplete gate) — one clear forensic incident is sufficient for `fix-epic` or `single-task`. Nor does it apply to **directed architectural changes** the user has explicitly authorised: the user's authorisation substitutes for the recurrence count, not for the pyramid reasoning.
...
4. **Least invasion first** — exhaust the instruction tier's tone and placement sub-features (§4 Escalation rules) before crossing mechanism classes: conventions before templates; templates before soft gates; soft gates before hard blocks; hard blocks before human approval. Escalate only on §5 evidence that both sub-features were walked, not just that the rule was once stated and once ignored.
