...
**Live vs planned (verified against ruleset `13762049` 2026-06-09).** Two GHA agent gates are deployed and required: `enforcer-status` (enforcer-v1, the LLM axiom review) and `qa-status` (marsha runtime verification — `agent-qa.yml` + `trigger-qa.yml` exist; the manual `/verify` path remains for the dev session). The merge gate also requires `admit-status` — the Environment-gated single human approval, parked at the `pr-fix-loop` Environment (required reviewer `nicsuzor`) — plus the mechanical checks (Lint, Pytest). `required_approving_review_count` is **0**; `merge-prep-status` has been **removed** from the gate (replaced by `admit-status`). Still not required: `alignment-status` is **in progress** — pauli runs **off-GHA**, host-invoked via the `strategic-review` skill (it needs PKB reachability, which GHA lacks), posts a verdict, and is intended to become a required check but is **not yet** in the ruleset. The post-admission **dev/mechanic split** is **not built** (`agent-mechanic.yml` / `mechanic.agent.md` absent); its name is **settled as "mechanic"** (`mechanic-status`, non-required) per the operative spec. Until Phase 5 ships, the live `merge-prep` (`agent-merge-prep.yml`) remains the **operative** post-admission mechanism: it **auto-tries to make the PR green** (auto-fixes failing checks, re-runs until they pass) **and addresses all prior review comments**, plus conflict-resolve (merge, **not** rebase — force-push prohibited), bot-approve, enable auto-merge, with self-loop detection + ceiling (`MAX_MERGE_PREP_RUNS=5`); its `merge-prep-status` is now non-required. The canonical contract is [`pr-pipeline.md`](../workflows/pr-pipeline.md); the table below marks current reality in its Status column.
...
| `alignment-status` | L6 | block  | pauli review of PKB design-intent (NOT axioms)          | in progress — host-invoked via `strategic-review`; not yet a required check        | host / user machine via the `strategic-review` skill (off-GHA — pauli needs PKB reachability); no `agent-alignment.yml` |
