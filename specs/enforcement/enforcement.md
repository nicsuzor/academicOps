...
**Live vs planned.** `enforcer-status` (enforcer-v1), `qa-status` (marsha via `agent-qa.yml`), and `admit-status` (Stage-2 Admission Gate via `stage2-admission.yml`) are the three deployed GHA agent gates required by the ruleset (Phase 4). `alignment-status` is **in progress** — pauli runs **off-GHA**, host-invoked via the `strategic-review` skill (it needs PKB reachability, which GHA lacks), posts a verdict, and is intended to become a required check but is **not yet** in the ruleset; the planned rebase-only split is **not built** (`agent-mechanic.yml` absent); its name is settled as **"mechanic"** per the v2 operative spec (see [`pr-pipeline-v2.md`](../workflows/pr-pipeline-v2.md) §11). `merge-prep-status` is **removed** (Phase 4) — replaced by `admit-status`. `required_approving_review_count` is **0** — the human gate is the `pr-fix-loop` GitHub Environment approval that sets `admit-status`, not a PR review approval. The table below is the v2 **target** contract; the Status column marks current reality.
...
| rebase-only split  | L4 | —      | Mechanical merge + conflict resolution only             | planned; not built — name settled as "mechanic" per [`pr-pipeline-v2.md`](../workflows/pr-pipeline-v2.md) §11 | `.github/workflows/agent-mechanic.yml` absent; live merge-prep still does this                                          |
| `qa-status`        | L6 | block  | marsha runtime/intent verification                      | **live** — `agent-qa.yml` runs automatically; required check in ruleset     | `.github/workflows/agent-qa.yml`                                                                                        |
