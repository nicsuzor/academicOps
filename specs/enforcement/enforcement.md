...
| `deliverable-verify` gate                                                                                                                                   | SubagentStop advisory: reminds the orchestrator to verify a subagent's deliverable/verdict before relaying it (substituted primitive, reframed failure, intent-vs-effect confusion). Added task-1029fccb — §4.1 add/escalate, 3 cited recurrences (#1028, #430, #1836). Does NOT touch the PreToolUse subagent exemption (unchanged, permanent — see [`GATES.md` § Subagent & worker session scope](GATES.md#subagent--worker-session-scope)). Excludes rbg/marsha's own SubagentStop (`not_compliance_subagent`) — auditing the auditor is recursion, not narrowing. | `honest-epistemics`                                       | `gates.deliverable_verify` — required key, no default (matches this map's no-silent-fallback convention for the `gates:` block)                                     | Middle     | [`GATES.md#deliverable-verify-gate`](GATES.md#deliverable-verify-gate)                                                                 |
