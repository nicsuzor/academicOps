...
- **Reopen on caught delivery loss** — detection and repair are separate duties with separate owners, and the guard above is only the detection half. A worker that wrote a terminal status to the graph and delivered nothing leaves that status behind; nothing downstream can tell it apart from real completion, and neither filing a fix subtask nor re-dispatching undoes it. The repair is owned by the dispatcher, which reopens the task through pauli on any non-zero container exit for a terminal-status unit. It sits there rather than in the launcher because writing to the knowledge base belongs to its sole writer, and a launcher carrying its own client for another plugin's tool namespace is a second copy of that plugin's job. **The obligation is currently stated nowhere the dispatcher reads it**: the `dispatch` skill that carried it is retired, [`plugins/orchestrate/agents/james.md`](../../plugins/orchestrate/agents/james.md) does not restate it, and the only shipped statement is the failure text `polecat run` prints on its way to a non-zero exit ([`lib/polecat/cli.py`](../../lib/polecat/cli.py)) — which reaches the dispatcher only if it reads the output it was handed.
...
A container emits no completion signal, so evidence reaches this boundary only because something carries it. The task record the worker writes before exiting is the only carrier, and there is deliberately no second one: james's responsibility ends at dispatch — he does not track or poll an asynchronous worker ([`plugins/orchestrate/agents/james.md`](../../plugins/orchestrate/agents/james.md)), and nothing in `orchestrate` waits for one. The consequence is that a worker which writes nothing to the graph reaches this boundary as silence rather than as a failure, which is what the sweep above exists to catch and what its absence currently leaves uncaught.
