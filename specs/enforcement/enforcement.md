...
**The claim is written twice, deliberately.** The dispatch surface records the claim on the task at launch — who it went to, under which session and surface, and when — for any dispatch whose loss would matter beyond its own session; the worker then claims the task itself from inside its session, which is what moves the status. Cheap read-only probes are exempt. The launch-time record closes the window in which a worker dies before ever claiming and the unit reads as work nobody picked up. Full invariant: [task-contract.md](task-contract.md).

**A boundary that agents sometimes never cross back through needs a sweep behind it.** Claims outlive dead sessions and merges land unwatched, so the reconcile skill ([`plugins/pkb/skills/reconcile/SKILL.md`](../../plugins/pkb/skills/reconcile/SKILL.md)) reads the graph against the evidence trail: it probes suspect claims and requeues abandoned ones to `ready` — never closing one silently — folds in what finished while nobody was watching, and routes completed-but-uncertified units to the dispatch surface for certification. It runs on engagement after an absence (commissioned by ida as a delegation, since she holds no PKB tools of her own), inside the consolidation cycle, and on demand, and it renders one synthesized result rather than a per-task feed. Behavioural spec: [reconcile.md](../workflows/reconcile.md).
...
**Certification at completion executes those nodes.** When a unit's return contract lands, the dispatch surface commissions the review the graph already carries — through the [`strategic-review`](../../plugins/aops/skills/strategic-review/SKILL.md) and [`verify`](../../plugins/aops/skills/verify/SKILL.md) skills — and writes the verdict back onto the task record; those skills each write their own verdict there too whenever the reviewed artifact has a task record, per [evidence-contract.md](evidence-contract.md). One path, never a second review standing beside the nodes decomposition already emitted. The dispatcher supplies no judgment of its own and never relays a worker's "confirmed" as a verdict. The acceptance that follows it belongs to the face, judged against user intent — the two-signature rule in [workflow.md](workflow.md).
