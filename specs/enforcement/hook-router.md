...
3. **Consolidated Feedback**: When multiple gates trigger simultaneously (e.g., a "hydration required" block and a "enforcer countdown" warning), users expect a single, well-formatted response from the router. Feedback should be prioritized (blocking reasons first) and deduplicated.
...
5. **Contextual Guidance**: When an action is denied or a threshold is approaching, the router must inject specific, actionable instructions into the agent's context (e.g., "Run /hydrate to unblock tool use") rather than generic error messages.
...
7. **Performance**: Hook routing must not noticeably degrade interactive responsiveness. Latency is a signal to monitor and calibrate against observed UX impact, not a fixed threshold to enforce mechanically.
