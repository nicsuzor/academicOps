...
1. The `autoMode` key is restored to the canonical source as the SSoT — **with no behavioural rules seeded this cycle** (per the decision above). When a rule is later seeded it follows the form above (principle + reasoning + cue + carve-outs + safer path + axiom slug).
