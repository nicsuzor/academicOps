...
> `lib/polecat/defaults/claude-settings.json`; this spec tracks the mechanism's
...
**Current state: the rule set is seeded and populated.** `autoMode` rules live in `lib/polecat/defaults/claude-settings.json` (`environment`/`allow`/`soft_deny`); no rule has yet been escalated to `hard_deny`. Individual rules are maintained in that file, not duplicated here — this spec tracks the mechanism, not each entry. New rules still follow the evidence loop above before landing.
