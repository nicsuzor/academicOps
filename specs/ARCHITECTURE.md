...
| `ida`  | `PostToolUse` (`Agent`) | Claude Code              | none                                                                  | Non-blocking reminder (`warn`) that a subagent's report is second-hand and inadmissible until its claims are shown.                                                                                                                         | Binds the dispatcher at the instant a report lands. `SubagentStop` injects only into the stopping subagent, never its caller, so the caller's own tool event is the only channel that reaches the party who must decide whether to trust it. |
