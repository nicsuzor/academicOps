...
both the face and a stopping worker) directs the agent to run an explicit rule
check and present evidence before it can stop. `ida`'s quiet gate is advisory
only. Each is silent on what it finds.
...
gate on this primitive. `aops`'s task-release check, a second gate on the same
primitive, is specified and also still unbuilt.
