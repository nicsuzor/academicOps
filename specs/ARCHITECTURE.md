...
4. **Dual-layer rule enforcement (`rbg`)** — turn-by-turn local-model evaluation
   of tool calls (`PreToolUse`), plus a stop gate that blocks once per
   stop-chain and directs the agent to run the RBG rule-compliance check
   (`axioms/` + project + local rules) before stopping (`Stop` /
   `SubagentStop`).
...
three jobs and no others: plan, by commissioning `aops:pauli`; launch polecats,
through `pc`; and track what is in flight. She holds between steps and filters
what comes back so the user sees only what needs their judgment. She reaches
exactly two agents, `aops:pauli` and `orchestrate:pc`, and nothing else.
...
The plugin advises on the same three sources automatically and in-session, on a
parallel turn-by-turn `PreToolUse` hook running a lightweight local Reflexes LLM
evaluator model. It returns `warn`, so a flagged call still proceeds.

The stop-side rule gate is registered in `plugins/rbg/hooks/handlers.py`
alongside `PreToolUse` and `UserPromptSubmit`. On `Stop` and `SubagentStop` it
withholds the stop once per stop-chain (`block`, `lib/hooks/dispatch.py`),
directing the agent to run the RBG rule-compliance check over the three sources
above and present checkable evidence before it stops.
...
| :------------ | :-------------------------------------- | :-------------------- | :------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `aops`        | `UserPromptSubmit`                      | Both                  | `PKB_MCP_URL`                                                                                                              | **Pillar 1.** Injects strategic context-search instructions and relevant PKB history, grounding every prompt in prior decisions before acting.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `orchestrate` | `Stop`                                  | Both                  | as above                                                                                                                   | **Observability.** Nothing injected. `stop` completes the trace at the turn boundary and clears the session's tracer state. Declared `async` on Claude Code.                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `rbg`         | `PreToolUse`                            | Claude Code           | `COPE_EVALUATOR_*` (local LLM model)                                                                                       | **Pillar 4, layer 1.** Parallel, non-blocking rule-compliance advisory carrying the matched rule text and reasoning.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `rbg`         | `UserPromptSubmit`                      | AGY (`PreInvocation`) | Live rule set files                                                                                                        | Summary roster of active rules for the turn, giving rule visibility on surfaces that lack tool-call interception.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `rbg`         | `Stop` / `SubagentStop`                 | Both                  | `stop_hook_active` / `background_tasks` checks                                                                             | **Pillar 4, layer 2.** Blocks once per stop-chain (`decision: "block"`), directing the agent to invoke the RBG rule checker (`axioms` + project + local rules) and present checkable evidence before stopping. Silent on the continuation stop and while background work runs. Lets the stop through, reporting on stderr, if its message file is missing or empty. Advisory-only on AGY. The chain allows one block, so it is not spent on a turn that is not the handback, nor on a block carrying no instruction.                                                             |
...
| `SubagentStop` (polecat container) | Stopping subagent worker (isolated container) | When an isolated worker finishes and attempts to stop             | `additionalContext` injected into worker context before handback. Advisory.               |
...
substance of what an agent did. One mechanism holds a stop open rather than
...
present evidence before it can stop. `ida`'s quiet gate is advisory only. Each is
...
and `is_continuation`, the once-per-stop-chain guard any such gate needs. One
plugin wires it (`rbg`); a second gate on the same primitive, `aops`'s
task-release check, is specified and still unbuilt.
