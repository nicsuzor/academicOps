...
The plugin does not gate session stop. `plugins/pkb/hooks/handlers.py` registers
`UserPromptSubmit` and nothing else, so a session can end while it still holds an
`in_progress` task. Holding the stop until the task is recorded and released is a
wanted gate, not a shipped one; what enforces the claim → release boundary today
is the task contract itself (specs/enforcement/task-contract.md) and the
reconcile sweep behind it.
...
| `ida`  | `PostToolUse` (`Agent`)                 | Claude Code              | none                                                                  | Non-blocking reminder (`warn`) that a subagent's report is second-hand and inadmissible until its claims are shown.                                                                                                                                                                                              | Binds the dispatcher at the instant a synchronous report lands. `SubagentStop` injects only into the worker, never its caller. `Notification` is not registered, here or anywhere — an async task notification therefore delivers its report with no hearsay reminder attached, which is a gap, not a design.                                                                         |
...
| `aops` | `SessionStart` — **not built**          | —                        | `CLAUDE_CODE_ENABLE_TELEMETRY`, `CLAUDE_CODE_ENHANCED_TELEMETRY_BETA` | 3-line session environment summary & credential isolation status. Only these two enablement vars are observable here — the `OTEL_*` export-config vars are set in a separate, settings-managed environment this hook subprocess cannot see.                                                                      | Reports telemetry enablement and scopes session credentials before execution begins. Ships nowhere: it lives in `plugins.disabled/aops`, which the build excludes.                                                                                                                                                                                                                    |
| `pkb`  | `Stop` / `SubagentStop` — **not built** | —                        | —                                                                     | Nothing. `plugins/pkb/hooks/handlers.py` registers `UserPromptSubmit` only. A session ends freely while it still holds an `in_progress` task.                                                                                                                                                                    | Wanted, not shipped: the intent is to hold the stop until the agent records its work and releases the task. The runtime can render the block (`lib/hooks/dispatch.py`, `block`); no handler emits one. Until it is wired, the claim → release boundary rests on the task contract and the reconcile sweep, not on session mechanics.                                                  |
| `rbg`  | `Stop` / `SubagentStop` — **not built** | —                        | —                                                                     | Nothing. `plugins/rbg/hooks/handlers.py` registers `PreToolUse` and `UserPromptSubmit` only. A session ends without any rule-compliance review having been required.                                                                                                                                             | Wanted, not shipped: the intent is to hold the stop until the agent invokes the RBG rule checker and presents checkable evidence. The runtime can render the block and `is_continuation` supplies the once-per-chain guard with zero state (`lib/hooks/dispatch.py`); no handler emits one. Pillar 4 is one layer today, not two.                                                     |
