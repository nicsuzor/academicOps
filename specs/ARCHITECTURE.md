...
substance of what an agent did. Two mechanisms now hold a stop open rather than
...
_stopping subagent's_ own context rather than the face's) directs ida to strip
its reply to load-bearing content before it speaks. Each obliges an action and
is silent on what it finds.
...
chain guard any such gate needs. Two plugins wire it, above; a third gate on
the same primitive, `pkb`'s task-release check, is specified and still unbuilt
(`ARCHITECTURE.md`, Hooks). This section governs: a gate described anywhere
else in this document but absent from a plugin's `HANDLERS` does not exist.
