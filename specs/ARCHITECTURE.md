...
James's own body no longer carries it. **Open defect, 2026-08-12 (needs
issue):** `plugins/orchestrate/README.md` and
`plugins/orchestrate/skills/strategic-review/SKILL.md` both still cite a
"Delegate, don't dictate" section of james's body that no longer exists, and a
dispatch that does not route through `brief` is told to delegate without being
told what a brief carries.
...
| `orchestrate` | `SubagentStop`                          | Claude Code                              | none                                                                                                                       | Advisory reminder (`warn`) carrying `plugins/orchestrate/hooks/messages/honesty.md`, the reporting protocol — every load-bearing claim paired with checkable evidence marked Observed or Reported, gaps and unfinished work named explicitly, a confidence level and the single check that would falsify the conclusion.                                           | Binds the **worker** at stop. In Polecat containers `SubagentStop` fires within the worker process; in native Task-tool runs the advisory lands in the caller's context with the completion notification. `SubagentStop` has no agy wire equivalent, so agy carries no honesty reminder — open defect, 2026-08-12 (needs issue).                                                                                                  |
