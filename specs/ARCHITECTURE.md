...
[`plugins/pkb/skills/brief/SKILL.md`](../plugins/pkb/skills/brief/SKILL.md).
...
| :------------ | :-------------------------------------- | :--------------------------------------- | :------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `orchestrate` | `PostToolBatch` — **not registered**    | —                                        | —                                                                                                                          | Nothing today. `plugins/orchestrate/hooks/messages/hearsay.md` ships and `rule_against_hearsay` is written, but `HANDLERS` in `plugins/orchestrate/hooks/handlers.py` carries no `PostToolBatch` entry, so the event reaches no handler. The receiver's half of the handback doctrine lands through the receiving agent's own body instead (orchestrate, above).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Would bind the **receiver** at the instant a synchronous report lands, `PostToolBatch` firing once after every call in a batch has resolved so a turn that dispatched several subagents is reminded once rather than once per report. `plugins/orchestrate/manifest/hooks.template.json` still declares the wire event, so the hook process runs and returns nothing.                                                                                                           |
| `orchestrate` | `Stop`                                  | Claude Code                              | none                                                                                                                       | Blocking reminder (`block`) carrying `plugins/orchestrate/hooks/messages/honesty.md`, the evidence contract — every load-bearing conclusion in a report carries falsifiable evidence, quoted verbatim with pinpoint citations, curated to the altitude of the claims the report itself makes rather than every intermediate step, so a supervisor can judge the reasoning without opening the sources; an assertion of absence carries the search methodology behind it. Trivial facts and a schema the task itself specifies are excepted; hearsay, raw dumps, hidden failures, meta-commentary and prose are barred. A subagent's claims and evidence may be relayed unverified when they already comply and are relevant to the report. Fires blocking (`block`) so the harness sets `stop_hook_active` to suppress re-entry on subsequent turns in the stop chain. Skipped when `agent_type` is `ida:ida`, which has its own reply gate. | Binds the **worker** at the boundary this actually reaches: a top-level `Stop` on Claude Code — the main session, an `--agent <name>` session, and james as a polecat's top-level agent. `SubagentStop` is not registered in `HANDLERS`, so a subagent handback receives nothing, in-session or inside a polecat. Not on agy: agy's `PostInvocation` maps onto canonical `Stop` and fires after every tool call, so registering it there re-injects the block per call (#2413). |
...
| `orchestrate` | `SubagentStop` — **not registered**     | —                                        | —                                                                                                                          | Nothing today. `honest_output` is written and would bind a worker at handback, but its `SubagentStop` entry in `HANDLERS` is commented out, so the event reaches no handler. `plugins/orchestrate/manifest/hooks.template.json` still declares the wire event, so the dispatcher runs and returns nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | The worker's half of the handback doctrine lands on `Stop` instead (above), which is the only boundary a non-subagent agent ever reaches. A subagent handback therefore receives nothing, in-session or inside a polecat.                                                                                                                                                                                                                                                       |
...
| `orchestrate` | `Stop`                                  | Both (Claude Code; AGY `PostInvocation`) | as above                                                                                                                   | Nothing injected. `stop` completes the trace and clears the session's tracer state. Declared `async` on Claude Code.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | **Observability.** The turn's trace is closed at its boundary. `Stop` carries two handlers in order — `stop` (this row) and `honest_output` (the honesty row above); the tracer half runs on both clients, the honesty half on Claude Code only.                                                                                                                                                                                                                                |
...
| `ida`         | `PostToolBatch`                         | Claude Code                              | none                                                                                                                       | Advisory quiet gate (`be_quiet`, `warn`): reminds ida to strip her own reply down to load-bearing content before she speaks to the person. Fires only when `agent_type` is `ida:ida`; every other agent's batch returns nothing. Always the same reminder — the hook has no transcript to judge, only that a batch has resolved. `PostToolBatch` is in `CONTINUATION_EVENTS`, so the dispatcher's self-loop guard keeps it to once per chain.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Fires once after a whole resolved tool batch, so a turn that ran many calls is reminded once rather than once per call. `PostToolBatch` has no agy wire equivalent (`lib/hooks/dispatch.py`), and the `PostInvocation` registration agy still carries maps to `Stop`, for which this plugin registers no handler — so the gate is Claude-only. See the parity note below the table.                                                                                             |
...
**Open defect, 2026-08-12 — ida's quiet gate does not fire on agy (needs
issue).** `plugins/ida/manifest/hooks.template.json` registers agy's
`PostInvocation`, which `TO_CANONICAL` maps to `Stop`; `HANDLERS` in
`plugins/ida/hooks/handlers.py` registers `PostToolBatch` alone. The agy
registration therefore spawns the dispatcher and runs nothing. Parity is owed:
the gate is Claude-only until either a `Stop` handler is registered or agy grows
a `PostToolBatch` equivalent.
...
**Two plugins can both register a block on the same client event** — each is a separate hook process, so `_merge`'s precedence is scoped to one plugin's own handler list and never adjudicates between them; the client fires both and each is honoured on its own. Today both `rbg`'s rule-check gate and `orchestrate`'s `honest_output` block on the face's `Stop`; `pkb`'s task-release gate (Hooks, above) would be the third once it ships.
