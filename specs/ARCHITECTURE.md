...
| `orchestrate` | `Stop` / `SubagentStop`                 | Claude Code              | none                                                                   | Advisory reminder (`warn`) carrying `plugins/orchestrate/hooks/messages/honesty.md`, the reporting protocol — every load-bearing claim paired with checkable evidence marked Observed or Reported, gaps and unfinished work named explicitly, a confidence level and the single check that would falsify the conclusion. Both events are declared synchronously: an `async` hook's output is discarded by the client, so an advisory declared `async` never reaches the agent. Skipped when `agent_type` is `ida:ida`, which has its own reply gate. | Binds the **worker** at stop and every other agent at its own turn boundary. For Polecat containers, `SubagentStop` fires within the worker process; for native Task-tool subagents under a parent CLI, `SubagentStop` advisory context lands in the caller's context upon task completion notification. Not on agy: agy's `PostInvocation` maps onto canonical `Stop` and fires after every tool call, so registering it there re-injects the block per call (#2413). |
