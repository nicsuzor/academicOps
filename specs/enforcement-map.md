...
| auto-mode `Backup File Prevention` (filename pattern) | warn     | PreToolUse  | active — task-06db60dc questions whether this should remain |
...
| auto-mode `Scope Discipline` (P#5)                  | warn                                  | PreToolUse           | flagged for removal (`task-06db60dc` — session-level pattern, not per-action) |
| auto-mode `Explicit Approval for Costly Ops` (P#50) | warn                                  | PreToolUse           | active — needs concrete threshold (`task-06db60dc`)                           |
...
| auto-mode `Delegated Authority Only` (P#99)        | warn     | PreToolUse  | flagged for removal (`task-06db60dc` — judgment call, not per-action) |
| auto-mode `Acceptance Criteria Own Success` (P#31) | warn     | PreToolUse  | flagged for removal (same)                                            |
...
| auto-mode `No Validation Bypass` (P#25)                          | block     | PreToolUse    | active — needs narrowing of `--force` to validation contexts (`task-06db60dc`) |
| auto-mode `Fail-Fast on Tool Failure` (P#9)                      | warn      | PreToolUse    | flagged for removal (`task-06db60dc` — workflow preference, not per-action)    |
| auto-mode `No Infrastructure Workarounds` (P#30)                 | warn      | PreToolUse    | flagged for removal (same)                                                     |
