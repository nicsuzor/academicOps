...
**Default location**: in-repo at `aops-core/<NAME>.md` (framework-wide — TAXONOMY, SURFACES, AXIOMS, GATES, etc.) or `.agents/<NAME>.md` (repo-local — ENFORCEMENT-MAP, CAPABILITIES). May live outside the repo in PKB or machine config when the slice depends on user or machine conditions. May be `.md` or `.yaml` (e.g. `polecat.yaml`).
