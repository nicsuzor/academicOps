...
- **Disposition**: Ida carries the academic/research disposition **inline in `ida.md`** (folded in 2026-06-27); Junior the framework-coordination one. There is no shared `academic-disposition.md` file — self-contained agents (doctrine revision above).
...
## Agent roster — disposition & capabilities

The framework ships six first-class agents in two classes: **head personalities** (own the user-facing surface, hold/drive a session) and the **review crew** (commissioned subagents that return verdicts). Each agent definition is **self-contained** — its disposition lives inline, not in a shared file (doctrine revision above). Source: `aops-core/agents/<name>.md` frontmatter. Keep this table current when an agent's disposition or `tools:` allowlist changes.

| Agent      | Class  | Disposition (what it is)                                                                                                                                                                                                                                                                               | Used for                                                                                                               | General capabilities (tool surface)                                                                                                                                     | Output                                           |
| :--------- | :----- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------- |
| **junior** | head   | Framework coordinator + default head personality. Interactive: holds between steps, answers self-answerable questions, delegates for context hygiene. Framework-coordination disposition.                                                                                                              | Day-to-day framework interaction; the user-facing chat; routing to subagents/polecat; task + memory management.        | Read/Write/Edit/Glob/Grep/Bash, Skill, Agent, AskUserQuestion, Outlook, Zotero; PKB **read + knowledge-writes + lifecycle** (deny-by-default — no graph-shaping verbs). | coordination, dispatch, task/memory mgmt         |
| **ida**    | head   | Interactive academic-research co-worker. Same hold-between-steps quality floor as Junior **+ academic-research disposition inline** (data immutable · questions drive design · reproducibility · methodological transparency · fail-fast on data quality). Dispatch default = local delegate-and-wait. | Research repos (auto-selected via `"agent": "ida"` in `.claude/settings.json`); live step-by-step research co-working. | Same floor as Junior + Outlook + Zotero; PKB read + knowledge-writes + lifecycle (no graph-shaping).                                                                    | research co-working                              |
| **rbg**    | review | The Judge — axiom-violation reviewer. Applies the universal axioms with **judgment, not mechanical matching**; fixes clear mechanical violations, flags judgment calls.                                                                                                                                | Compliance audits; PR `enforcer-status`; the periodic enforcer gate; `/strategic-review`.                              | Bash, Read, Grep, Glob, Edit, Write; PKB read.                                                                                                                          | OK / WARN / BLOCK (cite violation by axiom slug) |
| **pauli**  | review | The Architect of Thought & Memory — strategist + PKB custodian; the **sole graph-shaper** (decomposition, epics, edges, priority structure) via `/planner`.                                                                                                                                            | Design/alignment review (PKB design-intent, not axioms); PKB graph maintenance; strategic planning.                    | Read, Skill, Bash, Write, Outlook; **full PKB incl. graph mutation**.                                                                                                   | strategy/alignment verdict; graph edits          |
| **marsha** | review | The QA Reviewer — runtime verification. Assumes **it's broken until proven otherwise**; actually runs things; catches criterion substitution.                                                                                                                                                          | QA verification before exit; `/verify`; PR `qa-status`.                                                                | Read, Bash, Skill, Agent, Playwright (browser); PKB read.                                                                                                               | PASS / FAIL / REVISE                             |
| **james**  | review | The Orchestrator — multi-agent review reconciler. Commissions rbg + pauli + marsha, evaluates and synthesises.                                                                                                                                                                                         | PR reviews, design reviews, any artifact needing multi-perspective assessment; `/strategic-review`.                    | Read, Agent, Skill; PKB read.                                                                                                                                           | APPROVE / REVISE / ESCALATE                      |

**Head-agent posture axis** (junior/ida only): **interactive (non-polecat)** = hold between steps, soft handover, user drives; **polecat (autonomous)** = drive-to-done, hard handover. Resolved per-surface off `is_polecat_surface()` — see [`specs/ENFORCEMENT-MAP.md`](../ENFORCEMENT-MAP.md) §1 and the posture-gate rows. The review crew (rbg/pauli/marsha/james) are stateless commissioned subagents — they carry no posture.
...
5. **Salvage from PR #1987** (do NOT merge it as a unit). _(Update 2026-06-27: superseded by the self-contained doctrine — `academic-disposition.md` was folded INLINE into `ida.md` and deleted, not kept; the `/analyst` skill now states its disposition floor inline; analyst de-dup finished.)_ Original note: keep `academic-disposition.md`; fold the genuinely-shared parts of the interactive-posture prose into the shared skill (most of it is shared, not Ida-specific); **drop** the `session_type`-filtered hook policies, the standalone interactive template, the template-registry entry, and the 8 tests; **finish the analyst de-dup** (the PR added a pointer but did NOT remove `analyst/SKILL.md`'s duplicated immutability/reproducibility prose). Close or repurpose #1987.
...
- Ida's research disposition — **inlined in `aops-core/agents/ida.md`** (2026-06-27; the standalone `academic-disposition.md` was folded in and deleted, and the `/analyst` skill made self-contained — self-contained-agents doctrine).
