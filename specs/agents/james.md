...
- **Agreements between reviewers are merged, not duplicated.** Because RBG, Pauli, and Marsha review blind to each other in parallel, they can independently converge on the same underlying defect from their own distinct lens (e.g. RBG's axiom-compliance finding and Marsha's runtime-fitness finding naming the same code). When two or more reviewers flag the same underlying issue, the synthesis carries **one** entry naming every concurring reviewer and each one's distinct rationale — never a separate entry per reviewer restating the same defect. Unmerged duplicate rows are a synthesis defect, not thoroughness.
