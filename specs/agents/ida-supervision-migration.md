...
| **Driving**      | Getting a worker onto a surface, and the loop around that call                                        | `plugins/aops/skills/dispatch/SKILL.md` owns kit selection, launch, and collection; `.agents/skills/debug/SKILL.md` is the project-local half; orchestrate owns the loop |
...
**Certifying its own work.** Certification is commissioned, not performed: where acceptance turns on judgment rather than a probe, the verdict comes from marsha via `plugins/aops/skills/verify/SKILL.md` and is recorded. Ida adjudicating a unit is not ida certifying it, and stage 4 does not merge them. Most sharply, **ida can never be the channel that scores ida** -- every gate above is void if the measurement comes from ida's own account.
