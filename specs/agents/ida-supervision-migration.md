...
| **Driving**      | Getting a worker onto a surface, and the loop around that call                                        | `plugins/orchestrate/agents/pc.md` owns mode, image freshness, and launch; `.agents/skills/debug/SKILL.md` is the project-local half; orchestrate owns the loop |
