...
> Status: **draft spec** — proposes a structural rework of the v1 pipeline (`specs/pr-pipeline.md` historical / `pr-pipeline` PKB note). This document is the contract for migration; v1 remains the operative pipeline until each phase below ships.
>
...
1. **Does `enforcer-status` _require_ the rbg agent to actually run, or can a no-op static rule (e.g. "skip if diff is markdown-only") suffice?** v2 currently treats the status as the contract and is silent on whether a static check that posts `enforcer-status: success` constitutes valid coverage. Risk of abuse if a consumer wires a dummy emitter.

2. **What happens when `alignment-status` sits at `pending` because the host is down?** Spec proposes a 1-hour watchdog flipping it to `failure`. Open: should the watchdog instead post `success` with description "auto-passed (host unreachable)" so PRs aren't blocked by infrastructure outages? Trade-off between availability and integrity of the alignment gate. **Default in spec**: `failure` (integrity wins) — flag here so the user can override.
...
- PKB knowledge note: `pr-pipeline` (current spec; this v2 supersedes once Phase 5 completes — see "supersession protocol" below)
- `.github/rulesets/pr-review-and-merge.yml` — the ruleset to update in Phase 1 / Phase 3
- `.agents/ENFORCEMENT-MAP.md` — updated in this PR (see new "PR-pipeline agents" section)

### Supersession protocol

This document is `specs/pr-pipeline-v2.md`, sitting alongside `pr-pipeline` (the v1 PKB note) — not replacing it yet. v1 stays the operative description until Phase 5 ships. At that point:

- `pr-pipeline` (v1 note) is rewritten as a "historical / migrated" stub pointing here.
- `specs/pr-pipeline-v2.md` either stays at this filename (with a "v2" tag) or is renamed to `specs/pr-pipeline.md` (canonical). User decides at Phase 5; either is consistent with the rest of the framework.

The choice to ship as `pr-pipeline-v2.md` rather than overwriting v1 is deliberate: v1 references are widespread in PKB; an overwrite during the migration window would invalidate them. v2 sitting alongside lets us migrate references progressively per phase.
