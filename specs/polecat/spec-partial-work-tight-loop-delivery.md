...
- **The pre-dispatch decomposition gate** (the `decompose` skill) was built to run **live, default, mandatory** — the skill is retired to `plugins.disabled/skills.disabled/decompose/SKILL.md` and excluded from the build, so this gate does not currently execute. Its collision with "thin briefs, worker plans" was resolved to **NARROW** (a DECISION:Nic fork, decided reversibly): eligibility is now a composable rule pauli records when assembling the task's workflow at decomposition, not a two-tier epic carve-out — a rule with no shipped enforcement while the gate itself is unbuilt.
