...
- **NARROWS feature-dev's "No partial success"** (`aops/skills/aops/references/feature-dev-details.md` L34/L211/L278). That rule scopes to **a single claimed leaf**: _the claimed unit_ works completely or doesn't ship. It does **not** forbid cutting a large brief into a smaller whole leaf and shipping that. The in-place edit to feature-dev-details.md (Stage 5 chunk) must state this scoping explicitly — the rule is narrowed where it lives, not contradicted at a distance.
- **The pre-dispatch decomposition gate** (`aops/skills/planner/SKILL.md`; `decompose.md` steps 14-16) is **live, default, mandatory**. Its collision with "thin briefs, worker plans" is resolved to **NARROW** in §5 (was a DECISION:Nic fork; now decided, reversibly).
