...
3. Unless `--repo-dir` was given, clones the resolved repo (`git clone --local
   ...
   active branch's up-to-date `HEAD` if `--base` is omitted), and repoints `origin`
...
7. Builds the inner command from `AGENT_CMD` (`claude`, `agy`, `shell`/`bash`,
   ...
   rather than idling forever. `polecat.yaml`'s `timeout` supplies that value on _every_ headless dispatch.
8. On completion, verifies delivery before reporting success: for a seeded `agy`
   ...
   moved, the new commit must be present on the remote (directly matching a remote
   ref tip or reachable on a remote branch after fetch). A delivery-guard failure
...
3. **Delivery guard** — Test: a `run` that leaves uncommitted changes, or commits
   that never reach the remote (checked via `ls-remote` ref tips and remote-tracking
   reachability), exits non-zero and (with `--task`) names the task in the failure.
