...
> **Capability caveat — `silent`/ephemeral on `Stop` is the aspiration, not yet universally achievable.** These dispositions are the **uniform design intent for every client**; there is deliberately no per-client variant. Where a client cannot currently honour one, that is a **capability gap, not intended divergence**. The known gap: **no Claude/Gemini agent-only `Stop` channel** is known to exist today — every agent-visible Stop payload appears _also_ to be user-visible — so the Stop `silent`/ephemeral cells are honoured on agy but not yet on Claude/Gemini. Candidate fixes (an untested Stop field; or relocating the reminder to the next `UserPromptSubmit`) and the empirical capability cells live in [`CLIENT-TRANSLATION.md`](CLIENT-TRANSLATION.md). **This finding is under active re-investigation** — it is suspected to have regressed with a Claude Code version change rather than being a true API limit (see the open follow-up task).
>
