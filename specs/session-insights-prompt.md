...
Extract ALL Framework Reflection or Session Handover sections found in the transcript. These structured reflections are output by agents at session end (via `/dump`, `/learn`, or `/release` commands) and contain valuable session metadata.

**Where to find them:**

- Look for `## Framework Reflection` markdown headers in assistant messages
- Look for `### Session Handover` markdown headers in assistant messages (new format)
...
- **Primary Task**: [task_id] (title)
- **PR**: [url]
- **Branch**: [branch_name]
- **Issue**: [url or "none"]
- **Follow-ups**: [task_ids] (or "none")
...
### 14. Timeline Events (Path Reconstruction)

Array of event objects capturing key operations during the session. Typically extracted by `transcript_parser.py` rather than generated by LLM, but conforming to this structure:
...
```json
{
  ...
  "timeline_events": [
    ...
    {
      "timestamp": "2026-01-13T10:15:00+00:00",
      "type": "task_release",
      "task_id": "task-xxxx",
      "status": "done",
      "summary": "Release summary"
