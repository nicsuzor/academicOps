...
class ReferenceCandidate:
    ...
    line_end: int
    content: str
    suggested_reference: str | None
    existing_chunk: Path | None
...
class ReferenceRefactorer:
    ...
    REFERENCE_PATTERN = re.compile(
        r"@(?:bots/prompts/|references/|\.claude/prompts/|\$ACADEMICOPS_[A-Z]+/)[^\s]+\.md"
    )
    ...
    def _get_content_signature(self, file_path: Path) -> str | None:
    ...
    def refactor_file(
        self, claude_file: Path, dry_run: bool = False
    ...
    def _find_candidates(
        ...
        for i, line in enumerate(lines):
            ...
            if self.REFERENCE_PATTERN.search(line):
                if current_block:
                    ...
                    candidate = self._create_candidate(
                        file_path, current_block, block_start, i - 1
                    )
                    if candidate:
            ...
            is_instruction = any(
                re.match(pattern, line.strip()) for pattern in instruction_patterns
            )
        ...
        if current_block and len(current_block) > 2:
            candidate = self._create_candidate(
                file_path, current_block, block_start, len(lines) - 1
            )
            if candidate:
                candidates.append(candidate)
    ...
    def _create_candidate(
        self, file_path: Path, content_lines: list[str], start_line: int, end_line: int
        ...
        content_sig = " ".join(
            [line.strip().lower() for line in content_lines if line.strip()]
        )
    ...
    def _apply_refactoring(
        self, file_path: Path, lines: list[str], candidates: list[ReferenceCandidate]
    ) -> None:
        ...
        while i < len(lines):
            ...
            if (
                candidate_idx < len(candidates_sorted)
                and i == candidates_sorted[candidate_idx].line_start
            ):
    ...
    def consolidate_references(self, claude_file: Path, dry_run: bool = False) -> int:
        ...
        if duplicates_removed > 0 and not dry_run:
            ...
            print(
                f"✅ Removed {duplicates_removed} duplicate references from {claude_file}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Refactor CLAUDE.md files to use references"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--consolidate", action="store_true", help="Consolidate duplicate references"
    )
    ...
    if path.is_file() and path.name == "CLAUDE.md":
        ...
        for claude_file in claude_files:
            ...
            if args.consolidate:
                duplicates = refactorer.consolidate_references(
                    claude_file, dry_run=args.dry_run
                )
                if duplicates:
                    print(f"   Removed {duplicates} duplicate references")
