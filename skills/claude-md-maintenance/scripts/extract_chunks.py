...
class ChunkExtractor:
    ...
    REFERENCE_PATTERN = re.compile(
        r"@(?:bots/prompts/|references/|\.claude/prompts/)[^\s]+\.md"
    )
    ...
    def _generate_chunk_name(self, content: str) -> str:
        ...
        if "python" in lower_content and "type" in lower_content:
            return "python_typing"
        if "python" in lower_content:
            return "python_development"
        if "git" in lower_content and "commit" in lower_content:
            return "git_commit_practices"
        if "git" in lower_content:
            return "git_workflow"
        if "docker" in lower_content:
            return "docker_configuration"
        if "api" in lower_content:
            return "api_guidelines"
        if "debug" in lower_content:
    ...
    def _determine_target_path(
        self, source_file: Path, content: str, chunk_name: str
    ) -> Path:
        ...
        is_project_specific = any(
            marker in content.lower() for marker in project_markers
        )
        ...
        is_reusable = (
            any(marker in content.lower() for marker in reusable_markers)
            and not is_project_specific
        )

        if is_project_specific:
            # Project tier
            return self.base_dir / "bots" / "prompts" / f"PROJECT_{chunk_name}.md"
        if is_reusable and self.framework_dir:
...
"""
    ...
    def _update_claude_file(
        self, claude_file: Path, chunks: list[ContentChunk]
    ) -> None:
        ...
        while i < len(lines):
            ...
            if (
                chunk_idx < len(chunks_sorted)
                and i == chunks_sorted[chunk_idx].start_line
            ):
...
def main():
    ...
    if path.is_file() and path.name == "CLAUDE.md":
        ...
        extractor.extract_from_file(path, dry_run=args.dry_run)
    elif path.is_dir():
