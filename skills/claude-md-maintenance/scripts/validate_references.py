...
class ReferenceValidator:
        ...
        for i, line in enumerate(lines, 1):
            ...
            for pattern in self.REFERENCE_PATTERNS:
                for match in pattern.finditer(line):
                    ...
                    reference = self._validate_reference(claude_file, i, ref_text, ref_path)
    ...
    def _validate_reference(
        ...
        if ref_path.startswith("$"):
            ...
            target_path, status, error = self._resolve_relative_reference(source_file, ref_path)
    ...
    def _resolve_env_reference(self, ref_path: str) -> Tuple[Optional[Path], ReferenceStatus, Optional[str]]:
        ...
        if ref_path.startswith("$ACADEMICOPS/"):
            ...
            if not self.personal_dir:
                return None, ReferenceStatus.INVALID_PATH, "$ACADEMICOPS_PERSONAL not set"
            ...
            return None, ReferenceStatus.INVALID_PATH, f"Unknown environment variable in {ref_path}"
    ...
    def _resolve_relative_reference(
        ...
        source_file: Path,
        ref_path: str
    ) -> Tuple[Optional[Path], ReferenceStatus, Optional[str]]:
    ...
    def check_circular_references(self) -> List[List[Reference]]:
        ...
        def find_cycle(node: Path, visited: Set[Path], path: List[Path]) -> Optional[List[Path]]:
            if node in path:
                # Found cycle
            ...
            if start_node not in visited_global:
                ...
                if cycle:
                    ...
                    for i in range(len(cycle) - 1):
                        ...
                        for ref in self.references:
                            if ref.source_file == cycle[i] and ref.target_path == cycle[i + 1]:
    ...
    def print_report(self) -> None:
        ...
        if circular_chains:
            ...
            for i, chain in enumerate(circular_chains, 1):
                print(f"\n   Chain {i}:")
                for ref in chain:
                    print(f"      {ref.source_file.name} → {ref.target_path.name if ref.target_path else 'unknown'}")
        ...
        if not_found:
            ...
            for ref in not_found[:5]:  # Show fixes for first 5
                ...
                matches = [
                    ...
                    if ref_name.lower() in f.name.lower()
                ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate references in CLAUDE.md files")
