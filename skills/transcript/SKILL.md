...
**Output location**: Automatically saved to `$ACA_DATA/sessions/claude/`
...
**Both versions always generated** - full (with tool results) and abridged (without tool results).

**Custom output name** (optional):

```bash
# Basename only - saves to $ACA_DATA/sessions/claude/
uv run python scripts/session_transcript.py session.jsonl -o custom-name

# Absolute path - saves to specified location
uv run python scripts/session_transcript.py session.jsonl -o /full/path/to/transcript
```
