...
class SkillUpdater:
    ...
    def convert_to_imperative(self, content: str) -> str | None:
            ...
            if line.startswith("#") and " " in line:
                ...
                if match:
                    ...
                    if re.match(r"How to ", heading, re.IGNORECASE):
                        new_heading = re.sub(
                            r"How to ", "", heading, flags=re.IGNORECASE
                        )
                        new_heading = (
                            new_heading[0].upper() + new_heading[1:]
                            if new_heading
                            else new_heading
                        )
            ...
            for update_name, update_info in PATTERN_UPDATES.items():
                ...
                if matches:
                    if auto_apply:
                        if callable(update_info["replacement"]):
                            content = re.sub(
                                pattern, update_info["replacement"], content
                            )
                        else:
                            content = re.sub(
                                pattern, update_info["replacement"], content
    ...
    def update_python_file(
        self, file_path: Path, auto_apply: bool
    ) -> list[dict[str, str]]:
