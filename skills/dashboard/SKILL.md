...
Data source: `$ACA_DATA/dashboard/insights.json` (generated by session-insights skill Step 6)
...
| Session Insights | Pre-mined from `$ACA_DATA/dashboard/insights.json` (created by session-insights skill, not via Claude API) |
...
**CRITICAL**: synthesize_dashboard.py does NOT call Claude API. It loads pre-computed insights from `$ACA_DATA/dashboard/insights.json`, created offline by session-insights skill via Gemini. This keeps dashboard fast and reduces API dependencies.

Dashboard sources for Session Insights Panel:
- Insights JSON structure (skill compliance, corrections, failures, successes)
- Top context gaps extracted from failure analysis
- Metrics aggregated per-session then rolled up to daily view

If insights.json is missing: Dashboard will gracefully skip Session Insights panel. This is not an error—it just means session-insights skill hasn't run yet for that date.
