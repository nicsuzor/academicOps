...
- Query existing tasks for `source_email: <entry_id>`
- If found: Skip creation, note in summary
- If multiple actions from same email: All share same source_email, different titles

**Implementation**:

```
Scripts backend:
  - Read current_view.json (generated by task_view.py)
  - Check if any task has matching source_email metadata
  - Or: grep data/tasks/inbox/*.md for "source_email: <entry_id>"

MCP backend:
  - Use view_tasks() with no filtering
  - Check returned tasks for source_email field
```
