...
# --hostname is set only when the environment provides one — no baked device
# name. When unset, `tailscale up` falls back to its own default naming.
HOSTNAME_ARGS=()
[ -n "${HOSTNAME:-}" ] && HOSTNAME_ARGS=(--hostname="claude-web-${HOSTNAME}")
