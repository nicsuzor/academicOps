...
> **Note**: For user-facing prioritisation and ranking, use `focus_score` — the canonical composite (additive integer composition) that embeds severity, priority, deadline pressure, age/staleness, `downstream_weight`, stakeholder waiting, an `urgency` term, and a value-of-information term (`voi_value`, capped at 5000). See [[multi-parent]] §2.2 for the full, current model. Component fields (`downstream_weight`, `criticality`, `uncertainty`, and the surfaced `voi_value` / `urgency` debug values) remain visible in metadata for filtering, classification, and debugging, but ranking always goes through `focus_score` and should never be driven off a single component.
...
**`due` is exclusively for real-world deadlines — a date with an external consequence if missed** (a submission portal closes, a meeting happens, a filing window ends). It is never a proxy for how much the user wants something done or how soon they'd like to start. When Nic states intent without an external deadline — _"I want to do X today"_, _"let's get to this this week"_ — that is **P1 (Active intent)**, not a fabricated `due` date: P1's definition is literally "important to the user right now." Agents **must never originate a `due` date that has no external referent** — inventing one to make urgency "show up" in `focus_score` poisons deadline trust for every real deadline on the graph. If P1 is too crowded to carry the signal, the fix is Nic demoting other P1s, never an agent inventing a deadline.
...
**Promotion `→ queued` requires a recorded premise judgment.** Crossing into the dispatchable set is the universal chokepoint every piece of work passes through before compute is spent on it. Full procedure — what the promoter records, what the dispatcher does at `/pull`/`/dispatch`/`/supervisor` — is [[premise-gate]]; this is the operative home, not restated here.
...
- Promotion `→ queued` requires the promoter to record a one-sentence premise judgment in the body ([[premise-gate]]); dispatch hard-refuses a task that has none
