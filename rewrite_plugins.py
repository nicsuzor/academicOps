...
def rewrite_plugin(path):
    ...
    content = content.replace("from result import Result, warn", "from dispatch import Result, warn")
    content = content.replace("from result import Result, refuse", "from dispatch import Result, refuse")
    ...
    if "import messages" in content:
        ...
        content = content.replace("messages.load_pair(ctx.hooks_dir, ", "load_message_pair(ctx.hooks_dir, ")
        content = content.replace("messages.load_pair(ctx.hooks_dir,", "load_message_pair(ctx.hooks_dir,")
        content = content.replace('ctx.message("ruleset")', 'load_message_pair(ctx.hooks_dir, "ruleset")[0]')
