...
`agents/<name>.md.<client>` instead — one file per client, each written natively
...
alongside `<name>.md.<client>` fails the build rather than picking a winner.
"""
...
def resolve_client_agents(agents_dir: Path, client: str) -> None:
    """Resolve `<name>.md.<client>` variants in `agents_dir`, in place."""
    ...
    for other in CLIENTS:
        for variant in sorted(agents_dir.rglob(f"*.md.{other}")):
            if other != client:
                variant.unlink()
                continue

            target = variant.with_suffix("")
            if target.exists():
                raise BuildError(
