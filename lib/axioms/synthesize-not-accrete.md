...
History lives in version control and the session transcript — surfaces that are append-only by construction. It does not live in the knowledge store, and a task record is not exempt: a task body is a checklist of work to be done, rewritten in place as items complete, never a log of what happened.
