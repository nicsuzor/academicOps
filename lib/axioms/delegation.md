...
**A worker's final message is its entire return contract, and only a modelled surface is guaranteed to deliver it.** The surfaces above are the ones this framework actually carries a report back through: a subagent's or fork's last message returns to you directly; a polecat container's contract lands on the task; a queued task's brief is read on claim. A named/teammate-mode spawn — anything reached through `SendMessage`, `TeamCreate`, or a channel keyed to an idle/notification signal rather than a returned message — is not one of these: nothing here models its report delivery, in unmodelled spawn modes the harness may hand a child the spawner's own tool roster instead of its declared one, and the caller gets only an idle signal while the worker's actual output strands in its own transcript. Do not commission tool-bearing or report-bearing work through it. Whatever surface you do use, the worker's own final turn must carry its complete result — full evidence, as the last thing it does, never a status line — because nothing downstream reads anything earlier than that turn.
