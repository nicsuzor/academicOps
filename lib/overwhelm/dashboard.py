        ...
        if path.abandoned_work:
            ...
            for proj, items in sorted(abandoned_by_project.items()):
                ...
                for ab in items:
                    ...
                    task_id_val = ab.task_id or "unknown"
                    path_html += f"<div class='path-abandoned-item' style='border-left: 2px solid {proj_color}; background: rgba(255,255,255,0.03); padding: 4px 10px; border-radius: 4px; font-size: 0.85em;' title='Task ID: {esc(task_id_val)}'>□ {task_label}</div>"
                path_html += "</div>"
                path_html += "</div>"
