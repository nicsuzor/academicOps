...
class GitHubToBeads:
    ...
    def fetch_from_api(
        self, repo: str, token: Optional[str] = None, state: str = "all"
    ):
    ...
    def map_priority(self, labels: List[str]) -> int:
        ...
        label_names = [
            label.get("name", "").lower() if isinstance(label, dict) else label.lower()
            for label in labels
        ]
    ...
    def add_dependencies(self):
        ...
        for gh_issue_data in getattr(self, "_gh_issues", []):
            ...
            for ref_gh_id in referenced_gh_ids:
                ...
                if ref_bd_id:
                    dependencies.append(
                        {"issue_id": "", "depends_on_id": ref_bd_id, "type": "related"}
                    )
...
Examples:
  ...
  python gh2jsonl.py --repo owner/repo --prefix myproject --id-mode hash
    ...
    parser.add_argument(
        "--file", type=Path, help="JSON file containing GitHub issues export"
    )
    parser.add_argument(
    ...
    parser.add_argument("--prefix", default="bd", help="Issue ID prefix (default: bd)")
    parser.add_argument(
        "--start-id", type=int, default=1, help="Starting issue number (default: 1)"
    )
    parser.add_argument(
        "--id-mode",
