...
The main agent reads `mcp_pkb_get_task(<epic-id>)` and the epic body's `## Work Items`, `## Pattern Memory`, and `## Supervisor State` blocks. **No other reads.** No grep, no child task body fetches, no transcript reads.
