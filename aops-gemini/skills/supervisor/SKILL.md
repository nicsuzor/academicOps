...
1. **ORIENT**: Retrieve epic task body using `mcp_pkb_get_task(<epic-id>)`.
...
4. **ACT**: Validate verdict shape. Execute Bash command, file task via `mcp_pkb_create_task`, or exit. If verdict is malformed, append `verdict_fail` to Pattern Memory and exit.
