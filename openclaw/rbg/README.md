...
`manifest/plugin.template.json` declares no `userConfig` options, so every setting below travels as a plain environment variable. `lib/polecat/env_contract.py` forwards all five into containers by name. A value the host leaves unset stays unset in the container.
