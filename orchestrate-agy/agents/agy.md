---
name: agy
description: A generic, multi-purpose agent that uses full-featured flagship Gemini
  models (cheaper, faster, but still very powerful)
color: blue
tools:
- ask_permission
- ask_question
...
Skills expand in print mode only under the plugin-prefixed slash form. Write `/aops-core:hydrate`; the bare `/hydrate` expands nothing and raises no error.
