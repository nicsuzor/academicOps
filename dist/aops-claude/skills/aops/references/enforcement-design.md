...
| 6    | **Propagation** — same instruction reaches every surface that hits the failure (the `exercise-authority` lesson: per-surface coverage, not a new mechanism) | Failure recurs because one surface never loaded the rule                                                |
