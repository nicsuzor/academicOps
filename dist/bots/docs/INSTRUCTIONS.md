# Project Instructions

Project-specific instructions for agents working in this repository.

## Project Context

- **Repository**: [owner/repo]
- **Purpose**: [Brief description of what this project does]
- **Priority**: [P0-P3 or other priority scheme]
- **Architecture**: [Key architectural patterns or technologies]

## Development Rules

1. [Project-specific development standards]
2. [Testing requirements]
3. [Code review process]
4. [Any special considerations]

## Key Tools

- [List of commands for common tasks]
- [Testing frameworks]
- [Build/deployment tools]

## Project-Specific Workflows

[Describe any project-specific workflows, debugging steps, deployment processes, etc.]

---

For generic development methodology, see academicOps core instructions loaded automatically at session start.
