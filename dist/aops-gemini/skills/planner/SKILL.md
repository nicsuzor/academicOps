...
5. **Extract structured metadata** if mentioned in description or conversation:
   ...
   - `classification`: the executable-vs-needs-a-spike classifier (works with `uncertainty`; see the `classification` entry in [[../remember/references/TAXONOMY.md]]). If the user's prompt suggests a spike/probe/research shape, the planner SHOULD set `classification` accordingly (`spike`, `research`, etc.) so the leaf is correctly classified. (It would also feed the **proposed-but-not-yet-implemented** VoI ranking term — epic [[aops-9f5bfbe2]], spec draft [[mem-cf396d84]]; that term is not live in `focus_score` today.) **Never override a user-set `classification`** — only fill it in when absent.
6. Create task with body template (Problem, Solution, Files, AC). Apply the Task-Body Authoring Discipline ([[../aops/references/authoring-discipline]]): **state intent + AC, not prescription**, and do not invent mid-stream approval gates. Pass `due`, `effort`, `consequence`, and `priority` as explicit PKB parameters to `mcp_pkb_create_task` (not only in body prose) — the PKB uses `due` as a structured field for deadline-aware prioritization. **Priority defaults to P3** unless user explicitly elevated.
...
- **Prioritisation** (ACROSS): Use graph topology and computed properties to rank tasks. Surface high-`focus_score`, low-uncertainty tasks as ready priorities. Ranking uses `focus_score` — the canonical composite (additive integer composition) that embeds severity, priority, deadline pressure, age/staleness, downstream weight, stakeholder waiting, and an `urgency` term. See [[multi-parent]] for the current authoritative model. `uncertainty` is a separate `[0,1]` classifier (executable vs. needs-a-spike), **not** a focus_score term — use it to _classify_ tasks, not to rank them. A value-of-information term (`AF(x) = E[U(x)] + E[VoI(x)] × …`) is **proposed but NOT implemented** (epic [[aops-9f5bfbe2]], survey [[kb-9230ba76]]) — do not treat `voi_value` as live until it lands in code. Component fields (`urgency`, `downstream_weight`, etc.) remain visible for filter/debug but should never be the primary sort.

**Philosophy**:

- Plans are hypotheses, not commitments
- Prioritise by `focus_score` — the single composite ranking signal (see [[multi-parent]]); never rank by a lone component
...
3. **Property Check**: Check the parent's `scope` and `uncertainty`.
   ...
   - **Set `classification` on each subtask** to match its shape: spike subtasks get `classification: spike`, research subtasks get `classification: research`, execution subtasks omit `classification` (absent is treated as execution). This drives the executable-vs-needs-a-spike classifier (see the `classification` entry in [[../remember/references/TAXONOMY.md]]). It would also feed the **proposed-but-not-yet-implemented** VoI ranking term (epic [[aops-9f5bfbe2]], spec draft [[mem-cf396d84]]); that term is not live in `focus_score` today.
...
11. **RBG pre-flight check**: invoke RBG inline to review the proposed decomposition for axiom violations before creating tasks.
...
13. Create in PKB via `mcp_pkb_decompose_task(parent_id, subtasks)`.
...
- **Retire the superseded original (`superseded_by`)**: When the decomposition _carves an existing task's work into the new subtasks_ (or into siblings, or otherwise replaces it), the original must leave the dispatchable set in the **same** operation — do not leave it `queued`/`ready` with a now-stale body. Stamp `superseded_by: [<new-ids>]` on the original via `mcp_pkb_update_task`; this retires it (the PKB closes it) and leaves the redirect readable **on the task itself**, not only in this epic's Log. This is the #1584 fix: a redirect recorded only as prose in a parent's Log is invisible to `/pull`, which then dispatches the fossil. Do NOT rewrite the original's stale body — supersession is about dispatchability, not body content. See [[../remember/references/TAXONOMY.md#supersession-and-retirement-superseded_by]].
...
11. **Compose-only surface** — Planner is the canonical compose-only surface. It writes task bodies (briefs) into PKB and exits. **Planner never dispatches a worker against a brief it authored in the same invocation.** Dispatch is owned by /supervisor (the next tick reads the PKB body fresh) or by the polecat pull cycle. This is the planner-side projection of [Compose-then-Dispatch Separation](../aops/references/authoring-discipline.md#3-compose-then-dispatch-separation-recusal-propagated-to-the-dispatch-surface) — `recusal` propagated to the dispatch surface. Planner's "exit after writing to PKB" is the cure for the same-context author-and-dispatcher failure class. **Investigation boundary:** Pauli must not perform investigation (reading source code, running Bash, synthesising findings) as part of a planning invocation. Frame the question, name data sources, write the brief, and exit. See [Investigation boundary](../aops/references/authoring-discipline.md#investigation-boundary-paulis-identity-layer-projection-of-recusal).

## Decision Surfacing Heuristic

_Enforces `exercise-authority` Edge 2 (FM-2, FM-3, FM-4). This applies the Decision Surfacing Heuristic from the Task-Body Authoring Discipline ([[../aops/references/authoring-discipline]])._
...
- **Recording a DECIDE-class action without executing it.** "Cancel X once Y lands" / "supersede Z" / "retitle W" are actions whose decision is already made. Writing them into a task body as future work converts a DECIDE into a DEFER and leaks dead nodes into the graph. If the action is safe and unambiguous, do it in the same turn — do not narrate it as a deferred follow-up. Body-prose is allowed only as the record AFTER the action has been executed. `exercise-authority` Edge 2 (FM-1) applies recursively: don't ask, don't note-for-later, just act.
- **Re-surfacing a delegated agent's recommendation as a sign-off gate** (`exercise-authority` Edge 2, FM-2). When pauli/marsha/a subagent returns a recommendation with reasoning, you delegated the decision — execute it. Returning it to the user as "pauli recommends X, want to proceed?" is the agent rubber-stamping pattern.
- **Self-answered rhetorical questions** (`exercise-authority` Edge 2, FM-4). If you can write the answer in the same paragraph as the question, it is rhetorical. Act on the answer.
...
- **Goals** (`type: goal`) → Goals are identity-level commitments (the tier above targets). They carry **no severity, no consequence, no due** — identity commitments are unquantifiable. Never assign severity to a goal node. Targets (and work tasks, when no intermediate target applies) wire to goals via `contributes_to`; goals carry no operational stakes metadata.
...
- Planner modes (capture, decompose) and external skills (supervisor, survey, /q, /learn) MUST pass `severity=0` (or omit) when calling `mcp_pkb_create_task` or `mcp_pkb_decompose_task` for task-type nodes.
