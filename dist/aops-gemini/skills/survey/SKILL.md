...
the session id. Never infer "no markdown exists anywhere" from a single failed `list_directory` in one
guessed directory.
...
```
read_file(file_path="<path>", offset=1, limit=500)
read_file(file_path="<path>", offset=500, limit=500)
# continue until EOF
```
...
| PKB tasks            | Task lifecycle data                 | Via `mcp_pkb_task_search`     |
...
```
mcp_pkb_get_task(id="epic-a0523a25")  # halt if not in_progress
```
...
- **single-task**: `mcp_pkb_create_task` with issue body, AC, and `Closes #N` instruction. Apply the Trust the Worker doctrine ([[../aops/references/authoring-discipline]]).
...
template = mcp_pkb_get_task(id="epic-a0523a25")
cycle_id = "epic-a0523a25-" + YYYYMMDD + "-" + HHMM  # e.g. epic-a0523a25-20260125-1430
instance = mcp_pkb_create_task(id=cycle_id, body=template.body)
mcp_pkb_append(id=cycle_id, content="<cycle entry per schema>")
```
...
```
activate_skill(name="qa", args="Verify cycle <N> of /issue-sweep on epic-a0523a25. Sample 20% (min 3). Reviewers: Pauli (cohesion) + RBG (axiom compliance). Add Marsha if single-tasks dispatched.")
```
