...
```python
mcp_tasks_list_tasks(status="queued")
mcp_tasks_search_tasks(query="[keywords]")
```
...
```python
task = mcp_tasks_get_task(id="[TASK_ID]")
...
Phase 1 - Search:
mcp_tasks_search_tasks(query="instruction scope learning")
→ Found: aops-42 "learning: agents ignoring explicit scope instructions"

Phase 2 - Update existing task:
task = mcp_tasks_get_task(id="aops-42")
mcp_tasks_update_task(id="aops-42", body="[existing + new observation]")
...
Phase 1 - Search:
mcp_tasks_search_tasks(query="prompt_router TypeError bug")
→ No matching tasks
