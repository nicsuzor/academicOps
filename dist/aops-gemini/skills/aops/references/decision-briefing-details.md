...
```python
mcp_tasks_search_tasks(query="[query]")           # Search tasks
mcp_tasks_get_blocked_tasks()                      # Tasks with unmet dependencies
mcp_tasks_list_tasks(status="queued")              # Filter by status
mcp_tasks_list_tasks(project="<slug>")             # Filter by project (preferred over walking parent chains or ID-prefix heuristics)
mcp_tasks_complete_task(id="[ID]")                 # Complete task
mcp_tasks_update_task(id="[ID]", updates={"status": "someday"})   # Defer for later
mcp_tasks_update_task(id="[ID]", updates={"body": "..."})       # Add notes to task
```
...
mcp_tasks_search_tasks(query="RFC")

# Tasks explicitly marked as needing approval
mcp_tasks_search_tasks(query="approval")

# Blocked tasks (something is in the way)
mcp_tasks_get_blocked_tasks()

# Experiments needing direction - search for experiment tag
mcp_tasks_search_tasks(query="experiment")

# Investigations with proposed solutions
mcp_tasks_search_tasks(query="Investigate")
```
...
```python
# For approved RFCs
task = mcp_tasks_get_task(id="[ID]")  # Verify still active
mcp_tasks_update_task(id="[ID]", updates={"body": task["body"] + "\n\nApproved by user [DATE]"})
mcp_tasks_complete_task(id="[ID]")
# Note: Implementation task creation is SEPARATE work, not part of this workflow

# For rejected RFCs
mcp_tasks_update_task(id="[ID]", updates={"body": "Rejected: [user-provided reason if any]", "status": "cancelled"})

# For deferred items
mcp_tasks_update_task(id="[ID]", updates={"status": "waiting"})

# For prioritization decisions - add note documenting decision
task = mcp_tasks_get_task(id="[ID]")
...
Agent:
1. mcp_tasks_get_task(id="ns-p8n") → still active ✓
2. mcp_tasks_complete_task(id="ns-p8n")  # with approval note
3. mcp_tasks_get_task(id="ns-0ct") → still active ✓
4. mcp_tasks_update_task(id="ns-0ct", updates={"status": "cancelled"})
5. mcp_tasks_update_task(id="ns-tme", updates={"status": "waiting"})
