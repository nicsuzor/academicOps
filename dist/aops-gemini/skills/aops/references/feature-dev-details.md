...
6. **Create task** (per AXIOM #28 - episodic content → tasks): `mcp_tasks_create_task(title="experiment: [feature-name]", tags=["experiment"], body="[experiment plan content]")`
...
7. **MANDATORY: Spawn critic agent for independent review before commit**:
   ...
   ```
   activate_skill(name="aops-core:pauli", model="haiku",
   ...
   ```
   activate_skill(name="aops-core:pauli", model="opus",
...
5. **Complete experiment task**: `mcp_tasks_complete_task(id="[task-id]")` with body noting "Synthesized to spec: [spec-name]"
