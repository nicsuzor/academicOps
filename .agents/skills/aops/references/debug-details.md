...
| **RBG CANNOT_ASSESS**            | Audit file has incomplete context | Expected for short sessions; not a real failure |
...
3. RBG spawned but returned `CANNOT_ASSESS` (incomplete context)
4. Write tool failed ("file not read"), Bash heredoc also failed ("file exists")
5. Session timed out before completing

**Root cause**: Framework overhead (TDD prescription + rbg) consumed all time on a trivial task.
