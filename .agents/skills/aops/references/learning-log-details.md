...
```python
mcp__services__pkb__list_tasks(status="queued")
mcp__services__pkb__task_search(query="[keywords]")
```
...
```python
task = mcp__services__pkb__get_task(id="[TASK_ID]")
...
Phase 1 - Search:
mcp__services__pkb__task_search(query="instruction scope learning")
→ Found: aops-42 "learning: agents ignoring explicit scope instructions"

Phase 2 - Update existing task:
task = mcp__services__pkb__get_task(id="aops-42")
mcp__services__pkb__update_task(id="aops-42", body="[existing + new observation]")
...
Phase 1 - Search:
mcp__services__pkb__task_search(query="prompt_router TypeError bug")
→ No matching tasks
