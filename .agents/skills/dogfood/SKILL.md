...
- **The named surface is part of the instruction.** When a run specifies where it executes — a client, a container, an agent, a model — that surface is the condition under test. If execution on the specified surface is blocked, report the blocker and halt.
