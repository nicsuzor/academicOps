...
- **Environment approval gate**: Human merge approval happens via the GitHub `production` Environment on `summary-and-merge.yml` after the merge-prep agent graduates the PR — no comment-trigger mechanism
...
| Merge-prep re-runs lint/typecheck/pytest before graduation | PR 585 showed merge triggers silently failing when checks were stale; graduation now verifies current status rather than trusting prior runs                                                                             | 2026-02-23 |
