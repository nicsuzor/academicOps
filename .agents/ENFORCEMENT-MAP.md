...
| Mechanism              | Source                               | Rule(s)                             | Scope                       | Tier | Cost / Impact                                      | Behaviour                                                                                                                                                                                                        |
| ---------------------- | ------------------------------------ | ----------------------------------- | --------------------------- | ---- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ensure_triage_labels` | `aops-core/lib/triage_labels.py`     | (procedural: label bootstrap)       | Each tracked repo per cycle | n/a  | L0 · negligible (4 × `gh label create --force`)    | Idempotently provisions the canonical `triage:*` labels (name, colour, description) in every tracked repo at the top of each cron cycle. Warned-not-raised on per-label failure so one repo can't abort the run. |
...
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ------- | -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `enforcer-status` (rbg)            | Trigger: `.github/workflows/trigger-enforcer.yml`; Reusable: `.github/workflows/agent-enforcer.yml@enforcer-v1` (tag pending post-merge) + `.github/agents/enforcer.agent.md` (sources `aops-core/agents/rbg.md`)             | A1, A6, A8 (axiom compliance, scope, halt); P#65 (enforcement-map row required for new agents)      | Every PR with non-doc-only diff                                 | `block` | L6 · ~1.5–3k tok + 5–30s per fire × 1 per HEAD SHA (SHA-skip dedupe)                               | Reviews PR diff against axioms. Posts GitHub PR review (APPROVED / CHANGES_REQUESTED) + `enforcer-status` on HEAD SHA. SHA-skip prevents double-review of unchanged HEAD.                                                                          |
