...
every session, forever. The cheapest sufficient level wins. Indicative
costs below are order-of-magnitude — measure when proposing.

| Level | Mechanism                               | Indicative ongoing cost                                           | When justified                                                 |
| ----- | --------------------------------------- | ----------------------------------------------------------------- | -------------------------------------------------------------- |
...
| L4    | PreToolUse gate (`warn`)                | L3 cost + ~50–200 tok per fire                                    | Periodic compliance check; subagent dispatch acceptable        |
| L5    | PreToolUse gate (`block`)               | L3 cost + latency to evaluate every call                          | Hard-block needed; destructive / legal / privacy               |
| L6    | LLM-gated hook (subagent call per fire) | ~1.5–3k tok + 5–30s latency × every fire                          | Last resort; structural fixes (L1–L3) have failed              |
| L7    | Numbered axiom (A-tier)                 | Permanent context burn + every surface must reference             | Rule must beat trained reflex; cross-cutting; primary contract |
...
| `qa` gate             | Stop             | `aops-core/lib/gates/definitions.py`                    | (R: QA before completion)                         | All sessions                          | `warn`  | L3 · ~615 tok × 1/session                                   | Requires QA verification before session can stop                         |
