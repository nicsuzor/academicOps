...
def _load_baseline() -> dict[str, dict[str, int]]:
    ...
    return data.get("baseline", {})  # allow-fallback: absent key = no grandfathered sites, not an error
...
def _apply_baseline(violations: list[dict], baseline: dict[str, dict[str, int]]) -> list[dict]:
    ...
    for v in violations:
        ...
        pattern = v["pattern"]
        file_baseline = baseline.get(rel) or {}  # allow-fallback: file not in baseline = no grandfathered violations
