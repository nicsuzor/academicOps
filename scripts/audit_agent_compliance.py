...
def main():
    ...
    with open(COMPLIANCE_FILE, "w") as f:
        ...
        f.write(
            "Generated by `scripts/audit_agent_compliance.py`. "
            "Re-run to refresh; `git log` for regeneration history.\n\n"
        )
    ...
    # 4. Format generated files with dprint so table alignment is deterministic.
    # The generator emits unaligned tables; dprint right-pads cells to match the
    # widest cell per column. Without this step, regenerating creates noisy diffs
    # against the dprint-formatted state pre-commit would otherwise enforce.
    _format_with_dprint([COMPLIANCE_FILE, TOOL_MATRIX_FILE, REMEDIATION_FILE])


def _format_with_dprint(paths: list[Path]) -> None:
    """Run `dprint fmt` on the given files if dprint is available.

    Idempotency depends on this — without alignment, the generator's output drifts
    from the dprint-formatted file that pre-commit enforces.
    """
    ...
    if dprint is None:
        print("⚠️  dprint not found on PATH; output may not match repo formatting.")
        return
    subprocess.run([dprint, "fmt", *existing], check=True)
