...
def extract_scenarios_from_log(log_path, scenario_groups=None):
    ...
    # 10. Subagent internal tool calls (is_subagent=True, any tool)
    subagent_internal = []
    for idx, event in events:
        if event.get("hook_event") != "PreToolUse":
            continue
        if not event.get("is_subagent"):
            continue
        tn = event.get("tool_name") or ""
        # Skip spawn tools — those are covered by spawn_in_main_session
        if tn in ("Agent", "Task", "delegate_to_agent"):
            continue
        s = _event_to_scenario(event, idx, str(path), platform, "subagent_internal_tools")
        subagent_internal.append(s)

    if subagent_internal:
        group_name = f"{platform}_subagent_internal_tools"
        if group_name not in scenario_groups:
            scenario_groups[group_name] = []
        if len(scenario_groups[group_name]) < SCENARIO_GROUP_LIMIT:
            remaining = SCENARIO_GROUP_LIMIT - len(scenario_groups[group_name])
            scenario_groups[group_name].extend(subagent_internal[:remaining])

    # 11. Agent/Task spawn calls in main session (PreToolUse for spawn tools)
    spawn_calls = []
    for idx, event in events:
        if event.get("hook_event") != "PreToolUse":
            continue
        tn = event.get("tool_name") or ""
        if tn not in ("Agent", "Task", "delegate_to_agent"):
            continue
        s = _event_to_scenario(event, idx, str(path), platform, "spawn_in_main_session")
        spawn_calls.append(s)

    if spawn_calls:
        group_name = f"{platform}_spawn_in_main_session"
        if group_name not in scenario_groups:
            scenario_groups[group_name] = []
        if len(scenario_groups[group_name]) < SCENARIO_GROUP_LIMIT:
            remaining = SCENARIO_GROUP_LIMIT - len(scenario_groups[group_name])
            scenario_groups[group_name].extend(spawn_calls[:remaining])
