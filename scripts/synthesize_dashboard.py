...
def extract_narrative_signals(daily_content: str) -> dict:
    ...
    These are generated by extract_narrative.py during /session-insights.

    Returns:
        Dict with:
            - session_context: list of timestamped intents
            - abandoned_todos: list of unfinished items
    """
    result = {
        "session_context": [],
        "abandoned_todos": [],
    ...
    # Extract Abandoned Todos section
...
def load_yesterday_log(data_dir: Path) -> dict:
    """Load yesterday's daily log for carryover detection.

    Returns:
        Dict with unchecked items from yesterday
    """
    yesterday = (datetime.now() - timedelta(days=1)).strftime("%Y%m%d")
    daily_path = data_dir / "sessions" / f"{yesterday}-daily.md"
    ...
    # Extract unchecked items from yesterday
    unchecked = re.findall(r"- \[ \] (.+)", content)
    result["unchecked_items"] = unchecked[:5]  # Limit

    # Extract abandoned todos from yesterday
    signals = extract_narrative_signals(content)
    result["abandoned_todos"] = signals["abandoned_todos"][:5]
...
- SESSION CONTEXT (what was started): {session_context}
- ABANDONED TODAY: {abandoned_today}
- CARRYOVER FROM YESTERDAY: {yesterday_carryover}

Analyze this data and output a JSON object with these fields:

{{
  "narrative": [
    "<bullet 1: what the day started with>",
    "<bullet 2: where work shifted or got interrupted>",
    "<bullet 3: what remains undone from today and yesterday>"
  ],
  "accomplishments": {{
...
RULES:
- narrative: 3-5 bullets telling the day's story, each under 80 chars
  - Use SESSION CONTEXT for what was started
  - Note any context switches or distractions
  - Include ABANDONED TODAY and CARRYOVER FROM YESTERDAY for unfinished work
...
def main() -> None:
    ...
    print("Loading yesterday's log...")
    yesterday_log = load_yesterday_log(data_dir)
    ...
    # Format narrative signals for prompt
    session_context_str = "; ".join(narrative_signals["session_context"][:5]) if narrative_signals["session_context"] else "No session context"
    abandoned_today_str = "; ".join(narrative_signals["abandoned_todos"][:3]) if narrative_signals["abandoned_todos"] else "None"

    # Format yesterday carryover
    yesterday_items = yesterday_log["unchecked_items"] + yesterday_log["abandoned_todos"]
    yesterday_carryover_str = "; ".join(yesterday_items[:5]) if yesterday_items else "None"
    ...
    prompt = SYNTHESIS_PROMPT.format(
        ...
        abandoned_today=abandoned_today_str,
        yesterday_carryover=yesterday_carryover_str,
    )
    ...
    synthesis["data_sources"] = {
        ...
        "session_context": len(narrative_signals["session_context"]) > 0,
        "yesterday_log": len(yesterday_log["unchecked_items"]) > 0 or len(yesterday_log["abandoned_todos"]) > 0,
    }
