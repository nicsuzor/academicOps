...
do_transcript() {
    ...
    if [[ -f "${AOPS}/scripts/transcript.py" ]]; then
        # Removed --no-sync to resolve G12a task-9c0710ae gap
        uv run python "${AOPS}/scripts/transcript.py" --recent || echo "Warning: transcript generation failed" >&2
    else
