...
if [ -d "${GRAPH_DIR}/.git" ]; then
    # Resolve conflicts in graph files (incoming version wins)
    echo "    Checking for graph file conflicts..."
    git -C "${GRAPH_DIR}" diff --name-only --diff-filter=U | grep -E "graph.*\.(dot|svg|json|graphml)" | xargs -r git -C "${GRAPH_DIR}" checkout --theirs 2>/dev/null || true
    git -C "${GRAPH_DIR}" diff --name-only --diff-filter=U | grep -E "graph.*\.(dot|svg|json|graphml)" | xargs -r git -C "${GRAPH_DIR}" add 2>/dev/null || true
