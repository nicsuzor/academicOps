...
# calling agent-enforcer/agent-qa @dev on pull_request) and a
...
#
# WHAT GETS INSTALLED
#   .github/workflows/trigger-enforcer.yml  — fires agent-enforcer @dev on PR
#   .github/workflows/trigger-qa.yml        — fires agent-qa @dev on PR
...
jobs:
  qa:
    uses: nicsuzor/academicOps/.github/workflows/agent-qa.yml@dev
    with:
      pr_number: ${{ github.event.pull_request.number }}
      ref:       ${{ github.event.pull_request.head.ref }}
      sha:       ${{ github.event.pull_request.head.sha }}
