    ...
    try:
        ...
        if result and result.get("success") and msg_id:
            ...
            if task_info:
                ...
                task_dict.setdefault("source", {}).setdefault(
                    "draft_history", []
                ).append(
                    {
...
def modify_email(
            ...
            print_json(
                {
                    ...
                    "message": "No local task with this id",
                }
            )
            return {}
    ...
    if use_json:
        ...
        payload = {"messageId": message_id, "actions": {}}
        if archive:
    ...
    try:
        if archive and result and result.get("success"):
            ...
            if related_task:
                ...
                fm = result.get("finalMessageId") or (result.get("actions") or {}).get(
                    "archive", {}
                ).get("newId")
                if fm:
    ...
    if cmd == "draft":
        ...
        if len(sys.argv) > 2:
            ...
            if not raw.strip():
                ...
                print(
                    "  From file: task_process.py draft /path/to/draft.json",
                    file=sys.stderr,
                )
                print(
                    "  From stdin: echo '{...}' | task_process.py draft",
                    file=sys.stderr,
                )
                sys.exit(1)
            try:
