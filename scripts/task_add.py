...
def main():
    ...
    group.add_argument("--summary", "--details", dest="summary", default="", help="A detailed summary of the task.")
    group.add_argument("--details-from-file", help="Path to a file containing the task details.")
    ...
    if args.details_from_file:
        try:
            with open(args.details_from_file, 'r') as f:
                summary = f.read()
        except FileNotFoundError:
            print(f"Error: File not found for --details-from-file: {args.details_from_file}", file=sys.stderr)
            sys.exit(1)
