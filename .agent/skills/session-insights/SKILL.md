...
Insights are saved to `$ACA_SESSIONS/summaries/YYYYMMDD-{session_id}.json` using the unified schema (combining insights + dashboard data).
...
INSIGHTS_FILE="$ACA_SESSIONS/summaries/${DATE}-${SESSION_ID}.json"
...
- Save raw response to `$ACA_SESSIONS/summaries/YYYYMMDD-{session_id}.debug.txt`
...
$ACA_SESSIONS/summaries/20260113-a1b2c3d4.debug.txt
...
- Check `$ACA_SESSIONS/summaries/*.debug.txt` for raw Gemini responses
