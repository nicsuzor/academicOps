...
1. **Pre-hoc, at decomposition.** The `decompose` skill (pauli's lens) settles a unit's premise, worth, and acceptance criteria as part of cutting the work into a DAG — see [`aops/skills/decompose/SKILL.md`](../../decompose/SKILL.md). Dispatch surfaces (`/pull`, `/dispatch`) trust that decomposition and do not re-judge the premise themselves.
