...
**`due` is exclusively for real-world deadlines — a date with an external consequence if missed** (a submission portal closes, a meeting happens, a filing window ends). It is never a proxy for how much the user wants something done or how soon they'd like to start. When Nic states intent without an external deadline — _"I want to do X today"_, _"let's get to this this week"_ — that is **P1 (Active intent)**, not a fabricated `due` date: P1's definition is literally "important to the user right now." Agents **must never originate a `due` date that has no external referent** — inventing one to make urgency "show up" in `focus_score` poisons deadline trust for every real deadline on the graph (confirmed by a blind test, 2026-07-07: two independent agents converted stated same-day intent into `due: today`, one citing this very section as justification — see [[mem-624664d1]]). If P1 is too crowded to carry the signal, the fix is Nic demoting other P1s, never an agent inventing a deadline.
