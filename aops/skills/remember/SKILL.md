...
- Use `mcp__services__pkb__search` for semantic discovery. Do not use `Glob` or `Grep` on `$ACA_DATA/`.

### Hard Rules

- `mcp__services__pkb__create`: Create a document (projects, contexts, meeting notes).
- `mcp__services__pkb__create_memory`: Add a memory/note.
- `mcp__services__pkb__append`: Extend an existing document.
- `mcp__services__pkb__get_document`: Read a document.
- `mcp__services__pkb__search`: Search before creating new content — and before asserting a fact. Don't assume; look it up.
...
| Content                | Tool                                                                | Target                                                                                           |
| :--------------------- | :------------------------------------------------------------------ | :----------------------------------------------------------------------------------------------- |
| **Epics/projects**     | `mcp__services__pkb__create(type="epic"\|"project")`                | `projects/`                                                                                      |
| **Tasks/issues**       | `gh issue create` (GitHub primary)                                  | Synced automatically                                                                             |
| **Durable knowledge**  | `mcp__services__pkb__create` or `mcp__services__pkb__create_memory` | `knowledge/`, `context/`                                                                         |
| **Session findings**   | `mcp__services__pkb__update_task` on parent task                    | Task body                                                                                        |
| **Axioms/framework**   | `/framework` skill — do NOT write to `brain/context/`               | `skills/` via framework workflow                                                                 |
| **Context/goals**      | `mcp__services__pkb__create`                                        | `context/`, `goals/`                                                                             |
| **Meeting/call notes** | `mcp__services__pkb__create(type="meeting-note")`                   | `knowledge/<topic>/` or `projects/`                                                              |
| **Daily notes**        | `mcp__services__pkb__create(type="daily")`                          | Canonical; SSoT is [[aops/skills/daily/SKILL.md]] — derive the path there, don't restate it here |
...
- Update existing notes with `mcp__services__pkb__append`. Create new notes only when the topic does not exist.
...
1. **Search**: `mcp__services__pkb__search(query="topic")`.
2. **Evaluate**: If a canonical note exists, append to it. Do not create a separate file.
3. **Write**: Call `mcp__services__pkb__create(title, body, type, tags)` or `mcp__services__pkb__create_memory(title, body, tags)`.
