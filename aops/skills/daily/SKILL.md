---
name: daily
description: Daily note lifecycle — compose and maintain a cognitive prosthetic that makes the next action obvious. SSoT for daily note structure.
context: fork
agent: "aops:pauli"
---
...
A cognitive prosthetic designed to make the next action obvious (per Nic's ruling in the 2026-07-20 era correction cluster). It reports the state of the day, but it may also prioritise and recommend next actions to reduce cognitive load. Forward prioritisation is still supported by the user-owned `### My priorities` heading, which must be created empty and preserved across regenerations.
...
- **No empty sections, except `### My priorities` — never omit or delete that heading.** Omit every other section rather than render an empty heading (e.g. on a morning with no sessions yet, omit Today's Log entirely). `### My priorities` is the one structural exception: it is a template guard, so the generator must always write it, keep it empty if the user has added nothing, and preserve anything the user writes under it across regenerations. Its omission is blocked by that guard.
- **Add a per-project rollup section.** Ensure the note contains a project rollup showing one line per active project (`academicOps`, `brain`, `buttermilk`, `mem`, `sessions`, `overwhelm`) with its queued/blocked/needs-Nic state. Per the template's fixed order (`references/note-template.md`), this section sits below the decision-critical zone — it's reference detail, not orientation.
- **Dedup the lede.** The `daily_narrative` frontmatter field and the body lede must not contain duplicate content. Hard cap the lede to 2-3 present-tense lines at the very top.
- **Prioritize decisions; time-sensitive items rank FIRST.** Escalated Deadlines, Calendar (today), and "Needs your call" are the actionable zone and sit immediately after the lede, per the template's fixed order. Collapsible audit logging, PR/workflow status, raw task counts, and the per-project rollup default to collapsed or pushed below the fold, so that a cold reader gets full orientation from the top ~10 lines of the note.
- **Replace raw aggregate counts with curated focus.** In the Status section, display the top-3 highest-priority or focus tasks and a one-line count delta (e.g. `+2 ready, -1 blocked`) since the last run. Hide raw task counts inside a collapsed details block.
