...
def test_parse_valid_hook_log_files() -> None:
    ...
    Verifies that real hook log files from production
    (~/.claude/projects/<project>/*-hooks.jsonl) can be parsed completely without
    JSON errors.
    """
    from pathlib import Path
    ...
    assert hook_files, "No hook log files found in session directory"
    ...
    assert fully_valid_files, (
        f"No completely valid hook log files found. "
        f"Total valid lines parsed: {total_lines_parsed}"
    )
    assert total_lines_parsed > 0, "No valid JSON lines were parsed from hook log files"
