...
class TestHydrationDetection(unittest.TestCase):
    def setUp(self):
        ...
        self.temp_dir = Path(
            "/home/nic/.gemini/tmp/02446fdfe96b1eb171c290b1b3da4c0aafff4108395fdefaac4dd1a188242b94/hydrator"
        )
    ...
    def test_gemini_read_file_detection(self, mock_get_temp_dir):
        ...
        tool_input = {"file_path": str(self.temp_dir / "hydrate_xbq_whk1.md")}
        ...
        tool_input = {"file_path": "/some/other/path/hydrate_xbq_whk1.md"}
        is_detected = gate_registry._hydration_is_gemini_hydration_attempt(
            "read_file", tool_input, self.input_data
        )
    ...
    def test_gemini_run_shell_detection(self, mock_get_temp_dir):
        ...
        tool_input = {"command": f"cat {self.temp_dir}/hydrate_xbq_whk1.md"}
        is_detected = gate_registry._hydration_is_gemini_hydration_attempt(
            "run_shell_command", tool_input, self.input_data
        )
        self.assertTrue(
            is_detected, "Should detect run_shell_command on hydration temp file"
        )
    ...
    def test_post_hydration_trigger_gemini(self, mock_session_state, mock_get_temp_dir):
        ...
        # Assertions
        mock_session_state.update_hydration_metrics.assert_called_with(
            "test-session", turns_since_hydration=0
        )
    ...
    @patch("hooks.gate_registry.hook_utils.get_hook_temp_dir")
    @patch("hooks.gate_registry.session_state")
    def test_post_hydration_trigger_gemini_negative(
        self, mock_session_state, mock_get_temp_dir
    ):
