...
class TestHydratorReadOnly:
    ...
    def test_hydrator_cannot_use_edit_tool(self):
        ...
        with patch.dict(
            os.environ, {"CLAUDE_SUBAGENT_TYPE": "aops-core:prompt-hydrator"}
        ):
            ctx = GateContext(
    ...
    def test_main_agent_allowed_mutating_tools(self):
        ...
        env_without_subagent = {
            k: v for k, v in os.environ.items() if k != "CLAUDE_SUBAGENT_TYPE"
        }
        ...
        for variant in variants:
            with patch.dict(os.environ, {"CLAUDE_SUBAGENT_TYPE": variant}):
                ...
                assert result is not None, (
                    f"Should block Edit for hydrator variant: {variant}"
                )
                assert result.verdict == GateVerdict.DENY
