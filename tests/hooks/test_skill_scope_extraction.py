#!/usr/bin/env python3
"""Test skill scope extraction for enforcer context.

Regression test for: ns-siv (enforcer false positives on multi-step skills)
Root cause: enforcer saw active skill name but NOT the skill's authorized
workflow steps. When /learn does investigation+fix+test, enforcer blocked
...
enforcer_gate.py includes it in the context for compliance checking.
"""
...
class TestLoadSkillScope:
    """Test skill scope loading for enforcer context."""

    def test_load_skill_scope_for_learn_command(self):
        """Verify /learn command scope includes workflow steps.

        Regression: enforcer blocked /learn investigation+testing as scope creep
    ...
    def test_load_skill_scope_extracts_workflow_steps(self):
        ...
        that should be extracted for enforcer context.
        """
        scope = load_skill_scope("learn")
