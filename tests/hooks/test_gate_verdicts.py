...
class TestSubagentSessionSkipsGates:
    """All subagent sessions skip gate dispatch entirely.

    Backed by real log data (gemini_subagent_internal_tools) plus synthetic scenarios.
    Subagent tool calls are invisible to gates — the parent's Agent tool call
    is the only operation that counts.
    """

    SCENARIOS = _flatten_scenarios(
        "non_compliance_subagent_blocked",
        "gemini_subagent_internal_tools",
    )

    @pytest.mark.parametrize(
        "scenario",
        SCENARIOS[:20],  # Cap parametrization
        ids=[s["id"] for s in SCENARIOS[:20]],
    )
    def test_subagent_skips_gates(self, router, hydration_mode, scenario):
        state = _make_session_state(scenario)
        ctx = _make_context(scenario)
        ...
        assert result is None, (
            f"[{scenario['id']}] Subagent sessions should skip gates entirely, "
