...
2. Environment variable substitution (${AOPS})
...
class TestNoLegacyFallbacks:
    ...
    def test_does_not_load_from_old_agents_location(
        ...
        monkeypatch.setenv("AOPS", str(fake_bot))
        monkeypatch.chdir(tmp_path)
...
class TestEnvironmentVariables:
    ...
    def test_fails_without_academicops_bot(self, bot_root, monkeypatch, tmp_path):
        """Test that script fails without AOPS env var."""
    ...
    def test_works_without_academicops_personal(self, bot_root, monkeypatch, tmp_path):
        """Test that script works without ACA (optional)."""
        monkeypatch.setenv("AOPS", str(bot_root))
        monkeypatch.delenv("ACA", raising=False)
        monkeypatch.chdir(tmp_path)
...
5. **Environment Variables**: Required (AOPS), optional (ACA)
