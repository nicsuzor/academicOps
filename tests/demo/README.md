...
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [[test_reflexive_loop.py]]   | Enforcer fires during normal agent operation. Runs 6 bash commands → enforcer threshold reached → audit file created. Validates the self-reflexive monitoring loop works.                                                                                   |
...
| test_hook_sequence.py        | Hooks fire at correct lifecycle events: SessionStart (session completes), UserPromptSubmit (hydrator temp file created), PostToolUse (enforcer audit file created).                                                                                         |
...
| test_compliance_detection.py | Enforcer produces substantive, well-formed audit output. Uses semantic validation (not keyword matching): checks Session Context has ≥3 content lines, AXIOMS has ≥10 numbered items, HEURISTICS has ≥10 H-items, no repeated headers, content ≥1000 chars. |
| test_core_pipeline.py        | Full hydration pipeline works end-to-end: Stage 1 hydrator invoked → Stage 2 TodoWrite plan created → Stage 3 python-dev skill invoked → Stage 4 code written via Write tool.                                                                               |
...
| Enforcer                       | ~50%     | Threshold firing + audit structure verified; violation detection (drift, missing skills, axiom breaches) not yet tested |
...
- Other subagent types (planner, explore, enforcer variants)
