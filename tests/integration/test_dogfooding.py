...
class TestDogfoodingInstallation:
    """Validate installation script works in ${AOPS} itself."""
    ...
    def test_install_script_can_run_in_academicops(self):
        """Installation script should successfully run in ${AOPS}."""
        academicops = Path(os.environ.get("AOPS", "/home/nic/src/bot"))
        ...
        assert result.returncode == 0, (
            f"Installation failed in ${AOPS}:\n{result.stderr}"
        )

    def test_installation_creates_expected_structure(self):
        """After installation, ${AOPS} should have complete structure."""
        academicops = Path(os.environ.get("AOPS", "/home/nic/src/bot"))
...
class TestDogfoodingRuntime:
    """Validate runtime behavior when running Claude in ${AOPS}."""

    def test_hooks_execute_successfully(self):
        """Hooks should execute without errors in ${AOPS}."""
        academicops = Path(os.environ.get("AOPS", "/home/nic/src/bot"))
    ...
    def test_claude_md_files_discoverable(self):
        """CLAUDE.md files should be discoverable in ${AOPS}."""
        academicops = Path(os.environ.get("AOPS", "/home/nic/src/bot"))
...
class TestDogfoodingGitIntegration:
    """Validate git operations work correctly in ${AOPS}."""
    ...
    def test_bots_directory_tracked_in_git(self):
        ...
        # In ${AOPS}, /bots/ is source, should be in git
        result = subprocess.run(
            ["git", "ls-files", "bots/"],
            check=False,
            cwd=str(academicops),
...
class TestDogfoodingVsProjectInstallation:
    """Validate ${AOPS} structure matches what we install in projects."""

    def test_claude_folder_structure_matches(self):
        """${AOPS}/.claude/ should match project installation template."""
        academicops = Path(os.environ.get("AOPS", "/home/nic/src/bot"))
        ...
        # ${AOPS}/.claude/ should have same structure
        academicops_settings = academicops / ".claude/settings.json"

        assert academicops_settings.exists(), (
            "${AOPS}/.claude/settings.json missing (should match project template)"
        )
        ...
        assert "hooks" in academicops_config, "${AOPS} missing hooks"
    ...
    def test_bots_structure_matches_what_we_install(self):
        """${AOPS}/bots/ should be source for what we symlink in projects."""
        academicops = Path(os.environ.get("AOPS", "/home/nic/src/bot"))

        # ${AOPS}/bots/ is the source
        ...
        for subdir in required_subdirs:
            ...
            assert path.exists(), (
                f"${AOPS}/bots/{subdir}/ missing (source for project installations)"
            )

            # Should contain files (not empty)
            files = list(path.iterdir())
            assert len(files) > 0, f"${AOPS}/bots/{subdir}/ empty"
