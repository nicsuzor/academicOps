...
def task_server_setup(tmp_path_factory):
    ...
    p_a_root_res = create_task(task_title="Project A Root", project="proj-a", type="project")
    assert p_a_root_res["success"]
    p_a_root_id = p_a_root_res["task"]["id"]

    p_a_child1_res = create_task(task_title="Child 1 (A)", project="proj-a", parent=p_a_root_id)
    assert p_a_child1_res["success"]
    p_a_child1_id = p_a_child1_res["task"]["id"]

    blocker_res = create_task(task_title="Blocker Task", project="proj-a", status="active")
    assert blocker_res["success"]
    blocker_id = blocker_res["task"]["id"]
    update_task(id=p_a_child1_id, depends_on=[blocker_id])

    p_b_root_res = create_task(task_title="Project B Root", project="proj-b", type="project")
    ...
    create_task(task_title="Spoke 1", project="proj-b", depends_on=[hub_id])
    create_task(task_title="Spoke 2", project="proj-b", depends_on=[hub_id])

    parent_id = p_b_root_id
    for i in range(5):
        res = create_task(task_title=f"Deep Task {i + 1}", project="proj-b", parent=parent_id)
        assert res["success"]
        parent_id = res["task"]["id"]

    create_task(task_title="A done task", project="proj-a", status="done")
    create_task(task_title="Ready Task", project="proj-a", status="active")
...
class TestGetTasksWithTopology:
    def test_no_filters(self, task_server_setup):
        ...
        result = get_tasks_with_topology()
        assert result["success"]
        assert result["count"] == 14

    def test_filter_by_project(self, task_server_setup):
        """Test filtering by project."""
        result = get_tasks_with_topology(project="proj-a")
        assert result["success"]
    ...
    def test_filter_by_status(self, task_server_setup):
        ...
        result = get_tasks_with_topology(status="done")
        assert result["success"]
    ...
    def test_filter_by_min_depth(self, task_server_setup):
        ...
        result = get_tasks_with_topology(min_depth=5)
        assert result["success"]
        assert result["count"] == 1
    ...
    def test_filter_by_min_blocking_count(self, task_server_setup):
        ...
        task_ids = task_server_setup
        result = get_tasks_with_topology(min_blocking_count=2)
        assert result["success"]
        assert result["count"] == 1
    ...
    def test_topology_metrics_are_correct(self, task_server_setup):
        ...
        result = get_tasks_with_topology(project="proj-b")
        assert result["success"]
        ...
        result_a = get_tasks_with_topology(project="proj-a")
    ...
    def test_ready_days_calculation(self, task_server_setup):
        ...
        result = get_tasks_with_topology(status="active")
        assert result["success"]
        ...
        result_done = get_tasks_with_topology(status="done")
