try:
    ...
    class mock_mark:
        def parametrize(self, *args, **kwargs): return lambda f: f
    class mock_pytest:
        mark = mock_mark()
    pytest = mock_pytest()
...
from lib.session_naming import generate_session_filename, generate_base_name
...
class TestTranscriptNamingIntegration:
    ...
    def test_generate_transcript_filename_uses_naming_module(self):
        ...
        session_path = Path("/home/worker/.claude/projects/-home-worker-src-aops/session-uuid.jsonl")
        ...
        with mock.patch.object(Path, "stat", side_effect=mock_stat):
            filename, date_str, short_project, session_id, slug = transcript._generate_transcript_filename(
                session_path, entries, slug="test-slug"
            )
