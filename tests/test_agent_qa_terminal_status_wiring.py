"""Functional test for agent-qa.yml's "Post terminal status" step.

Extracts the REAL "run:" script from the live workflow file and executes it
against a stubbed `gh` (which faithfully emulates `gh api --jq <expr>`
filtering, not just a raw dump), asserting on the actual
`gh api .../statuses/...` call it makes — closing a gap an RBG compliance
audit caught: this logic lived as inline YAML bash with no test coverage
at all.
"""
...
def _run(tmp_path: Path, *, reviews: list) -> dict:
...
def test_formal_review_approved_posts_success(tmp_path: Path):
    ...
    out = _run(tmp_path, reviews=reviews)
    assert out["state"] == "success"
    assert out["context"] == "qa-status"


def test_formal_review_changes_requested_posts_failure(tmp_path: Path):
    reviews = [{"commit_id": SHA, "state": "CHANGES_REQUESTED"}]
    out = _run(tmp_path, reviews=reviews)
    assert out["state"] == "failure"


def test_no_review_is_a_failure(tmp_path: Path):
