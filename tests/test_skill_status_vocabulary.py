...
(references/TAXONOMY.md), mirrored in tests/policy.toml under [aops.taxonomy].
...
def test_dump_skill_status_template_uses_canonical_statuses():
    """dump/SKILL.md enumerates terminal statuses as a bulleted list
    (`- \\`done\\`: ...`), not the `<status: a|b|c>` inline template other
    skills may use. Check whichever shape is present.
    """
    ...
    status_list_re = re.compile(r"^-\s+`([a-z_]+)`:", re.MULTILINE)

    template_matches = list(status_template_re.finditer(text))
    list_matches = status_list_re.findall(text)
    assert template_matches or list_matches, (
        f"No terminal-status enumeration (template or bulleted list) found in {dump_skill}"
    )
    ...
    invalid_listed = [s for s in list_matches if s not in VALID_STATUSES]
    assert not invalid_listed, (
        f"{dump_skill.relative_to(REPO_ROOT)} mandates non-canonical statuses "
        f"in its terminal-status list: {invalid_listed}. Valid statuses are: {sorted(VALID_STATUSES)}"
    )
...
def test_dump_skill_does_not_template_literal_blocked():
    """Assert dump skill does not offer 'blocked' as a writable release status,
    whether templated as `<status: a|b|c>` or enumerated as a bulleted list."""
    dump_skill = PLUGINS_DIR / "aops" / "skills" / "dump" / "SKILL.md"
    text = dump_skill.read_text(encoding="utf-8")
    status_template_re = re.compile(r"<status:\s*([^>]+)>", re.IGNORECASE)
    status_list_re = re.compile(r"^-\s+`([a-z_]+)`:", re.MULTILINE)
    ...
    assert "blocked" not in status_list_re.findall(text), (
        f"{dump_skill.relative_to(REPO_ROOT)} lists 'blocked' as a writable release status. "
        "'blocked' is derived and should not be offered as a writable status."
    )
