"""Regression test: junior agent tool list must be a superset of planner-required PKB tools.
...
# Tools required by the skills junior drives (from planner/remember/sleep/aops allowlists).
REQUIRED_TOOLS = [
...
def _parse_junior_tools() -> list[str]:
...
def test_junior_tool_list_parses():
    tools = _parse_junior_tools()
    assert isinstance(tools, list)
    assert len(tools) > 0


def test_junior_includes_planner_required_tools():
    tools = _parse_junior_tools()
    missing = [t for t in REQUIRED_TOOLS if t not in tools]
    assert not missing, f"junior is missing tools required by planner/skills: {missing}"
