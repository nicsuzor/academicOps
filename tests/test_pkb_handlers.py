"""Behavioral tests for plugins/aops-core/hooks/handlers.py.
...
PKB_HOOKS = REPO_ROOT / "plugins" / "aops-core" / "hooks"
RUN_MCP = REPO_ROOT / "plugins" / "aops-core" / "scripts" / "run-mcp.sh"
...
def _require_search_enabled():
    if not _policy.get("aops-core", _policy.get("pkb", {})).get("search_the_pkb_enabled", False):
...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "No missing-message guard in plugins/aops-core/hooks/handlers.py. "
...
# (plugins/aops-core/hooks/handlers.py) says otherwise: it "always takes the
