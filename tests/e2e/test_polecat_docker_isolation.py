...
def test_crew_spawns_docker_container_claude(temp_polecat_home, tmp_path):
    ...
    assert "not found" in output.lower() or "denied" in output.lower() or "tty" in output.lower(), "Should fail executing docker"
...
def test_crew_spawns_docker_container_gemini(temp_polecat_home, tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "polecat.cli", "--home", str(temp_polecat_home), "crew", "repo", str(repo), "-n", "gemini-worker", "-g"],
        env=env,
        capture_output=True,
        text=True,
        cwd=os.getcwd() + "/polecat",
    ...
    output = result.stdout + result.stderr
    assert "GEMINI_SANDBOX_IMAGE=aops-sandbox" in output, "Should set GEMINI_SANDBOX_IMAGE for gemini CLI"
