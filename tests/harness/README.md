...
## Dev-loop: live-editing plugin source against both CLIs

A separate, faster loop for iterating on academicOps plugin source (hooks,
lib, skills, commands, agents, scripts) and testing it against **both** the
`claude` and `antigravity` (`agy`) CLIs inside a polecat crew container,
without touching the production image or config real polecats use.

Three pieces:

- **`ghcr.io/nicsuzor/aops-crew:dev`** — built by `make build-docker-dev`
  (mirrors `make build-docker`'s `AOPS_DIST_SOURCE=local` build, but tags
  ONLY `:dev` — it never writes `:latest` or the bare
  `ghcr.io/nicsuzor/aops-crew` tag that real polecats pull).
- **The `aops-dev` polecat.yaml profile** — generated by
  `scripts/gen_dev_polecat_config.py` into `~/.polecat-dev/polecat.yaml` (a
  profile entirely separate from `$AOPS_SESSIONS/polecat.yaml`). It bind-
  mounts each plugin's EDITABLE source subdirs only (`hooks/`, `lib/`,
  `skills/`, `commands/`, `agents/`, `scripts/`, `.agents/`,
  `.claude-plugin/`) from this checkout's `dist/` straight into the
  container's installed plugin paths — for Claude, the version-pinned
  `~/.claude/plugins/cache/academicOps/<plugin>/<version>/` path (version
  read live from `.claude-plugin/marketplace.json`); for Antigravity, the
  unversioned `~/.gemini/antigravity-cli/plugins/<plugin>/` path. Generated
  artifacts (`.venv`, `.uv-cache`, `tests/`, `__pycache__`) are deliberately
  excluded so the container's own pre-built venv is never clobbered by a
  host-only interpreter path.
- **`scripts/dev-crew.sh`** — a thin tmux wrapper (`start`, `send`, `watch`,
  `logs`, `stop`) around the tmux pattern above, pre-wired to the `:dev`
  image + `aops-dev` profile.

Typical loop: edit source under `aops-core/hooks/` (etc.), `make build-dev`
to refresh `dist/` (fast, no Docker rebuild), then just re-run
`scripts/dev-crew.sh send <name> "..."` — the bind mount makes the edit
visible in the running container immediately, no container or CLI restart
required (only rebuild the image itself when you need to test image-level
changes, e.g. Dockerfile edits or system packages).

**This is a dev-loop only, NOT a fidelity/pre-ship check.** A full,
no-mount `make verify-docker` (or `make build-docker` for a normal local
build) remains the real gate before shipping — it proves the plugin
actually installs cleanly from a fresh `dist/`, which this mounted setup
deliberately bypasses.

Unrelated to `scripts/pty_hook_probe.py` (a separate, deliberately-frozen
conformance probe with its own no-mutate-live-install guarantee) — this
dev loop does not replace or modify it.
