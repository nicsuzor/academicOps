...
class TestInstructionTreeGeneration:
    ...
    def test_repository_scanner_finds_all_components(self, repo_root):
        ...
        assert "description" in dev_agent, (
            "Should extract description from agent frontmatter"
        )
        assert len(dev_agent["description"]) > 0, "Description should not be empty"
        assert "developer" in dev_agent["description"].lower(), (
            "Description should mention developer"
        )
        ...
        test_writing_skill = next(
            s for s in components["skills"] if s["name"] == "test-writing"
        )
        assert "description" in test_writing_skill, (
            "Should extract description from skill SKILL.md frontmatter"
        )
        assert len(test_writing_skill["description"]) > 0, (
            "Description should not be empty"
        )
        assert "test" in test_writing_skill["description"].lower(), (
        ...
        assert "description" in dev_command, (
            "Should extract description from command frontmatter"
        )
        assert len(dev_command["description"]) > 0, "Description should not be empty"
        assert (
            "development" in dev_command["description"].lower()
            or "dev" in dev_command["description"].lower()
        ), "Description should mention development"
        ...
        assert "load_instructions" in hook_names, (
            "Should find load_instructions.py hook"
        )
        ...
        load_hook = next(
            h for h in components["hooks"] if h["name"] == "load_instructions"
        )
        assert "description" in load_hook, (
            "Should extract description from hook docstring"
        )
        assert len(load_hook["description"]) > 0, "Description should not be empty"
        assert (
            "load" in load_hook["description"].lower()
            or "instruction" in load_hook["description"].lower()
        ), "Description should mention loading/instructions"

        # ASSERT - Verify core discovered
        assert "file" in components["core"], "Should have core file path"
        assert "references" in components["core"], (
            "Should parse @references from core/_CORE.md"
        )
    ...
    def test_markdown_generator_produces_readme_section(self, repo_root):
        ...
        assert "## Agents" in markdown or "### Agents" in markdown, (
            "Should have Agents section"
        )
        assert "## Skills" in markdown or "### Skills" in markdown, (
            "Should have Skills section"
        )
        assert "## Commands" in markdown or "### Commands" in markdown, (
            "Should have Commands section"
        )
        assert "## Hooks" in markdown or "### Hooks" in markdown, (
        ...
        assert "-" in markdown or "|" in markdown, (
            "Should use markdown list or table format"
        )
    ...
    def test_markdown_includes_component_descriptions(self, repo_root):
        ...
        dev_agent = next(
            (a for a in components["agents"] if a["name"] == "DEVELOPER"), None
        )
        ...
        assert dev_agent["description"] in markdown, (
            f"Should include description: {dev_agent['description']}"
        )
        assert "`agents/DEVELOPER.md`" in markdown, (
            "Should include agent path in backticks"
        )
        ...
        test_skill = next(
            (s for s in components["skills"] if s["name"] == "test-writing"), None
        )
        ...
        assert test_skill["description"] in markdown, (
            f"Should include description: {test_skill['description']}"
        )
        assert "`skills/test-writing`" in markdown, (
            "Should include skill path in backticks"
        )

        # ASSERT - Verify dev command has description in correct format
        # Expected format: - **/dev** - {description} (`commands/dev.md`)
        dev_command = next(
            (c for c in components["commands"] if c["name"] == "dev"), None
        )
        ...
        assert "**/dev**" in markdown or "**dev**" in markdown, (
            "Should include command name in bold"
        )
        assert dev_command["description"] in markdown, (
            f"Should include description: {dev_command['description']}"
        )
        assert "`commands/dev.md`" in markdown, (
            "Should include command path in backticks"
        )
        ...
        load_hook = next(
            (h for h in components["hooks"] if h["name"] == "load_instructions"), None
        )
        assert load_hook is not None, "load_instructions hook should exist for testing"
        assert load_hook["description"], (
            "load_instructions hook should have description"
        )

        assert "**load_instructions**" in markdown, "Should include hook name in bold"
        assert load_hook["description"] in markdown, (
            f"Should include description: {load_hook['description']}"
        )
        assert "`hooks/load_instructions.py`" in markdown, (
            "Should include hook path in backticks"
        )
    ...
    def test_readme_update_preserves_content_outside_markers(self, repo_root, tmp_path):
        ...
        from generate_instruction_tree import (
            generate_markdown_tree,
            scan_repository,
            update_readme_with_tree,
        )
...
"""
        ...
        assert "This is the introduction." in updated_content, (
            "Introduction should be preserved"
        )

        # ASSERT - Content after markers preserved
        assert "This is the conclusion." in updated_content, (
            "Conclusion should be preserved"
        )

        # ASSERT - Old content replaced with new tree
        assert "OLD CONTENT TO BE REPLACED" not in updated_content, (
            "Old content should be removed"
        )
        assert (
            "## Instruction Tree" in new_tree_content
            or "### Agents" in new_tree_content
        ), "New tree content should be present"

        # ASSERT - Markers still present
        assert "<!-- INSTRUCTION_TREE_START -->" in updated_content, (
            "Start marker should remain"
        )
        assert "<!-- INSTRUCTION_TREE_END -->" in updated_content, (
        ...
        assert (
            "Agents" in content_between_markers or "Skills" in content_between_markers
        ), "Generated tree should be between markers"

    def test_validation_script_detects_stale_instruction_tree(
...
""")
        ...
        assert is_current, (
            f"Validation should pass with current tree. Message: {message}"
        )
        assert message == "" or "current" in message.lower(), (
            "Success message should indicate tree is current"
        )
        ...
        assert not is_current, (
            f"Validation should fail with stale tree. Message: {message}"
        )
        assert (
            "stale" in message.lower()
            or "outdated" in message.lower()
            or "mismatch" in message.lower()
        ...
        is_current, message = validate_tree_is_current(test_repo)
        assert is_current, (
            f"Validation should pass after tree regeneration. Message: {message}"
        )
    ...
    def test_aops_trainer_skill_documents_instruction_tree_maintenance(self):
        ...
        assert (
            "instruction tree" in skill_content
            or "documentation maintenance" in skill_content
        ), "Skill should mention instruction tree or documentation maintenance"

        # ASSERT - Script references present
        assert (
            "generate_instruction_tree" in skill_content
            or "validate_instruction_tree" in skill_content
    ...
    def test_aops_trainer_documents_description_maintenance_and_architectural_analysis(
        self,
    ):
        """
        ...
        assert (
            "description maintenance" in skill_content_lower
            or "description" in skill_content_lower
        ), "Skill should document description maintenance"
        assert (
            "extract" in skill_content_lower and "description" in skill_content_lower
        ...
        assert (
            "architectural analysis" in skill_content_lower
            or "architecture" in skill_content_lower
        ), "Skill should document architectural analysis"
        assert "overlap" in skill_content_lower, (
            "Skill should mention overlap detection"
        )
        assert (
            "fragmentation" in skill_content_lower or "fragment" in skill_content_lower
        ), "Skill should mention fragmentation detection"

        # ASSERT - Reference to instruction tree script
        assert (
            "generate_instruction_tree" in skill_content_lower
            or "instruction tree" in skill_content_lower
        ...
        assert (
            "when to use" in skill_content_lower
            and "description" in skill_content_lower
        ) or (
            "instruction tree" in skill_content_lower
            and "maintenance" in skill_content_lower
        ), (
    ...
    def test_scanner_collects_skill_symlink_dependencies(self, repo_root):
        ...
        analyst_skill = next(
            (s for s in components["skills"] if s["name"] == "analyst"), None
        )
        assert analyst_skill is not None, "analyst skill should exist"
        assert "dependencies" in analyst_skill, (
            "analyst skill should have dependencies field"
        )

        dependencies = analyst_skill["dependencies"]
        assert len(dependencies) > 0, (
            "analyst skill should have at least one dependency"
        )
        ...
        git_commit_skill = next(
            (s for s in components["skills"] if s["name"] == "git-commit"), None
        )
        assert git_commit_skill is not None, "git-commit skill should exist"
        assert "dependencies" in git_commit_skill, (
            "git-commit skill should have dependencies field"
        )

        git_dependencies = git_commit_skill["dependencies"]
        assert len(git_dependencies) > 0, (
            "git-commit skill should have at least one dependency"
        )
        ...
        trainer_skill = next(
            (s for s in components["skills"] if s["name"] == "aops-trainer"), None
        )
        assert trainer_skill is not None, "aops-trainer skill should exist"
        assert "dependencies" in trainer_skill, (
            "aops-trainer skill should have dependencies field"
        )

        trainer_dependencies = trainer_skill["dependencies"]
        assert len(trainer_dependencies) > 0, (
            "aops-trainer skill should have at least one dependency"
        )
        ...
        assert "SKILL-PRIMER.md" in trainer_dep_basenames, (
            "Should find SKILL-PRIMER.md symlink"
        )
    ...
    def test_scanner_collects_command_load_instructions_dependencies(self, repo_root):
        ...
        assert "dependencies" in dev_command, (
            "dev command should have dependencies field"
        )
        ...
        assert "DEVELOPMENT.md" in dependencies, (
            "Should find DEVELOPMENT.md in load_instructions calls"
        )
        assert "TESTING.md" in dependencies, (
            "Should find TESTING.md in load_instructions calls"
        )
        assert "DEBUGGING.md" in dependencies, (
            "Should find DEBUGGING.md in load_instructions calls"
        )
        assert "STYLE.md" in dependencies, (
            "Should find STYLE.md in load_instructions calls"
        )
    ...
    def test_markdown_displays_component_dependencies(self, repo_root):
        ...
        analyst_section = markdown[
            markdown.index("**analyst**") : markdown.index("**analyst**") + 500
        ]
        assert "Dependencies:" in analyst_section or "Loads:" in analyst_section, (
            "Analyst skill should show dependencies"
        )
        assert (
            "MATPLOTLIB" in analyst_section or "matplotlib" in analyst_section.lower()
        ...
        git_commit_section = markdown[
            markdown.index("**git-commit**") : markdown.index("**git-commit**") + 500
        ]
        assert (
            "Dependencies:" in git_commit_section or "Loads:" in git_commit_section
        ), "git-commit skill should show dependencies"
        assert (
            "FAIL-FAST" in git_commit_section
            or "fail-fast" in git_commit_section.lower()
        ), "Should mention FAIL-FAST dependency"

        # ASSERT - /dev command shows its load_instructions.py dependencies
        assert "**/dev**" in markdown or "**dev**" in markdown, (
            "Should list dev command"
        )
        ...
        assert (
            "3-tier" in dev_section
            or "framework" in dev_section
            or "personal" in dev_section
            or "project" in dev_section
    ...
    def test_markdown_includes_reverse_dependency_index(self, repo_root):
        ...
        assert (
            "### Instruction Files" in markdown or "## Instruction Files" in markdown
        ...
        assert (
            "MATPLOTLIB.md" in files_section or "matplotlib" in files_section.lower()
        ), "Should list MATPLOTLIB.md instruction file"
        # Find MATPLOTLIB section and check for analyst
        if "MATPLOTLIB.md" in files_section:
            matplotlib_line = files_section[
                files_section.index("MATPLOTLIB.md") : files_section.index(
                    "MATPLOTLIB.md"
                )
                + 200
            ]
        ...
        assert (
            "FAIL-FAST.md" in files_section or "fail-fast" in files_section.lower()
        ), "Should list FAIL-FAST.md instruction file"
        if "FAIL-FAST.md" in files_section:
            failfast_line = files_section[
                files_section.index("FAIL-FAST.md") : files_section.index(
                    "FAIL-FAST.md"
                )
                + 200
            ]
        ...
        dev_line = files_section[
            files_section.index("DEVELOPMENT.md") : files_section.index(
                "DEVELOPMENT.md"
            )
            + 200
        ]
    ...
    def test_scanner_finds_all_instruction_files_including_orphans(self, repo_root):
        ...
        assert len(all_files) >= 21, (
            f"Should find at least 21 instruction files, found {len(all_files)}"
        )
    ...
    def test_compact_format_truncates_descriptions_to_7_words(self, repo_root):
        ...
        assert len(parts) == 2, (
            f"Expected format '- NAME - description', got: {dev_line}"
        )
        description_part = parts[1].strip()
        word_count = len(description_part.split())
        assert word_count <= 10, (
            f"Description should be <=10 words, got {word_count}: {description_part}"
        )
    ...
    def test_compact_format_includes_instruction_flow_tree(self, repo_root):
        ...
        assert (
            "### Instruction Flow" in markdown or "## Instruction Flow" in markdown
        ...
        assert (
            "SessionStart" in flow_section or "session start" in flow_section.lower()
    ...
    def test_compact_format_includes_quick_stats_with_orphan_detection(self, repo_root):
        ...
        assert "instruction" in stats_section.lower(), (
            "Should show instruction file count"
        )
        ...
        if "orphan" in stats_section.lower():
            assert (
                "HYDRA" in stats_section
                or "DBT" in stats_section
                or "E2E-TESTING" in stats_section
        ...
        if "overlap" in stats_section.lower() or "⚠️ OVERLAP" in stats_section:
            ...
            assert (
                "scribe" in stats_section.lower()
                or "task-manager" in stats_section.lower()
                or "task" in stats_section.lower()
