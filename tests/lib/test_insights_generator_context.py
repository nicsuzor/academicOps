...
class TestExtractRecentContext(unittest.TestCase):
    ...
    def test_extract_by_session_id(self, mock_extract, mock_find):
        ...
        mock_session = SessionInfo(
            path=mock_path, project="test-project", session_id=session_id, last_modified=None
        )
    ...
    def test_extract_by_short_hash(self, mock_extract, mock_find):
        ...
        mock_session = SessionInfo(
            path=mock_path, project="test-project", session_id=short_hash, last_modified=None
        )
        mock_find.return_value = [mock_session]
        ...
        # Verify
        self.assertEqual(result, "[User]: Request")
        mock_find.assert_called_once()
        mock_extract.assert_called_once_with(mock_path, include={"conversation"}, max_turns=20)
    ...
    def test_extract_by_prefix(self, mock_extract, mock_find):
        ...
        mock_session = SessionInfo(
            path=mock_path, project="test-project", session_id=full_id, last_modified=None
        )
        mock_find.return_value = [mock_session]
        ...
        # Verify
        self.assertEqual(result, "[User]: Prefix match")
        mock_find.assert_called_once()
        mock_extract.assert_called_once_with(mock_path, include={"conversation"}, max_turns=20)
    ...
    def test_extract_by_direct_path(self, mock_is_file, mock_exists, mock_extract):
        ...
        # Verify
        self.assertEqual(result, "[User]: Direct path")
    ...
    def test_empty_conversation(self, mock_extract, mock_find):
        ...
        mock_session = SessionInfo(
            path=Path("/tmp/empty.jsonl"), project="test", session_id=session_id, last_modified=None
        )
        mock_find.return_value = [mock_session]
