...
- **Terse follow-ups after task completion**: For short/ambiguous prompts (< 15 words), check session context FIRST. If a task was just completed (`/pull` visible in recent prompts), assume the follow-up relates to that work or its parent goal. Don't TRIAGE as "vague" when context is available.
