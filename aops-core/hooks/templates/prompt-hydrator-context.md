...
- **Short confirmations (highest priority)**: If the prompt is extremely short (≤ 3 words: "yes", "ok", "do it", "sure"), treat it as a confirmation of the MOST RECENT agent response and tools, NOT a new request from the task queue.
- **Other terse follow-ups after task completion**: For other short/ambiguous prompts (4–15 words), check session context FIRST. If a task was just completed (`/pull` visible in recent prompts), assume the follow-up relates to that work or its parent goal. Don't TRIAGE as "vague" when relevant context is available.
