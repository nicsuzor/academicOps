...
### 5. Shiny Shitty NLP — Programmatic LLM Wrappers (P#49)

Detect if the agent is proposing to build code that wraps LLM API calls instead of designing work for agentic execution.

- **Signal**: Agent proposes a Python script that imports `anthropic`, `openai`, or similar SDK to call an LLM programmatically.
- **Signal**: Agent designs an "evaluation harness", "assessment script", or "scoring pipeline" that makes API calls to an LLM model within a deterministic script.
- **Signal**: Agent proposes building a new tool/script to "use an LLM" for judgment, classification, or evaluation — when the agent ITSELF is an LLM that can do this directly via a skill or workflow.
- **Signal**: Phrases like "we could call the API to...", "build a script that uses Claude/GPT to...", "programmatic LLM evaluation".
- **Exception**: Legitimate uses include: the `claude-api` skill (teaching users to build apps with Claude API), MCP server tooling, and the `session-insights` skill (which delegates to Gemini for cost-effective batch processing with explicit user approval per P#50).
- **Root cause**: Agents' training data predates mature agentic frameworks. They default to "write code that calls an API" when the correct answer is "design a skill/workflow that an agent executes." This framework runs on Claude Code, Gemini CLI, Jules, and GitHub agents — these agents ARE the LLM. Smarts should be agentic; code should be minimised.

### 6. Aimless Exploration (P#58)
