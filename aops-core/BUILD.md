...
| --------------------------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| `aops-core/skills/`, `agents/`, `commands/`, `lib/` | `dist/aops-claude/<same>/`                    | `dist/aops-cowork/<same>/` (aops-core base, cowork-only blocks kept)                          | `dist/aops-gemini/<same>/` (skills mostly)     |
| `aops-cowork/` (real package)                       | (not used)                                    | `dist/aops-cowork/<same>/` — overlaid ON TOP of the aops-core base (e.g. `cowork-sync` skill) | (not used)                                     |
...
| `aops-cowork/pyproject.toml` (tracked)              | (not used)                                    | `dist/aops-cowork/pyproject.toml` (version stamped; lib-only)                                 | (not used)                                     |
...
`aops-cowork` is a **real, tracked package** (`aops-cowork/`) that the build COMPOSES with the `aops-core/` source tree — it is not a manifest fabricated from `templates/`. The build copies the aops-core base into `dist/aops-cowork/` and then overlays the `aops-cowork/` package's own files on top. It is Claude-shaped (same `.claude-plugin/plugin.json` + `.mcp.json` layout) but ships **four** cowork-specific differences:
...
2. **`cowork-sync` skill included.** The skill at `aops-cowork/skills/cowork-sync/SKILL.md` lives ONLY in the cowork package and is overlaid onto the build. It describes the PKB → native task list mirror that the Cowork harness depends on; because it is not in the aops-core base, the other surfaces never see it (no strip step needed).
...
4. **No hooks bundled.** The cowork build ships **no `hooks/` directory and no `hooks.json`** — the `if platform != "cowork"` guard in section 2 of `build_aops_core` skips the hook copy, and the tracked `aops-cowork/pyproject.toml` declares `packages = ["lib"]` (no `hooks`) so `uv sync --frozen` doesn't reference a missing dir. Installing `aops-core` into Cowork from the `nicsuzor/aops` main `dist` marketplace makes Cowork fire the standard aops-core hook stack (empirically confirmed — see `mem-fe29111a` / task `aops-04075740`), so one shared hook stack serves both surfaces. Bundling hooks here too would register the router a second time and double-fire every lifecycle hook. aops-cowork is therefore an **additive, skills-only** layer on top of aops-core.
...
- **A whole skill that only makes sense on Cowork** → drop it under `aops-cowork/skills/<name>/`. The cowork build overlays everything in the package's `skills/` automatically, and the other surfaces never see it (it isn't in the aops-core base) — no exclusion needed.
