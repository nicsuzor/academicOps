...
| --------------------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| `aops-core/skills/`, `agents/`, `commands/`, `lib/` | `dist/aops-claude/<same>/`                    | **NOT copied** — except individual files carrying `<!-- cowork:only -->` markers (currently `aops-core/skills/end_session/SKILL.md` and `aops-pkb/commands/pull.md`, scanned across both plugin trees), copied singly with markers stripped/content kept | `dist/aops-gemini/<same>/` (skills mostly)     |
...
3. **Cowork-only marker files, and ONLY those files.** Source files (currently `aops-core/skills/end_session/SKILL.md` and `aops-pkb/commands/pull.md` — the latter inherited from aops-core in the aops-b225ec53 extraction) wrap a short Cowork-specific paragraph in `<!-- cowork:only --> ... <!-- /cowork:only -->`. The cowork build scans both the `aops-core/` tree AND the `aops-pkb/` tree (aops-pkb has no cowork build of its own, so this is the only place its marker content ships) for `.md` files containing a real (regex-matched, not just string-contains) marker block and copies ONLY those files — nothing else from either tree is copied. The copied files get the markers stripped and the content kept; every other build of aops-core (claude/gemini/antigravity) gets the full aops-core tree with both the markers AND the content stripped, and aops-pkb's own (claude-only) build strips the block the same way. The marker handling lives in `_process_cowork_markers` / `_COWORK_BLOCK_RE` in `scripts/build.py`.
