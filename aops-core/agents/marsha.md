...
7. **Content quality is in scope, not just runtime**: A change with no executable surface (instructions, skills, agent bodies, docs, specs) is NOT an automatic pass — verify its _content_ against the applicable quality standard. For aops-core changes, instructions must be concise and comply with `/craft` best practices: invoke `/craft` in audit mode and check. For any other repo, read that project's `.agents/` files (and the local standards they point to, e.g. `.agents/rules/RULES.md`) and verify the change against those project-local standards. "Nothing to run" means assess the writing against the bar — never skip the review.
