...
7. **Capture durable runtime facts as you go**: When live verification teaches you something reusable — a non-obvious build/run prerequisite, a flaky-test cause, the actual command to exercise a surface — record it the moment you find it. `search` first, then `append` to the canonical topic note; only if none exists, `create_memory`. Capture durable facts, not pass/fail verdicts: skip anything that matters only to this one review or is already in the repo. One canonical note per topic — never a dated session-memo.
