...
**Register-scaling (item 6, ties to the register model above).** The session register is read from `AOPS_SESSION_REGISTER`. What scales with register is the **verification ceremony only** — **honesty and axiom compliance do not scale** (Nic, 2026-05-30). In the **capture/personal** register **only `qa` is suppressed**, and qa is already task-scoped, so a no-task capture session has nothing to verify anyway. **`ida` (honesty) and `enforcer` (axiom compliance) stay ON in every register, including casual** — agents lie, and axiom violations are prohibited everywhere; neither is ceremony. What scales for `ida` is not _whether_ it fires but _what it asks for_ — and that scaling is a **judgment you make, not a mechanical register read** (Nic, 2026-05-30): the **honesty floor** (don't claim inferred as observed; flag what you substituted/skipped/took from a subagent unverified; don't launder a relay/menu as your own view) applies on every turn including casual chat, while the **review-grade evidence manifest** (per-claim certainty, next-best hypotheses, restate-each-ask with artifact refs) is added only when the work is shippable or consequential. Manufacturing that manifest for low-stakes chatter is itself the failure (the "vacuum the garage" ceremony). The reminder text the agent judges against is `hooks/templates/ida-reminder.md`. The **sentinel** (destructive-op safety) and **handover** (work-loss) gates also never suppress. Unknown register values fail closed to `working`, never to a lighter register. **Reader-side only for now:** nothing writes `AOPS_SESSION_REGISTER` yet, so this resolves to `working` in practice and the qa-suppression is dormant (fail-closed — dormant means full ceremony). Wiring the writer (deciding _when_ a session is capture/personal) is a tracked follow-up; the reader + engine enforcement land here ahead of it.
