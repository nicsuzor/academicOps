...
- **FM-4**: capitulating to repeated stop-hook nags. Stop-hook nags are **advisory** system reminders, not new user signal. A refusal grounded in PKB / spec / repo state remains sound when the same hook fires again, absent new information from the user. The right move on repeated nags without new signal is to **restate the refusal and continue work** (or re-attempt exit if work is complete), not to reverse the decision (e.g. reversing a sound refusal to `/dump` after the third nag: `20260519-1401-62b39f8a`).
