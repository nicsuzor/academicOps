...
**Who writes the register (the activation half).** The register is an **explicit launch-time stakes declaration, never inferred mid-session.** The hooks read `AOPS_SESSION_REGISTER` from `os.environ`, and the deliberate posture design is env-as-SSoT (no session-state posture file), so the var can only be set at process launch — a mid-session skill cannot relight it. The polecat launcher is the writer: it stamps `AOPS_SESSION_REGISTER` into the container env from the resolved session config's `register` field (`polecat_config.SessionDefaults.register`, set in `polecat.yaml` or per-run via `--set register=capture`), forwarded by the `AOPS_` prefix rule. When `register` is unset the launcher does **not** stamp the var (and pops any inherited one), so the reader fail-closes to `working`. This is why a heuristic SessionStart "guess" is deliberately **not** used: a wrong guess would fail _open_ onto lighter ceremony, which the whole reader design refuses. Host-side interactive `claude --agent junior` sessions set the register the same way — from the launching shell's env (WS3 launch-context, the `sessions` repo) — not from this repo.
...
**Register-scaling.** Register read from `AOPS_SESSION_REGISTER`. In capture/personal register, enforcer/ida/qa gates suppressed; sentinel and handover are not. Written at launch by the polecat launcher from `polecat_config.SessionDefaults.register` (`polecat.yaml` or `--set register=capture`); unset ⇒ launcher doesn't stamp it ⇒ reader fail-closes to `working`. Never inferred mid-session (a wrong guess would fail open onto lighter ceremony).
