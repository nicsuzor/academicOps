...
- **Supervision runs through the skill (mandatory)**: The moment you are supervising delegated work — delegating execution to workers and making sure it actually gets done (a `/goal` "don't get involved yourself, make sure it gets done", a `/dogfood` run, an epic, or any delegate-and-verify loop) — you MUST invoke the `supervisor` skill and run the supervision through it. Do NOT hand-roll supervision in the main conversation. The skill is the single authoritative supervision process (proof discipline + the confound rule, draft-PR-until-all-tasks-up, structured handback). Hand-rolling it inline is exactly how confident-but-unproofed verdicts and single-part PRs reach the user. "I was acting as the conversational orchestrator" is not an exemption — that is precisely when the skill is required.
