...
### Python Testing Detection (MANDATORY)

**Trigger patterns** (case-insensitive):

- Task involves writing, modifying, or fixing Python tests
- Task body mentions "test", "pytest", "E2E", "unit test", "test suite"
- Task targets files matching `test_*.py` or `*_test.py`
- Task involves "real data", "captured data", "DEBUG_HOOKS", "fixtures"

**When detected**:

1. **MUST invoke `/python-dev`** — the testing philosophy in `python-dev/references/testing.md` is critical context
2. **MUST inject these rules into the execution plan**:
   - Use real captured data, not fabricated fixtures (H33)
   - Pass raw data through the full pipeline (e.g. `normalize_input()` → `_dispatch_gates()`) — never construct intermediate objects by hand
