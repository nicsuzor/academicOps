...
description: Default end-of-session flow - commit, push, file a PR, call release_task with structured session fields, emit a terse 5-10 line handover block, halt. No Framework Reflection prose.
triggers:
  - "end session"
  - "session end"
  - "close session"
...
domain:
  - operations
allowed-tools: Bash, mcp__pkb__release_task, mcp__pkb__create_task, mcp__pkb__update_task, Read
owner: pauli
permalink: skills/end_session
---

# /end_session — Default session close
...
This is the **default** end-of-session path. Use [[dump]] for special-case handovers (emergency, cross-machine / cross-environment / cross-project transfer, interrupted session needing full context capture). Use [[recap]] mid-session for a read-only view of what's been done so far.

## Contract

Per [[session-handover-contract]]:

- Terminal output is a terse 5–10 line markdown block — no Framework Reflection prose.
...
- Does **not** emit a `## Framework Reflection` heading or any `**Field**:` prose block. That format is retired.
...
## Boundary — end_session vs /dump vs /recap

| Skill          | When                                             | Output                                             |
| -------------- | ------------------------------------------------ | -------------------------------------------------- |
| `end_session`  | Default close — normal end-of-session            | Terse handover block + structured YAML             |
| `/dump`        | Emergency / cross-machine / interrupted transfer | Extended capture: env, machine, longer summary     |
| `/recap`       | Mid-session read-only                            | List of tasks touched in this `$AOPS_SESSION_ID`   |

Paste this same block verbatim into `/dump` and `/recap` when T8 rewrites them.
