...
| **Triage**     | Detect under-specified tasks needing decomposition  |
...
Detect under-specified tasks that lack actionable content. Uses `scripts/triage_tasks.py`.

```bash
uv run python $AOPS/aops-core/skills/garden/scripts/triage_tasks.py <path> [--recursive] [--min-body 100] [--format table|json]
```

**Classification:**

- **needs-decomposition**: title is reasonable but body has no actionable content (no AC, no instructions, no context). These should be fleshed out or sent back for proper decomposition.
- **needs-deletion**: title is vague (single word, <15 chars) AND body is empty. Opaque to anyone picking it up.
- **ok**: either has sufficient body (>100 chars of meaningful content) or title is self-explanatory ("X should Y", "Fix/Add/Implement + specific object").

Skips completed/cancelled tasks and container types (project, goal, hub, epic). Handles escaped `\n` in YAML scalars to avoid false positives.

Also run as part of the sleep cycle (Phase 4: Staleness Sweep).
