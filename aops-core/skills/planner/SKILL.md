...
- **Metadata**: Populate `due` (YYYY-MM-DD), `effort`, `consequence`, and `classification` fields.
  - `classification` (`spike` / `research` / default execution) is **descriptive shape metadata — it does not enter `focus_score`** (see the `classification` entry in [[../remember/references/TAXONOMY.md]]). The live `voi_value` term is computed from graph structure alone (leaf status, dependency resolution, and `contributes_to` target uncertainty × edge weight × downstream weight — [[multi-parent]] §VoI). Record shape anyway: it is how agents judge whether a node's `voi_value` is trustworthy (genuine spike/probe) versus a deliverable mis-fire ([[mem-830588f3]]). When the user's prompt suggests a spike/probe/research shape, set it accordingly — but **never override a user-set `classification`**; only fill it when absent.
