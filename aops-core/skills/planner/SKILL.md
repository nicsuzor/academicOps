...
**No pessimistic closure from missing information**: Embracing uncertainty (affordable-loss, pilot-in-the-plane) cuts **both** ways — it must not collapse into assuming the _worst_. **Absence of recorded evidence is not a negative result**: a goal is not "missed," an objective not "failed," a contribution did not "fail to land" merely because the record (transcript, log, doc) does not capture it. When a plan turns on a real-world event, intent, or outcome you cannot directly observe, treat the unknown as an **unknown-unknown to surface and resolve** — solicit ground truth from the principal/primary source via `AskUserQuestion` — **before** concluding failure. Default to "unverifiable — needs primary-source confirmation," never a confident negative. Apply the discount symmetrically: do not assume the optimistic reading either. (Source incident: #1891 — a review hardened a transcript-derived inference into a confident negative, later reversed by the human primary source.)
