...
- **Retire the superseded original (`superseded_by`)**: When the decomposition _carves an existing task's work into the new subtasks_ (or into siblings, or otherwise replaces it), the original must leave the dispatchable set in the **same** operation — do not leave it `queued`/`ready` with a now-stale body. Stamp `superseded_by: [<new-ids>]` on the original via `mcp__pkb__update_task`; this retires it (the PKB closes it) and leaves the redirect readable **on the task itself**, not only in this epic's Log. This is the #1584 fix: a redirect recorded only as prose in a parent's Log is invisible to `/pull`, which then dispatches the fossil. Do NOT rewrite the original's stale body — supersession is about dispatchability, not body content. See [[../remember/references/TAXONOMY.md#supersession-and-retirement-superseded_by]].
