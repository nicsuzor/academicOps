...
5. **Extract structured metadata** if mentioned in description or conversation:
   ...
   - `classification`: now **load-bearing for VoI computation** (see the `classification` entry in [[../remember/references/TAXONOMY.md]] and the spec [[mem-cf396d84]]). If the user's prompt suggests a spike/probe/research shape, the planner SHOULD set `classification` accordingly (`spike`, `research`, etc.) so the leaf earns its `voi_value` term. **Never override a user-set `classification`** — only fill it in when absent.
