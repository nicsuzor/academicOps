...
- **Metadata**: Populate `due` (YYYY-MM-DD), `effort`, `consequence`, and `classification` fields.
  - `classification` (`spike` / `research` / default execution) is **load-bearing for the live `voi_value` term** in `focus_score` (see the `classification` entry in [[../remember/references/TAXONOMY.md]] and [[multi-parent]] §2.2): a spike/probe-shaped leaf only earns its VoI term when its shape is recorded. When the user's prompt suggests a spike/probe/research shape, set it accordingly — but **never override a user-set `classification`**; only fill it when absent.
...
- **Prioritization**: Rank tasks strictly using the composite `focus_score` signal — the canonical additive composition of priority, severity, deadline pressure, age/staleness, downstream weight, stakeholder waiting, the `urgency` term, and the live **`voi_value`** term. See [[multi-parent]] §2.2 (composition) / §7. `voi_value` (live since 2026-06-01, capped at 5000) is the third generation of the ranking heuristic — successor to `downstream_weight × criticality` and the interim urgency-only ranking — and rewards spike/probe-shaped leaves with uncertain but important downstream. Component fields (`urgency`, `downstream_weight`, `voi_value`, …) stay visible for filter/debug but are never the primary sort.
...
- **Set `classification` per subtask**: Match each subtask's shape — `spike` for spikes, `research` for research, omit (treated as execution) for execution tasks. This feeds the live `voi_value` term so spike/probe-shaped leaves with uncertain but important downstream rank correctly (see [[multi-parent]] §2.2). Never override a user-set `classification`.
