...
- **Use concrete details from user prompts, not abstract labels.** "Debugged PKB search for blood test hydration tips" tells a story. "Blood test PKB lookup" is a label. The `description` field from `user_prompt` timeline events contains the ground truth — use it.
