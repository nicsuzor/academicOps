...
> **Note (taxonomy)**: Daily planning legitimately surfaces `ready` (decomposed-but-not-yet-queued) tasks because the whole point of the daily review is to help the user decide what to promote from `ready` → `queued` for agent dispatch. See [[../../../remember/references/TAXONOMY.md]] §Status Values. Agent dispatch (`/pull`) pulls from `queued`, NOT `ready`.
