...
After the deadline list, emit a short factual block of the top 5 tasks ranked by composite `urgency` (severity × edge weight × slack × decay) — restricted to `status` in {`queued`, `ready`, `in_progress`}. Use `mcp__pkb__list_tasks(format="json")` and read each task's `urgency` field; sort descending. List one per line:

```
High-urgency:
- [task-id] [[Title]] — urgency 0.83 (SEV<n>, due YYYY-MM-DD or no deadline)
```
...
```
⚠ SEV4 concurrency cap exceeded: N active (cap = 2). Severity inflation suspected — review or downgrade before adding more.
```
