...
- **→ Task**: Create a task via `mcp__pkb__create_task()` with content from the capture. Ask user for project/priority if not obvious. **MANDATORY**: Link to hierarchy with `parent`.
