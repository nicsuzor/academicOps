...
- **High-focus surface**: Top 5–10 active tasks (queued/ready/in_progress) ranked by composite `focus_score`. Split into **Target-propagated urgency (SEV3+)** and **Other high-focus work** buckets (threshold: `urgency >= 100`). Includes inline badges `[→target]` for tasks contributing to high-severity targets. Factual surfacing of what the graph computes — not a recommendation. Omitted when the PKB does not yet emit `focus_score`. See [[instructions/status-snapshot]] §3.3a.
