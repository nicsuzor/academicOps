...
**Then measure the knowledge layer separately.** `graph_stats.orphan_count` is **actionable-only** — it excludes `note`/`knowledge`/`memory` nodes entirely, so a healthy-looking `orphan_count` (e.g. 22) can hide ~1,200 disconnected knowledge nodes. Run:
...
> _Not verified (detector source not read):_ _why_ this holds. The `mem` detector source is a remote server unreachable from the polecat worktree, so it is not established whether the detector counts only `parent`/`contributes_to` frontmatter edges, or honours wikilinks but treats a link into another orphan as still-disconnected, or excludes wikilinks by design (a wikilink is not a parent claim). Do not assert the mechanism as fact downstream.
...
> _Disposition:_ the in-loop fix is **curation** (link / reparent / MOC / merge / archive — below), which reduces the count regardless of detector mechanism. Any detector refinement (honour wikilinks / Relations sections) is tracked separately under the mem/PKB-health epic and must not block knowledge-layer curation.
...
- **Knowledge-layer orphan detection**: `mcp__pkb__pkb_orphans(types=["note","knowledge","memory"], include_all=true)`. This is a **separate population** the default call never returns — ~1,200 nodes vs. ~22 actionable. Feed it to Phase 9's knowledge-layer curation strategy. Do not let an all-green actionable `orphan_count` stand in for knowledge-layer health; they are measured by different calls.
...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **link/reparent** | Active knowledge whose parent concept clearly exists in the graph (a canonical topic note, MOC, or epic).                                                          | Add the structured edge to that parent (`batch_reparent`/`batch_update`, `dry_run=false`). A prose `[[wikilink]]` alone does **not** clear orphan status (see Phase 0 note) — write the parentage edge the detector actually reads. |
...
| **archive**       | Legacy import or superseded episodic record with no live consumer — e.g. `career-planning-*` / OneNote imports from 2016–19, `status: done`, no current relevance. | `batch_archive` with reason. Per P#123 **age alone is not the trigger** — archive only when irrelevance is established by content, not date. Reversible (archive ≠ delete).                                                         |
...
<knowledge_orphan_count start → end (delta); dispositions: X linked/reparented, Y MOC'd, Z merged, W archived, S surfaced-for-review (list IDs of surfaced + any ambiguous). "skipped — under trigger" or "converged — count stable 2 cycles" when not run.>
