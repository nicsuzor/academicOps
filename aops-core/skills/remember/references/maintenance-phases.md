...
**Then measure the knowledge layer separately.** `graph_stats.orphan_count` is **actionable-only** — it excludes `note`/`knowledge`/`memory` nodes entirely, so a healthy-looking `orphan_count` (e.g. 22) can hide ~1,200 disconnected knowledge nodes (measured 2026-06-10, pre-mem#425 detector fix — the live number is whatever the call below returns, never this prose). Run:
...
> **Detector behaviour — what is observed vs. unverified (2026-06-10).**
> _Observed (decisive, from live `pkb_orphans` output):_ a memory carrying prose wikilinks is still flagged as an orphan. `mem-055ec916` links `[[mem-ac562ec9]]` twice in its body, yet both it **and** its link target appear in the knowledge-orphan list. So **a prose wikilink — inbound or outbound — does not by itself remove orphan status.** Curation must therefore not assume "has a wikilink ⇒ not an orphan."
> _Since verified (review pass, 2026-06-10):_ the polecat worktree could not reach the `mem` detector source, but the review session read it directly — `mem/src/graph_store.rs` `orphans()` (main@2737824, pre-#425) filters **solely** on the `parent` field (absent or dangling ⇒ orphan); wikilink-derived `Link` edges are never consulted. Mechanism confirmed by observation, not inference.
> _Unmeasured:_ what fraction of the ~1,200 orphans bear wikilinks at all. Sampled legacy imports (`career-planning-*`, OneNote imports) had none, but the population was not enumerated, so the claim "honouring wikilinks would not materially move the count" is a hypothesis, not a measurement.
> _Disposition:_ the in-loop fix is **curation** (link / reparent / MOC / merge / archive — below), which reduces the count regardless of detector mechanism. The detector refinement is in flight: **mem PR #425** ([[mem-272609ea]]) makes `orphans()` type-aware so deliberate wikilink-derived `Link` edges clear orphan status for knowledge types (consistent with the verified parent-only mechanism above). Once it lands, expect `knowledge_orphan_count` to drop substantially without any curation — that drop is detector correction, not curation progress; do not claim it in the cycle delta. The `> 50` trigger and the triage below remain valid for the true-orphan residual. Detector work must not block knowledge-layer curation.
...
- **Knowledge-layer orphan detection**: `mcp__pkb__pkb_orphans(types=["note","knowledge","memory"], include_all=true)`. This is a **separate population** the default call never returns — ~1,200 nodes vs. ~22 actionable at 2026-06-10 measurement (expect far fewer post-mem#425). Feed it to Phase 9's knowledge-layer curation strategy. Do not let an all-green actionable `orphan_count` stand in for knowledge-layer health; they are measured by different calls.
...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **link/reparent** | Active knowledge whose parent concept clearly exists in the graph (a canonical topic note, MOC, or epic).                                                          | Add the structured edge to that parent (`batch_reparent`/`batch_update`, `dry_run=false`). A structured parent edge clears orphan status under every detector version. As of 2026-06-10 a prose `[[wikilink]]` alone did **not** (see Phase 0 note); once mem#425 lands, a deliberate wikilink to a connected node also clears it for knowledge types — for memories (which have no `parent` param at capture), a body wikilink to a live hub is then the preferred, lighter-weight connection. Verify which detector behaviour is live before assuming. |
...
<baseline metrics_hash, key actionable counts; knowledge_orphan_count (this cycle) vs. previous cycle — render the delta, e.g. "knowledge_orphan_count: 1187 (was 1203, −16)">
