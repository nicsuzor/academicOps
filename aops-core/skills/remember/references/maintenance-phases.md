...
> **If the mining goal is to extract user prompts or command invocations** (e.g. what `/learn` was called with, prompt frequency, skill usage patterns), use the **structured summaries corpus** instead of reading transcript markdown files. Path: `$AOPS_SESSIONS/summaries/YYYY-MM/*.json`, field `timeline_events[type="user_prompt"]`. Apply interactive-client filter (`client in ("claude-code","claude-desktop")`). See `specs/CAPABILITIES.md §Session Summaries` for full reference. Raw transcript files are the fallback for content not present in summaries (agent reasoning, tool calls, full context).
