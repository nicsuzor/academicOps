...
For every task currently in `merge_ready` or `review` (cap 50 per cycle, oldest-modified first). **`merge_ready` and `review` are not the same parked state** — see [[taxonomy]] "`merge_ready` vs `review`": `merge_ready` is parked on a PR and may auto-close on merge; `review` is parked on a _human decision_ and is **never auto-closed by this backstop** — the most it does to a `review` task is surface it. Treat the two sets accordingly:
