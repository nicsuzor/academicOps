...
- `mcp__plugin_aops-core_pkb__list_tasks` / `mcp__plugin_aops-core_pkb__task_search` — find actionable tasks (any non-terminal status: `inbox`, `ready`, `queued`, `in_progress`, `merge_ready`, `review`, `blocked`, `paused`). Use `project=<project-id>` with `list_tasks` to scope to a single project; never infer project membership from ID prefixes or by walking parent chains.
