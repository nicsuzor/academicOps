...
- **Deployed to**: `~/.claude.json` mcpServers (merged via `scripts/install.py`)
- **Sync command**: `make install-dev` or `uv run python scripts/install.py` (run after config changes)
...
- Run `uv run python $AOPS/scripts/install.py` to sync from authoritative source
