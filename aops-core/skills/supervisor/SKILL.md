...
The two subagent invocations are independent contexts. The dispatch-agent does not inherit the compose-agent's reasoning trace; it reads the brief fresh from PKB and forms its own judgment. This is the cure A17 demands; the cost is one extra subagent call, not a tick exit and re-entry.
...
_Enforces A7 Edge 2 (FM-1, FM-2, FM-6). See `.agents/rules/AXIOMS.md` § A7._
