...
description: >
  ...
  by `/loop`; cross-tick state lives in the task body. Any orchestrator MUST
  invoke this skill for supervision; never hand-roll it inline.
triggers:
  - "supervise"
  - "supervisor"
  - "shepherd"
...
A delegate-and-verify run that ends with a tail of not-yet-closed items must be
