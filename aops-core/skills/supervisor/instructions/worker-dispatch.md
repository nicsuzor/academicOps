...
**Worker-Type Capability Check (mandatory).** Before dispatching, verify whether the target worker type carries the capabilities the brief requires — PKB-dependent work must only be briefed by task-ID to PKB-capable worker types; non-PKB worker types (e.g. `general-purpose`, `Jules`, `GHA` runners) must receive the brief inline or piped. See the **Worker-Type ↔ Required-Capability Dispatch Guard** in [[../../aops/references/authoring-discipline#3-compose-then-dispatch-separation]].
