...
**Row 3 mechanic — auto-resolve missing `project:` from unambiguous ancestor** — if the task's `project:` frontmatter is missing, pauli MUST attempt one resolution pass before halting. Read `target_ancestors` from the task (already returned by `mcp__pkb__get_task`). For each ancestor ID, call `mcp__pkb__get_task` and collect the value of its `project:` frontmatter (or its `node_type=project` ID, if applicable). If **exactly one** distinct project value is found across all ancestors, pauli sets it on the dispatch target via `mcp__pkb__update_task(id=<task-id>, updates={"project": "<resolved>"})` and proceeds to row 4. If **zero** project-bearing ancestors are found, or **two or more** distinct project values appear, halt at row 3 with `project ambiguous from ancestors: <values>` (or `project unresolvable: no ancestor carries project`) — do NOT pick. The resolution pass is one PKB read per ancestor (typically ≤ 3) and is cheaper than a ~30s polecat worktree refusal. Friction source: dogfood 2026-05-13 (F9) — `aops-88447bbb`. Pairs with the upstream PKB lint (`mem-69a2394b`) that prevents `status=queued` without `project:` in the first place — supervisor-side resolution is the runtime safety net, not the primary fix.
