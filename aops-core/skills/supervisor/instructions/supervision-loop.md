...
- `$AOPS_SESSIONS/polecats/<task-id>/` — markdown transcripts
- `$POLECAT_HOME/transcripts/<task-id>.jsonl` — raw JSONL

Convert raw JSONL if needed:
```bash
uv run python aops-core/scripts/transcript.py $POLECAT_HOME/transcripts/<task-id>.jsonl
```
