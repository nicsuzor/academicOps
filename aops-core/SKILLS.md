...
| `/hypervisor` | "batch tasks", "parallel processing", "process queue in parallel", "polecat swarm", "polecat herd", "spawn polecat", "run polecats" | Batch parallel task processing with atomic locking. Spawns multiple worker agents that pull from shared queue without duplication. |
