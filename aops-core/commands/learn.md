...
Follow the retro mode workflow exactly: select transcript → read in full → identify structural and systemic findings (don't just list symptoms) → look for patterns across findings → for each finding produce facts + Root Cause Category + structural shape/rule context + an impact statement → file forensic issues (factual structural articulation is fine, but no remediation proposals) → stamp transcript → framework reflection.
