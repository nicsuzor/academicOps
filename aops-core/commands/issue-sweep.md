...
description: Triage one batch of open GitHub issues — classify, present, gate on user, queue confirmed fix-epics for later /supervisor dispatch.
triggers:
  - "issue sweep"
  - "sweep issues"
  - "triage issues"
...
**Purpose**: Run **one cycle** of the open-issue sweep on `nicsuzor/academicOps`. Classify a batch (≤ 20) of open issues against a fixed disposition rubric, present the proposed dispatch plan to the user, wait for sign-off, then execute the confirmed actions (close / comment / file polecat task / **queue** fix-epic). Fix-epics are left `queued` for the user to run via `/supervisor` in a later session — this command does NOT invoke `/supervisor` inline. Record cycle outcomes in the loop epic body. **HALT after one cycle** — re-invoke `/issue-sweep` for the next.
...
| `fix-epic`       | Multi-step, multi-file, or design-required (no obvious AC without scoping)                                                                                                                                                                                                                                                          | Propose epic to user. On confirmation: create epic + decompose, leave `queued` for user-chosen `/supervisor` dispatch in a later session | `triaged-epic`                                                   |
...
5. **`fix-epic`** (only those confirmed by the user): create epic via `mcp__pkb__create_task` parented under the appropriate component epic (NOT under `epic-a0523a25` — per its Coherence rule, per-issue fixes are not children of the loop epic). Decompose into subtasks. Per P#109, create a `verify-parent` task that depends on all subtasks — this task confirms the epic's bundled issues are fully resolved after implementation completes. Leave the epic in `queued` status. Stamp each bundled issue with `triaged-epic` and a comment linking the epic ID, noting that `/supervisor` is not auto-invoked.

**Do NOT invoke `/supervisor` from this command.** Running N supervisor sessions inline conflicts with the halt-after-one-cycle principle in step 8, and the user is better placed than this command to sequence which queued epic runs first (prerequisites, blast radius, current branch state). The cycle log (step 6) surfaces the new epic IDs and recommends a prioritised dispatch order so the user — or a subsequent `/supervisor` invocation — can act on them. The `queued` status IS the dispatch handoff.
...
- **Does not fix any issue.** Single-tasks and fix-epics are queued in PKB, not executed in this command. Their actual work is done by polecat workers and `/supervisor`-driven sub-agents in later sessions.
- **Does not invoke `/supervisor`.** Fix-epics are left in `queued` status with a prioritised dispatch order in the cycle log; the user picks which epic to run next. Inline `/supervisor` invocation per fix-epic would conflict with the halt-after-one-cycle principle (step 8).
...
| Stamping `triaged-epic` before the user's `y` arrives                  | Violates loop-epic quality guarantee #4 (no silent dispatch) — even the label is dispatch | All stamps live in step 5, AFTER step 4's gate returns `y`. Never overlap.                                |
| Invoking `/supervisor` from inside this command                        | Conflicts with halt-after-one-cycle (step 8); user is better placed to sequence dispatch  | Leave fix-epics `queued`; surface prioritised order in the cycle log; user dispatches in a later session. |
