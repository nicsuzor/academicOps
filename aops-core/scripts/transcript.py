    ...
    if args.all or not args.session_file:
        ...
        for s in sessions:
            ...
            try:
                ...
                session_summary.permission_denials = session_ctx.get("permission_denials", [])
                ...
                for _d in session_summary.permission_denials or []:
                    _dn = _d if isinstance(_d, dict) else {}
                    timeline_events.append(
                        {
                            "timestamp": None,
                            "type": "permission_denial",
                            "tool": _dn.get("tool_name"),
                            "tool_use_id": _dn.get("tool_use_id"),
                            "description": (
                                f"auto-mode denied {_dn.get('tool_name')} "
                                f"({_dn.get('tool_use_id')})"
                            ),
                        }
                    )
