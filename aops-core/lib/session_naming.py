...
# Clients that are never a human typing interactively — every session launched
# under one of these is an automated/worker run. Bare terminal launches
# (``claude-code``) and Claude Cowork (``cowork``, i.e. Nic on claude.ai) are
# deliberately absent: those are interactive surfaces.
AUTOMATED_CLIENTS = frozenset(
...
def is_automated_session(
    *,
    ...
    files parsed by ``user_prompts.py`` do not carry these on the summary, so the
    path is the load-bearing signal there.
    ...
    # Subagent sessions are commissioned sub-runs, not a human typing.
    if subagent_type or parent_session:
        return True, "subagent"

    # stop-hook-feedback: the only "user" turn is the injected honesty reminder.
    if slug == "stop-hook-feedback":
        return True, "stop-hook"

    # Crew identity is data-about-the-session (set from the crew/<name>/ path or
    # filename shortform), never the transcribing runtime.
    if crew:
        return True, "crew"

    # Path markers: subagent dirs and the polecats/ tree.
    if session_path is not None:
        p = str(session_path)
        if "/polecats/" in p or "polecats/" in p:
            return True, "polecat"
        low = p.lower()
        if "subagent" in low or "/agent-" in p or "/subagents/" in p:
            return True, "subagent"

    # Derive surface/client from the path when not given explicitly.
    if (client is None or surface is None) and session_path is not None:
        origin = infer_session_origin_from_path(session_path, provider=provider)
        client = client or origin.get("client")
        surface = surface or origin.get("surface")

    if client in AUTOMATED_CLIENTS:
        return True, client
