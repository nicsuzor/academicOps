...
| Framework infrastructure, hooks, skills, AXIOMS, self-aware core     | `framework`                   | Changes to framework infrastructure   |
...
### Agentic-First Design (P#49 — No Shitty NLP)

This framework runs on agentic platforms: Claude Code, Gemini CLI, Jules, GitHub agents. These agents ARE the LLM.

**Do NOT propose** scripts or tools that call LLM APIs programmatically (Python wrappers around Anthropic/OpenAI SDK, custom evaluation harnesses, scoring pipelines). Instead, design the work as a **skill, workflow, or agent task** that a capable agent executes directly. Smarts should be agentic; code should be minimised.

Two forms of the same anti-pattern:

- **Classic**: regex/keyword matching for semantic decisions
- **Shiny**: writing code that calls an LLM API instead of delegating to an agent that IS an LLM
