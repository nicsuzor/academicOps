...
Almost all agent actions should follow skill invocation for repeatability.
...
Use unique names across all namespaces (skills, commands, hooks, agents).
...
Create persistent instructions or don't promise. Verbal commitments without framework support will be forgotten.
...
Use memory server for semantic search, never grep markdown for knowledge retrieval.
...
Never modify runtime config directly. Edit source files and run setup to regenerate.
...
Plans must be reviewed by critic agent before presenting to user.
...
Don't restart Streamlit after changes. It hot-reloads automatically.
...
Document whether content is for script processing or LLM reading. Workflow differs.
...
Strip deliberation from specs. Keep only the resolved decisions.
...
Create scripts, never inline Python in markdown or prompts.
...
Describe USER outcomes, not implementation details.

**Derivation**: Implementation-focused criteria miss the point. User outcomes define success.

## Semantic vs Episodic Storage (P#46)

Classify content before creating. Semantic (timeless truth) goes to $ACA_DATA. Episodic (observations) goes to bd issues.
...
Feature development includes acceptance tests. No feature is complete without verification.
...
TodoWrite for approved steps only. Use bd/tasks for work that spans sessions.
...
Before attributing failure to a specific cause (model, component, configuration), verify the attribution with evidence. Ask: "What unwarranted assumptions am I making?"
...
Hooks never call LLM directly. Spawn background subagent instead.
...
Use real captured data for tests, not fabricated examples.
...
Check/update governing spec first before modifying framework files.
...
When verifying outcomes, use LLM semantic understanding to evaluate whether the INTENT was satisfied. NEVER use keyword/substring matching.
...
Demo tests must expose the ENTIRE INTERNAL WORKING of the feature being demonstrated - all intermediate states, decision points, and data transformations. "Full output" means making the feature's internal machinery visible, not just printing the final response without truncation.
...
Test real scenarios, not contrived examples that pass surface checks.
...
Use observable side-effects for verification, not response text parsing.
...
Use headings for structure, not horizontal lines (`---`, `***`, `___`). Horizontal lines are visual noise.
